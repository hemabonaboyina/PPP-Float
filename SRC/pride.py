...
def _rts_smooth(fwd_results, nom):
    ...
    N=len(data)
    # v83: dim inferred from stored state — 5 for single constellation, 6 for GE (with ISB)
...
def _proc_gal(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    # CHANGE 1: E1 missing → satellite truly unusable.
    if P1_val == 0. or L1_val == 0.:
        return None
    # CHANGE 2: E5a missing → keep in KF, disable AR only.
    _e5a_missing = (P5_val == 0. or L5_val == 0.)
    ...
    # v82 PART 1: AR eligibility — E5a absence takes priority over OSB check.
    # CHANGE 3: no_AR=True immediately when E5a missing; OSB check only when E5a present.
    no_AR = False
    ar_skip_reason = None
    if _e5a_missing:
        no_AR = True
        ar_skip_reason = 'no_e5a'
    elif (code1_type not in ob or code2_type not in ob or
            phase1_type not in ob or phase2_type not in ob):
        no_AR = True
        ar_skip_reason = 'no_osb'
    ...
    P1c = P1 - b_C1
    # CHANGE 4a: P2c/L2m only meaningful when E5a present; use 0.0 as sentinel.
    P2c = P5 - b_C5 if not _e5a_missing else 0.0

    # RAW phase observables converted to metres, then phase OSB removed
    lam1 = LAMBDA_E1
    lam2 = LAMBDA_E5A
    L1m  = L1 * lam1 - b_L1
    L2m  = L5 * lam2 - b_L5 if not _e5a_missing else 0.0
    ...
    # CHANGE 4b: MW/GF require both frequencies; set to 0 when E5a absent.
    # WL/NL pipeline already gates on no_AR=True so these values are never used.
    bl1 = b_L1; bl5 = b_L5
    if not _e5a_missing:
        ...
        b_wl_sat_cyc=0.0; MW_cyc=0.0; GF_m=0.0
    ...
    return dict(sid=sid,unit=unit,mh=mh,mw=mw,el=el,
                ...
                ar_skip_reason=ar_skip_reason,  # v79: 'no_osb' | 'bad_bias' | None
                use_for_ppp=True,               # FIX: Galileo always enters KF when E1 present
                use_for_ar=not no_AR,           # FIX: AR only when E5a+OSB present
                _fallback_used=_e5a_missing)    # FIX: True when E5a absent (E1-only mode)
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    N_GLOBAL = 6 if constellation == 'GE' else 5
    ISB_IDX  = 5 if constellation == 'GE' else None   # slot for GPS ISB
    ...
    if ISB_IDX is not None: P[ISB_IDX, ISB_IDX] = 25.**2   # tighter init: ~25 m (typical GPS-GAL IF offset < 10 m)
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            ...
            # ── v90: C1C/L1C-ONLY GALILEO PHASE EXCLUSION ────────────────────
            # Detect single-frequency Galileo (E5a absent).  These satellites
            # carry unresolvable ionosphere on the single L1 phase row and
            # contaminate the dual-frequency ionosphere-estimating filter.
            # They are fully excluded here — no state, no H rows, no AR.
            # Dual-frequency Galileo (_fallback_used=False) is untouched.
            # GPS is untouched (sid[0]=='G' branch never enters this block).
            _is_c1c_only_gal = (sid[0] == 'E' and m.get('_fallback_used', False))
            if _is_c1c_only_gal:
                _c1c_excl_count += 1
                print(f"[C1C_PHASE_EXCLUDED] SOD={sod:.0f} sat={sid} "
                      f"— E5a absent, L1 phase excluded from PPP filter")
                _c1c_excl_csv_w.writerow([
                    f"{sod:.1f}", sid,
                    1,   # c1c_only_detected
                    1,   # phase_excluded
                    0,   # code_retained (fully excluded, not code-only)
                    0,   # ambiguity_created
                    0,   # ionosphere_created
                    0,   # rows_added
                    0,   # used_geometry_only (fully excluded)
                ])
                continue   # skip ALL further processing for this satellite
            # ── end v90 ───────────────────────────────────────────────────────
            ...
            if slip:
                ...
                _ar_gf_I      = (m['P2c'] - m['P1c']) / (m['gamma'] - 1.) \
            ...
            if not phi.get(sid,False):
                ...
                if not _fallback_used:
                    _sigma_new_fresh = max(SIGMA_IONO_FLOOR,
                                           K_SIGMA_IONO * abs(I_init))
                else:
        ...
        for ri,m in enumerate(geom):
            ...
            _omit_p2l2 = (
                m['sid'][0] == 'E' and
                (m.get('_fallback_used', False) or m['sid'] in _E5A_OMIT_SATS)
            )
        ...
        # ── RMS-SPLIT DIAGNOSTICS (v85-diag) — READ-ONLY ─────────────────────
        # All residuals are computed from the CURRENT post-update state (x, P).
        ...
        for _rm in geom:
            ...
            _rms_omit_p2l2 = (
                _rm['sid'][0] == 'E' and
                (_rm.get('_fallback_used', False) or _rm['sid'] in _E5A_OMIT_SATS)
            )

            # Post-fit P1 residual
            _res_p1 = _rm['P1c'] - (_rrp + x[_rki])
            ...
            if _rms_omit_p2l2:
                ...
                # Only an L1 row entered the KF; no dual-freq iono cancellation.
                # The L1 residual carries the full STEC (up to 27 m at IISC peak).
                # Route to single-freq accumulators ONLY — excluded from the
                # dual-freq L1/L2 split so nL1_dual stays equal to nL2_dual.
                _rms_P1_single.append(_res_p1)
                _rms_L1_single.append(_res_l1)
            else:
                # ── Dual-frequency satellite (L1+L2 both in KF) ─────────────
                # Compute L2/P2 residuals and add all four together, atomically,
                # so nP1 == nL1 == nP2 == nL2 is always maintained.
                _res_p2 = _rm['P2c'] - (_rrp + _rgam * x[_rki])
        ...
        # After the fix: all four dual-freq lists are appended atomically
        # → nP1 == nL1 == nP2 == nL2 is guaranteed at all times.
        # nL1_single is independent and has no required relationship to nL2.
        assert len(_rms_P1) == len(_rms_L1) == len(_rms_P2) == len(_rms_L2), (
        ...
        if _l1_rms is not None and _l1_rms > 1000.0:  # > 1 m  (1000 mm)
        ...
        # Separately report anomalous single-freq L1 for traceability:
        ...
        _rms_csv_w.writerow([
            f"{sod:.1f}",
            # Dual-frequency columns (nL1 == nL2 guaranteed)
            ...
            # Single-frequency C1C-only Galileo columns
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS+Galileo PPP v90 — C1C-Only Galileo Phase Excluded")
    print("="*72)
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation (v91) — Adaptive IONEX Iono Sigma',
