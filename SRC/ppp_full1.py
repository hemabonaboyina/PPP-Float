...
def _rts_smooth(fwd_results, nom, REF):
    ...
    N   = len(data)
    dim = 6                          # smooth position(3) + clock(1) + ZWD(1) + ISB(1)
    ...
    for k in range(N-2, -1, -1):
        ...
        Q_k[5,5]        = 1e-10 * dt  # ISB: near-constant hardware bias
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    x=np.zeros(6); x[3]=iclk; x[4]=izwd; x[5]=0.0   # state 5 = Galileo ISB
    P=np.zeros((6,6))
    ...
    # In GPS-only or Galileo-only mode this state is never observed and
    # stays at zero — harmless because it multiplies a zero design matrix row.
    P[5,5]=1000.**2
    _ISB_IDX = 5   # index of the Galileo ISB state
    ...
    for epoch in eplist:
        ...
        Q[4,4]=1e-7*dt
        # Galileo ISB: near-constant hardware bias, very small random walk.
        # 1e-10 m²/s allows <1mm drift per day while still being estimable.
        Q[5,5]=1e-10*dt
        ...
        for sid,so in sorted(sobs.items()):
            ...
            if not phi.get(sid,False):
                if sid in _amb_init:
                    ...
                    isb_now = x[_ISB_IDX] if m.get('_sys')=='E' else 0.
        ...
        for ri,m in enumerate(geom):
            ...
            # for Galileo sats only. The ISB state absorbs the Galileo system
            # time offset relative to GPS time that isn't in the clock products.
            if is_gal:
                rp = rp + xs[_ISB_IDX]
            rr=2*ri
            H[rr,0]=-u[0]; H[rr,1]=-u[1]; H[rr,2]=-u[2]; H[rr,3]=1.; H[rr,4]=mw
            if is_gal: H[rr,_ISB_IDX]=1.   # ISB partial derivative for Galileo
            z[rr]=m['PIF']-rp; Rd[rr]=_sig(m['el'],SC)**2
            rl=2*ri+1
            H[rl,0]=-u[0]; H[rl,1]=-u[1]; H[rl,2]=-u[2]; H[rl,3]=1.; H[rl,4]=mw; H[rl,ki]=1.
            if is_gal: H[rl,_ISB_IDX]=1.   # ISB for phase as well
        ...
        phase_res_now=[m['LIFc']-(_rp(m,x[3],x[4])+(x[_ISB_IDX] if m.get('_sys')=='E' else 0.)+x[m['ki']]) for m in geom]
        ...
        phase_res_mature=[m['LIFc']-(_rp(m,x[3],x[4])+(x[_ISB_IDX] if m.get('_sys')=='E' else 0.)+x[m['ki']])
        ...
        code_res=[(m['PIF']-_rp(m,x[3],x[4])-(x[_ISB_IDX] if m.get('_sys')=='E' else 0.))
                  for m in geom]
        ...
        ISB=x[_ISB_IDX]   # Galileo inter-system bias in metres
...
if __name__=='__main__':
    ...
    postpos(None,None,0.,0.,PrcOpt(),SolOpt(),FilOpt(),
            INFILES,os.path.join(DATA,'ppp_resultsfull1.csv'))