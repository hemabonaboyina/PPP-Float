...
def _ppp_pass(epochs, sp3t, sp, sc, clkd, osb, ah, nom, iclk, izwd,
              ...
              elm=math.radians(10.), SC=3.00, SP=0.012,  # SP-EXPERIMENT: 4×σ → 16×R_phase (was 0.006)
    ...
    NEWBORN_DECOUPLE_EPOCHS = 20  # clock-decoupling taper: H_clk = age/20 for age ≤ 20
    STARTUP_AMB_RAMP_EPOCHS = 60  # ambiguity participation ramp: H_amb = age/60 for age < 60
    ...
    # ── NEWBORN-CLK-DECOUPLE accumulators (REMOVED — ramp-only experiment) ──
    # H[clk] tapered logic deleted; clock column = 1.0 for ALL epochs.
    _clk_series          = []   # post-update receiver clock (m) per epoch
    # ──────────────────────────────────────────────────────────────────────────

    # ── STARTUP-AMB-RAMP accumulators ─────────────────────────────────────────
    # Track how ambiguity H-column scaling affects observability during startup.
    _ramp_n_rows_affected   = 0    # phase rows where _amb_scale < 1.0
    _ramp_scales_all        = []   # every _amb_scale value for affected rows
    # Startup phase: epochs 0..(STARTUP_AMB_RAMP_EPOCHS-1), mature: beyond
    _ramp_startup_sign_nu   = []   # signed residuals during startup (for sign coherence)
    _ramp_mature_sign_nu    = []   # signed residuals during mature phase
    _ramp_startup_dclk      = []   # |Δclock| per epoch during startup
    _ramp_mature_dclk       = []   # |Δclock| per epoch during mature
    _ramp_startup_code_rms  = []   # code RMS per epoch during startup
    _ramp_mature_code_rms   = []   # code RMS per epoch during mature
    _ramp_startup_amb_sigma = {}   # sid → list of amb_sigma_mm during startup
    _ramp_prev_clk_m        = None  # previous clock value for Δclock computation
    # ──────────────────────────────────────────────────────────────────────────
    ...
    for epoch in eplist:
        ...
        for ri, m in enumerate(geom):
            ...
            # state and the standard observation-domain residual checks below.
                ...
                if not math.isfinite(phase_res):
                    ...
                    H[rl,4]=mw
                    # ── STARTUP-AMB-RAMP: taper H[amb] for young ambiguities ─────
                    # Hypothesis: newborn/young ambiguities participating at full
                    # weight before the clock state is observable drives the filter
                    # into a wrong-equilibrium lock-in (large code-residual plateau).
                    # Only H[rl,ki] (ambiguity column) is scaled; H[pos], H[clk],
                    # H[ZWD], and all of z, R, ambiguity state are UNCHANGED.
                    _age_ramp = max(0, _ep_birth_aud)
                    if _age_ramp >= STARTUP_AMB_RAMP_EPOCHS:
                        _amb_scale = 1.0
                    else:
                        _amb_scale = _age_ramp / float(STARTUP_AMB_RAMP_EPOCHS)
                    H[rl, ki] = _amb_scale
                    # Per-row forensic diagnostic (only when scale < 1)
                    if _amb_scale < 1.0:
                        _ramp_n_rows_affected += 1
                        _ramp_scales_all.append(_amb_scale)
                        print(
                            f"  [AMB-RAMP]"
                            f"  SOD={sod:.0f}"
                            f"  sat={m['sid']}"
                            f"  age={_age_ramp}ep"
                            f"  scale={_amb_scale:.4f}"
                            f"  amb_sigma={_amb_sig_aud:.1f}mm"
                        )
                    # Track startup amb sigma evolution (first STARTUP_AMB_RAMP_EPOCHS per arc)
                    if _age_ramp < STARTUP_AMB_RAMP_EPOCHS:
                        _sid_r = m['sid']
                        if _sid_r not in _ramp_startup_amb_sigma:
                            _ramp_startup_amb_sigma[_sid_r] = []
                        _ramp_startup_amb_sigma[_sid_r].append(_amb_sig_aud)
                    # ── RAMP-ONLY EXPERIMENT: H[clk] = 1.0 unconditionally ──────
                    # Newborn clock decouple REMOVED.  Clock column participates
                    # at full weight (1.0) for ALL satellite ages and ALL epochs.
                    # Only H[rl,ki] (ambiguity column) is tapered by the ramp above.
                    H[rl,3] = 1.
                    # ─────────────────────────────────────────────────────────────
                    z[rl]  = phase_res
                    phase_sig = _sig(m['el'], SP)
                    # Apply Huber weight: R_eff = R_base / w²
                    # Large residuals → w < 1 → R_eff grows → Kalman gain shrinks.
                    Rd[rl] = phase_sig**2 / (_w_huber * _w_huber)
        ...
        # ── ROOT-TRIGGER PATCH 6 ASSERTION #3: mean phase innov > 10 m ─────
        if _ph_innov_list:
        ...
        # ── PATCH-3: STARTUP CLOCK UPDATE LIMITER ────────────────────────────
        ...
        # ── STARTUP-AMB-RAMP: per-epoch signed residual and Δclock tracking ──
        _ramp_is_startup = (nproc < STARTUP_AMB_RAMP_EPOCHS)
        # Accumulate signed phase residuals for this epoch into the right bucket
        for _ramp_nu in _all_phase_res_mm:   # list built earlier this epoch
            if _ramp_is_startup:
                _ramp_startup_sign_nu.append(_ramp_nu)
            else:
                _ramp_mature_sign_nu.append(_ramp_nu)
        # Δclock using post-update clock
        _cur_clk_m = float(x[3])
        if _ramp_prev_clk_m is not None:
            _dclk_ep = abs(_cur_clk_m - _ramp_prev_clk_m) * 1e3  # mm
            if _ramp_is_startup:
                _ramp_startup_dclk.append(_dclk_ep)
            else:
                _ramp_mature_dclk.append(_dclk_ep)
        _ramp_prev_clk_m = _cur_clk_m
        # (Code RMS for STARTUP-AMB-RAMP accumulated below after code_res_all is computed)
        # ────────────────────────────────────────────────────────────────────────
        ...
        # ── PSD monitor checkpoint C: post-filter_standard + symmetrisation ────
        _psd_check(P, _sidx_inv, 'POST-EKF', sod)
        ...
        # ── STARTUP-AMB-RAMP: accumulate code RMS after it's computed ───────
        if code_rms > 0:
            if nproc < STARTUP_AMB_RAMP_EPOCHS:
                _ramp_startup_code_rms.append(code_rms)
            else:
                _ramp_mature_code_rms.append(code_rms)
        # ────────────────────────────────────────────────────────────────────────
    ...
    print(f"\n{'='*72}")
    print(f"[FORENSIC-COVARIANCE-AUDIT]  SP=0.012 (16×R_phase vs baseline SP=0.003; 4×R vs SP=0.006)")
    print(f"{'='*72}")
    ...
    # ── AMB-RAMP-ONLY-AUDIT ──────────────────────────────────────────────────
    # Newborn clock decouple REMOVED.  Ambiguity ramp ONLY active.
    # This summary diagnoses whether the ramp alone suppresses the oscillation.
    print(f"\n{'='*72}")
    print(f"[AMB-RAMP-ONLY-AUDIT]  Ramp-only experiment (NbClkDecouple REMOVED)")
    print(f"  H[amb] = age/{STARTUP_AMB_RAMP_EPOCHS}  for age < {STARTUP_AMB_RAMP_EPOCHS} epochs")
    print(f"  H[clk] = 1.0  ALWAYS (no newborn clock taper)")
    print(f"{'='*72}")

    # ── 1. Sign coherence (oscillation diagnostic) ──────────────────────────
    print(f"\n  [ARO-1] SIGN COHERENCE (oscillation diagnostic)")
    if _fa_epoch_sign_coh:
        _aro_sc = sum(_fa_epoch_sign_coh) / len(_fa_epoch_sign_coh) * 100
        print(f"  Sign-coherent epoch fraction  : {_aro_sc:.1f}%")
        print(f"  Ramp+Decouple (previous run)  : 62.3%")
        print(f"  Huber only                    : 93.2%")
        print(f"  Baseline                      : ~70–85%")
        if _aro_sc < 70.:
            print(f"  ✓ Oscillation suppressed (< 70%) — ramp alone SUFFICIENT")
        elif _aro_sc < 85.:
            print(f"  ~ PARTIAL — ramp contributes but clock decouple also needed")
        else:
            print(f"  ✗ Oscillation RETURNED — ramp alone INSUFFICIENT;")
            print(f"    clock decouple was the effective ingredient")

    # ── 2. Common-mode lag-1 ACF of ΔClock ──────────────────────────────────
    print(f"\n  [ARO-2] CLOCK-SERIES ANALYSIS")
    if len(_clk_series) >= 3:
        _dclk_aro = [(_clk_series[i] - _clk_series[i-1]) * 1e3
                     for i in range(1, len(_clk_series))]
        _dclk_aro_mean_abs = sum(abs(d) for d in _dclk_aro) / len(_dclk_aro)
        _mu_aro = sum(_dclk_aro) / len(_dclk_aro)
        _c_aro  = [d - _mu_aro for d in _dclk_aro]
        _num_aro = sum(_c_aro[i] * _c_aro[i+1] for i in range(len(_c_aro)-1))
        _den_aro = sum(d*d for d in _c_aro)
        _lag1_aro = _num_aro / _den_aro if _den_aro > 0 else float('nan')
        _max_osc_aro = 1; _cur_osc_aro = 1
        for i in range(1, len(_dclk_aro)-1):
            if _dclk_aro[i] * _dclk_aro[i-1] < 0:
                _cur_osc_aro += 1
                _max_osc_aro = max(_max_osc_aro, _cur_osc_aro)
            else:
                _cur_osc_aro = 1
        print(f"  Mean |ΔClock|       : {_dclk_aro_mean_abs:.2f} mm/epoch")
        print(f"  Lag-1 ACF(ΔClock)   : {_lag1_aro:+.4f}")
        print(f"    (Ramp+Decouple previous: -0.2095  | target ≈ 0)")
        print(f"  Longest sign-flip run: {_max_osc_aro} epochs")
        if abs(_lag1_aro) < 0.15:
            print(f"  ✓ Lag-1 ACF near zero — clock noise approximates white noise")
        elif _lag1_aro < -0.15:
            print(f"  ✗ Lag-1 ACF strongly negative — sign-alternation oscillation persists")
    else:
        print(f"  Insufficient clock samples for ACF")

    # ── 3. Newborn phase residual metrics (first 20 epochs per arc) ─────────
    # Computed inline from _ramp_startup_sign_nu (residuals in startup window).
    print(f"\n  [ARO-3] NEWBORN / STARTUP PHASE RESIDUALS")
    _startup_res_aro = [r for r in _ramp_startup_sign_nu]  # all startup residuals
    if _startup_res_aro:
        _nb_rms_aro  = math.sqrt(sum(r*r for r in _startup_res_aro)/len(_startup_res_aro))
        _nb_max_aro  = max(abs(r) for r in _startup_res_aro)
        _mature_all  = [abs(r) for r in _fa_acc_nu_mm]
        _mat_rms_aro = math.sqrt(sum(r*r for r in _mature_all)/len(_mature_all)) if _mature_all else float('nan')
        _over200_cnt = sum(1 for r in _startup_res_aro if abs(r) > 200.)
        _startup_settle = None
        _running_window = [abs(r) for r in _startup_res_aro[:20]]
        if _running_window and max(_running_window, default=999) < 200.:
            _startup_settle = 0
        else:
            for _s_ep in range(1, len(_startup_res_aro)):
                if abs(_startup_res_aro[_s_ep]) < 200.:
                    _startup_settle = _s_ep; break
        print(f"  Startup phase RMS    : {_nb_rms_aro:.1f} mm  (n={len(_startup_res_aro)} rows)")
        print(f"  Max |startup res|    : {_nb_max_aro:.1f} mm")
        print(f"  Mature phase RMS     : {_mat_rms_aro:.1f} mm  (comparison)")
        print(f"  Rows > 200 mm        : {_over200_cnt}")
        print(f"  Ramp+Decouple RMS    : 3189.7 mm  (previous run reference)")
        if _nb_rms_aro < 500.:
            print(f"  ✓ Newborn residual RMS COLLAPSED vs Ramp+Decouple 3189 mm")
        elif _nb_rms_aro < 2000.:
            print(f"  ~ Partial improvement vs Ramp+Decouple 3189 mm")
        else:
            print(f"  ✗ Newborn residuals still large — decouple was needed to suppress spikes")
    else:
        print(f"  No startup residuals recorded")

    # ── 4. Code-phase delta (proxy for clock coupling) ───────────────────────
    # Use _ramp_startup_code_rms and _ramp_mature_code_rms already accumulated.
    print(f"\n  [ARO-4] CODE RMS  startup vs mature")
    _rcs = (sum(_ramp_startup_code_rms)/len(_ramp_startup_code_rms)
            if _ramp_startup_code_rms else float('nan'))
    _rcm = (sum(_ramp_mature_code_rms)/len(_ramp_mature_code_rms)
            if _ramp_mature_code_rms else float('nan'))
    print(f"  Startup code RMS     : {_rcs:.1f} mm  (n={len(_ramp_startup_code_rms)} epochs)")
    print(f"  Mature  code RMS     : {_rcm:.1f} mm  (n={len(_ramp_mature_code_rms)} epochs)")
    if math.isfinite(_rcs) and math.isfinite(_rcm) and _rcm > 0:
        _aro_ratio = _rcs / _rcm
        print(f"  Ratio startup/mature : {_aro_ratio:.3f}")

    # ── 5. Experiment progression table ──────────────────────────────────────
    print(f"\n  [ARO-5] EXPERIMENT PROGRESSION")
    print(f"  {'Experiment':<40s}  {'sign-coh%':>9s}  {'NIS':>6s}  {'lag1ACF':>8s}  {'nb_rms mm':>9s}")
    print(f"  {'─'*40}  {'─'*9}  {'─'*6}  {'─'*8}  {'─'*9}")
    print(f"  {'Baseline (SP=0.003)':<40s}  {'~70–85%':>9s}  {'5.21':>6s}  {'?':>8s}  {'?':>9s}")
    print(f"  {'SP=0.012 + Huber':<40s}  {'93.2%':>9s}  {'2.01':>6s}  {'?':>8s}  {'?':>9s}")
    print(f"  {'SP=0.012 + Huber + NbClkDecouple':<40s}  {'62.3%':>9s}  {'5.21':>6s}  {'-0.2095':>8s}  {'3189':>9s}")
    _aro_nis = _fa_mean(_fa_acc_nis) if _fa_acc_nis else float('nan')
    _aro_sc_str = f"{_aro_sc:.1f}%" if _fa_epoch_sign_coh else "?"
    _aro_lag_str = f"{_lag1_aro:+.4f}" if len(_clk_series) >= 3 else "?"
    _aro_nb_str  = f"{_nb_rms_aro:.0f}" if _startup_res_aro else "?"
    print(f"  {'+ RampOnly (this exp — NbClkDecouple off)':<40s}  {_aro_sc_str:>9s}  {_aro_nis:>6.2f}  {_aro_lag_str:>8s}  {_aro_nb_str:>9s}")

    # ── 6. Verdict ────────────────────────────────────────────────────────────
    print(f"\n  [ARO-VERDICT]")
    print(f"  SUCCESS (ramp alone sufficient):")
    print(f"    • sign coherence < 70%  AND  newborn RMS collapses  AND  lag-1 ACF ≈ 0")
    print(f"  FAILURE MODE A (oscillation returns):")
    print(f"    • sign coherence ≥ 85% → clock decouple was the oscillation-breaking element")
    print(f"    → NEXT: keep BOTH ramp AND decouple, fix the 9–13 m spike separately")
    print(f"  FAILURE MODE B (newborn spikes persist):")
    print(f"    • newborn RMS > 2000 mm → ambiguity birth equation / prior is wrong")
    print(f"    → NEXT: investigate birth initialisation, covariance, or prior mismatch")
    _aro_verdict = "UNKNOWN"
    if _fa_epoch_sign_coh and _startup_res_aro:
        if _aro_sc < 70. and _nb_rms_aro < 500.:
            _aro_verdict = "SUCCESS"
        elif _aro_sc >= 85.:
            _aro_verdict = "FAILURE-A (oscillation returned; clock decouple needed)"
        elif _nb_rms_aro >= 2000.:
            _aro_verdict = "FAILURE-B (newborn spikes persist; birth equation suspect)"
        else:
            _aro_verdict = "PARTIAL"
    print(f"\n  RAMP-ONLY VERDICT: {_aro_verdict}")
    print(f"{'='*72}\n")

    # ── STARTUP-AMB-RAMP AUDIT ───────────────────────────────────────────────
    print(f"\n{'='*72}")
    print(f"[RAMP-AUDIT]  Startup Ambiguity Participation Ramp Experiment")
    print(f"  STARTUP_AMB_RAMP_EPOCHS = {STARTUP_AMB_RAMP_EPOCHS}")
    print(f"  H[amb] = age/{STARTUP_AMB_RAMP_EPOCHS}  for age < {STARTUP_AMB_RAMP_EPOCHS} epochs")
    print(f"  H[amb] = 1.0          for age >= {STARTUP_AMB_RAMP_EPOCHS} epochs")
    print(f"{'='*72}")

    _ramp_total_rows = len(_rw_all_weights)
    _ramp_frac = 100. * _ramp_n_rows_affected / max(_ramp_total_rows, 1)
    print(f"\n  [RAMP-1] SCALE STATISTICS")
    print(f"  Total phase rows entered EKF : {_ramp_total_rows}")
    print(f"  Rows with _amb_scale < 1.0   : {_ramp_n_rows_affected}  ({_ramp_frac:.1f}%)")
    if _ramp_scales_all:
        _ramp_mean_scale = sum(_ramp_scales_all) / len(_ramp_scales_all)
        _ramp_min_scale  = min(_ramp_scales_all)
        print(f"  Mean amb scale (affected)    : {_ramp_mean_scale:.4f}")
        print(f"  Min  amb scale               : {_ramp_min_scale:.4f}")
    else:
        print(f"  No rows had scale < 1.0")

    # [RAMP-2] Sign coherence startup vs mature
    print(f"\n  [RAMP-2] SIGN COHERENCE  startup(<{STARTUP_AMB_RAMP_EPOCHS}ep) vs mature")
    def _ramp_sign_maj(nu_list):
        # Per-row sign majority fraction (proxy for systematic bias direction)
        if not nu_list:
            return float('nan')
        pos = sum(1 for r in nu_list if r > 0)
        neg = sum(1 for r in nu_list if r < 0)
        tot = pos + neg
        return max(pos, neg) / tot * 100. if tot else float('nan')

    _rsc_startup = _ramp_sign_maj(_ramp_startup_sign_nu)
    _rsc_mature  = _ramp_sign_maj(_ramp_mature_sign_nu)
    print(f"  Startup sign-majority% : {_rsc_startup:.1f}%  (n={len(_ramp_startup_sign_nu)} rows)")
    print(f"  Mature  sign-majority% : {_rsc_mature:.1f}%  (n={len(_ramp_mature_sign_nu)} rows)")
    if math.isfinite(_rsc_startup) and math.isfinite(_rsc_mature):
        if _rsc_startup > _rsc_mature + 5:
            print(f"  \u2717 Startup residuals MORE coherent — startup equilibrium bias likely")
        elif _rsc_startup < _rsc_mature - 5:
            print(f"  \u2713 Startup residuals LESS coherent — ramp reduced early bias lock-in")
        else:
            print(f"  ~ Startup / mature sign coherence similar")

    # [RAMP-3] Mean |Δclock|
    print(f"\n  [RAMP-3] MEAN |\u0394Clock| mm/epoch  startup vs mature")
    _r_dclk_s = (sum(_ramp_startup_dclk) / len(_ramp_startup_dclk)
                 if _ramp_startup_dclk else float('nan'))
    _r_dclk_m = (sum(_ramp_mature_dclk) / len(_ramp_mature_dclk)
                 if _ramp_mature_dclk else float('nan'))
    print(f"  Startup |\u0394Clock| mean  : {_r_dclk_s:.2f} mm/ep  (n={len(_ramp_startup_dclk)} epochs)")
    print(f"  Mature  |\u0394Clock| mean  : {_r_dclk_m:.2f} mm/ep  (n={len(_ramp_mature_dclk)} epochs)")
    _r_dclk_ratio = float('nan')
    if math.isfinite(_r_dclk_s) and math.isfinite(_r_dclk_m) and _r_dclk_m > 0:
        _r_dclk_ratio = _r_dclk_s / _r_dclk_m
        print(f"  Ratio startup/mature   : {_r_dclk_ratio:.2f}")
        if _r_dclk_ratio < 0.7:
            print(f"  \u2713 Startup \u0394clock excursions REDUCED")
        elif _r_dclk_ratio > 1.3:
            print(f"  \u2717 Startup \u0394clock excursions LARGER")
        else:
            print(f"  ~ Startup / mature \u0394clock similar")

    # [RAMP-4] Code RMS startup vs mature
    print(f"\n  [RAMP-4] CODE RMS  startup vs mature")
    _r_crms_s = (sum(_ramp_startup_code_rms) / len(_ramp_startup_code_rms)
                 if _ramp_startup_code_rms else float('nan'))
    _r_crms_m = (sum(_ramp_mature_code_rms) / len(_ramp_mature_code_rms)
                 if _ramp_mature_code_rms else float('nan'))
    print(f"  Startup code RMS mean  : {_r_crms_s:.1f} mm  (n={len(_ramp_startup_code_rms)} epochs)")
    print(f"  Mature  code RMS mean  : {_r_crms_m:.1f} mm  (n={len(_ramp_mature_code_rms)} epochs)")
    _r_crms_ratio = float('nan')
    if math.isfinite(_r_crms_s) and math.isfinite(_r_crms_m) and _r_crms_m > 0:
        _r_crms_ratio = _r_crms_s / _r_crms_m
        print(f"  Ratio startup/mature   : {_r_crms_ratio:.3f}")
        if _r_crms_ratio < 0.85:
            print(f"  \u2713 Startup code RMS LOWER — early equilibrium bias reduced by ramp")
        elif _r_crms_ratio > 1.15:
            print(f"  \u2717 Startup code RMS HIGHER — ramp did not collapse wrong equilibrium")
        else:
            print(f"  ~ Code RMS similar startup / mature")

    # [RAMP-5] Ambiguity sigma evolution
    print(f"\n  [RAMP-5] AMBIGUITY SIGMA EVOLUTION DURING STARTUP (first {STARTUP_AMB_RAMP_EPOCHS} epochs per arc)")
    if _ramp_startup_amb_sigma:
        _ramp_all_ss = [s for v in _ramp_startup_amb_sigma.values() for s in v]
        _ramp_n_arcs = len(_ramp_startup_amb_sigma)
        _ramp_mean_ss = sum(_ramp_all_ss) / len(_ramp_all_ss) if _ramp_all_ss else float('nan')
        _ramp_fast    = sum(1 for s in _ramp_all_ss if s < 100)
        print(f"  Satellites tracked     : {_ramp_n_arcs}")
        print(f"  Mean amb sigma         : {_ramp_mean_ss:.1f} mm  (n={len(_ramp_all_ss)} rows)")
        print(f"  Rows with sigma < 100mm: {_ramp_fast} / {len(_ramp_all_ss)}")
    else:
        print(f"  No startup ambiguity sigma data")

    # [RAMP-6] Experiment comparison table
    print(f"\n  [RAMP-6] EXPERIMENT PROGRESSION")
    print(f"  {'Experiment':<44s}  {'NIS':>6s}  {'Code RMS mm':>11s}")
    print(f"  {'─'*44}  {'─'*6}  {'─'*11}")
    print(f"  {'Baseline (SP=0.003)':<44s}  {'5.21':>6s}  {'37k–38k':>11s}")
    print(f"  {'SP=0.012 + Huber + NbClkDecouple':<44s}  {'2.21':>6s}  {'(see log)':>11s}")
    if _fa_acc_nis:
        _cur_nis  = _fa_mean(_fa_acc_nis)
        _crms_all = _ramp_startup_code_rms + _ramp_mature_code_rms
        _crms_tot = sum(_crms_all) / len(_crms_all) if _crms_all else float('nan')
        print(f"  {'+ RampOnly (NbClkDecouple off, this exp)':<44s}  {_cur_nis:>6.2f}  {_crms_tot:>10.0f}mm")

    # [RAMP-VERDICT]
    print(f"\n  [RAMP-VERDICT]")
    print(f"  SUCCESS: startup code RMS collapses AND early \u0394clock excursions reduce")
    print(f"           AND wrong-equilibrium lock-in (29\u201338m bias plateau) eliminated")
    print(f"  FAILURE: same code-RMS plateau persists \u2192 ambiguity authority is NOT the")
    print(f"           dominant trigger; wrong equilibrium has a deeper observability cause")
    if math.isfinite(_r_crms_ratio):
        if _r_crms_ratio < 0.85:
            _verdict = 'SUCCESS'
        elif _r_crms_ratio > 1.10:
            _verdict = 'FAILURE'
        else:
            _verdict = 'INCONCLUSIVE'
        print(f"  Code-RMS ratio={_r_crms_ratio:.3f}  VERDICT: {_verdict}")
    print(f"{'='*72}\n")
    # ──────────────────────────────────────────────────────────────────────────
