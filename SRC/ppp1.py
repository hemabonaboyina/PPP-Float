"""
ppp.py  v82 — NL Stability: Phase Quality Gate + Best-4, Tighter Constraint, Cooldown
===========================================================================
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    """GPS satellite — STRICT signal set: C1W + L1W + C2W + L2W ONLY.

    v83 STRICT: GPS requires C1W/L1W/C2W/L2W for PPP-AR signal consistency.
    No fallback signals. No mixed usage. If any of the four required signals
    is absent this epoch the satellite is skipped entirely (not included in
    the PPP filter or AR).
    """
    # STRICT GPS signal selection: C1W/L1W/C2W/L2W ONLY.
    # No fallback, no switching. If any signal absent this epoch, skip satellite entirely.

    no_AR = False   # set True when OSBs are absent for the chosen signal set

    # PART 1/2 — STRICT GPS signal set: C1W + L1W + C2W + L2W ONLY.
    # No fallback, no switching. If any required signal is absent, skip satellite entirely.
    GPS_FIXED_SIGS = ('C1W', 'C2W', 'L1W', 'L2W')

    # PART 3 — LOCK SIGNAL MAP: assign fixed signals once, reuse always.
    if sid not in _sat_signal_map:
        _sat_signal_map[sid] = GPS_FIXED_SIGS
    code1_type, code2_type, phase1_type, phase2_type = _sat_signal_map[sid]

    # PART 4 — NO SIGNAL SWITCHING: if any required signal is missing this epoch, skip entirely.
    P1_val = so.get(code1_type,  0.)
    L1_val = so.get(phase1_type, 0.)
    P2_val = so.get(code2_type,  0.)
    L2_val = so.get(phase2_type, 0.)
    if P1_val == 0. or L1_val == 0. or P2_val == 0. or L2_val == 0.:
        return None  # required C1W/L1W/C2W/L2W signals absent this epoch — skip entirely

    # PART 5 — VALIDATION CHECK: GPS must always use C1W↔L1W  C2W↔L2W.
    if code1_type != 'C1W' or phase1_type != 'L1W' or code2_type != 'C2W' or phase2_type != 'L2W':
        print(f"[SIGNAL ERROR] {sid} unexpected signal map: {_sat_signal_map[sid]}")
        return None
    ...
    # PART 6 — MODEL CONSISTENCY CHECK (DEBUG ONLY):
    # After OSB correction, L1-P1 should be within ~1 m (ambiguity + iono).
    # Values > 2 m indicate signal/OSB mismatch or missing bias correction.
    _L1_minus_P1 = L1m - P1c
    if abs(_L1_minus_P1) > 2.0:
        print(f"[MODEL ERROR] {sid} L1-P1={_L1_minus_P1:.2f} m")
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    # v82 PART 1: per-satellite phase residual history for diagnostics ONLY.
    # Appended each epoch with the L1 post-update residual (same value used for PhsRMS).
    # No filtering or rejection logic uses this — observation only.
    _phase_res_hist=defaultdict(lambda: deque(maxlen=50))
    ...
    # ── v82 PART 4: per-satellite phase residual CSV ─────────────────────────
    _phase_qual_csv_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "phase_quality_log.csv")
    _phase_qual_csv_fh = open(_phase_qual_csv_path, "w", newline="")
    _phase_qual_csv_w  = _csv.writer(_phase_qual_csv_fh)
    _phase_qual_csv_w.writerow(["epoch", "sat", "constellation",
                                "phase_rms_mm", "max_residual_mm", "used_for_NL"])

    # ── NL candidate selection debug CSV (v82) ───────────────────────────────
    _nl_debug_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "nl_debug_log.csv")
    _nl_debug_fh = open(_nl_debug_path, "w", newline="")
    _nl_debug_w  = _csv.writer(_nl_debug_fh)
    _nl_debug_w.writerow([
        "epoch", "sat", "sys",
        "sigma_N1_m", "range_100", "corr_frac", "frac_std",
        "selected", "reason"
    ])
    ...
    _nl_skip_phase_accum = 0  # v81 STEP 2: phase quality gate skips
    ...
    for epoch in eplist:
        ...
        # STRICT_SELECT tracking — populated each epoch, printed every 300 epochs.
        _strict_used    = []   # [(sid, sig_label)]
        _strict_skipped = []   # [(sid, missing_signal)]

        for sid,so in sorted(sobs.items()):
            if sid[0] not in ('G','E'): continue
            if sid[0] not in constellation: continue

            # Pre-check signal availability for STRICT_SELECT debug (no logic change).
            if sid[0]=='G':
                _req_sigs   = ('C1W','C2W','L1W','L2W')
                _sig_label  = 'C1W/C2W'
            else:
                _req_sigs   = ('C1C','C5Q','L1C','L5Q')
                _sig_label  = 'C1C/C5Q'
            _missing_sig = next((s for s in _req_sigs if not so.get(s,0.)), None)
            ...
            if sid[0]=='E':
                ...
                m=_proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,
                         satx,att,recx,blq=blq,sta=sta,tow_total=tow_total)
            if m is None:
                if _missing_sig is not None:
                    _strict_skipped.append((sid, _missing_sig))
                continue
            _strict_used.append((sid, _sig_label))
        ...
        # ── STRICT_SELECT debug (every 300 epochs) ───────────────────────────
        if epoch_idx % 300 == 0:
            print(f"[STRICT_SELECT] SOD={sod:.0f}")
            for _ss, _sl in _strict_used:
                _ar_ok = not next((m.get('no_AR',False) for m in geom if m['sid']==_ss), True)
                _tick  = '✔' if _ar_ok else '✔(no_osb)'
                print(f"  {_ss} → {_sl} {_tick} USED")
            for _ss, _sm in _strict_skipped:
                print(f"  {_ss} → missing {_sm} ❌ SKIPPED")
            print(f"  total_used_sats={len(_strict_used)}  "
                  f"total_skipped_sats={len(_strict_skipped)}")
        ...
        # --- HARD PHASE OUTLIER REJECTION (SAFE) ---
        # Pre-screen geom: remove satellites with catastrophic phase residuals (>2.5 m).
        # Residuals are computed here using the current (pre-update) state x.
        # This block is placed AFTER residual computation and BEFORE:
        #   - H matrix build, Kalman update, ambiguity update, NL fixing.
        # Only L1/L2 phase is checked. Code, iono, OSB, NL logic are entirely untouched.
        _xs_pre = x.copy()
        _geom_clean = []
        for _m_pre in geom:
            _ki_pre  = _m_pre['ki']
            _rp_pre  = _rp(_m_pre, _xs_pre[3], _xs_pre[4])
            _I_pre   = _xs_pre[_ki_pre]
            _N1_pre  = _xs_pre[_ki_pre + 1]
            _N2_pre  = _xs_pre[_ki_pre + 2]
            _res_L1  = _m_pre['L1mc'] - (_rp_pre - _I_pre + _m_pre['lam1'] * _N1_pre)
            _res_L2  = _m_pre['L2mc'] - (_rp_pre - _m_pre['gamma'] * _I_pre + _m_pre['lam2'] * _N2_pre)
            if not np.isfinite(_res_L1) or abs(_res_L1) > 2.5:
                if epoch_idx % 300 == 0:
                    print(f"[REJECT PHASE] sat={_m_pre['sid']} L1_res={_res_L1:.2f} m")
                continue
            if not np.isfinite(_res_L2) or abs(_res_L2) > 2.5:
                if epoch_idx % 300 == 0:
                    print(f"[REJECT PHASE] sat={_m_pre['sid']} L2_res={_res_L2:.2f} m")
                continue
            _geom_clean.append(_m_pre)
        geom = _geom_clean
        if len(geom) < 4: continue
        # --- END HARD PHASE OUTLIER REJECTION ---
        ...
        phase_res_now=[]
        _phase_res_sat={}   # v81 STEP 2: per-satellite phase residual (metres)
        for m in geom:
            ki=m['ki']; rp=_rp(m,x[3],x[4])
            res_L1=m['L1mc']-(rp - x[ki] + m['lam1']*x[ki+1])
            phase_res_now.append(res_L1)
            _phase_res_sat[m['sid']] = res_L1   # store for NL phase quality gate
            # v82 PART 1: append to per-satellite history (diagnostic only, no filtering)
            _phase_res_hist[m['sid']].append(res_L1)
            # --- PHASE RESIDUAL MONITOR (SAFE) ---
            if abs(res_L1) > 5.0:   # meters
                print(f"[PHASE WARN] SOD={sod:.0f} SAT={m['sid']} RES={res_L1:.2f} m")
        ...
        _ep_skip_sigma      = 0
        _ep_skip_phase      = 0   # v81 STEP 2: phase quality gate
        _ep_nl_count        = 0
        ...
        for _mps in geom:
            ...
            # v81 STEP 2: phase quality gate — mirror of main candidate loop.
            if abs(_phase_res_sat.get(_sps, 0.0)) > 2.0: continue
            _ki_ps = sidx[_sps]
        ...
        # --- v81 PART 7: balanced per-constellation Best-2+2 selection ---
        _gps_pre = [(s, sid) for s, sid in _nl_preselect if sid.startswith('G')]
        _gal_pre = [(s, sid) for s, sid in _nl_preselect if sid.startswith('E')]
        _gps_best = [sid for _, sid in _gps_pre[:2]]
        _gal_best = [sid for _, sid in _gal_pre[:2]]
        _nl_eligible_sids = set(_gps_best + _gal_best)
        # fallback: pad to 4 if one constellation is weak
        if len(_nl_eligible_sids) < 4:
            for _, sid in _nl_preselect:
                if sid not in _nl_eligible_sids:
                    _nl_eligible_sids.add(sid)
                if len(_nl_eligible_sids) == 4:
                    break
        print(f"[NL SELECT] GPS={len(_gps_best)} GAL={len(_gal_best)} TOTAL={len(_nl_eligible_sids)}")
        ...
        for m in geom:
            ...
            if _rng_sat > 6.0:
                ...
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    None, round(_rng_sat, 3), None, None,
                    0, "range"
                ])
                continue   # PART 5: do NOT remove from filter — just skip NL

            # --- PHASE QUALITY GATE (v81 STEP 2 — CRITICAL) ---
            # Block NL fixing for satellites whose L1 phase residual exceeds 2 m.
            # E34 produces ~5 m residuals; good sats are typically < 0.5–1 m.
            # Satellite is NOT removed from the PPP filter — only NL is withheld.
            _phase_res_m = _phase_res_sat.get(sid_m, 0.0)
            if abs(_phase_res_m) > 2.0:
                _ep_skip_phase += 1
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    None, round(_rng_sat, 3), None, None,
                ...
                print(f"[NL SKIP PHASE] {sid_m} res={_phase_res_m:.2f} m")
                continue
            ...
            if sigma_N1_m > 0.12:
                ...
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    round(sigma_N1_m, 3), round(_rng_sat, 3), None, None,
                    0, "sigma"
                ])
                continue
            ...
            if buf_n < 15:
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    round(sigma_N1_m, 3), round(_rng_sat, 3),
                    round(corr_frac, 4), round(frac_std, 4) if frac_std != 1.0 else None,
                    0, "buf_n"
                ])
            ...
            if frac_std >= _frac_std_gate:
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    round(sigma_N1_m, 3), round(_rng_sat, 3),
                    round(corr_frac, 4), round(frac_std, 4),
                    0, "unstable"
                ])
            ...
            if abs(frac_for_fix) > NL_RES_THRESH:
                _nl_debug_w.writerow([
                    nproc, sid_m, sid_m[0],
                    round(sigma_N1_m, 3), round(_rng_sat, 3),
                    round(frac_for_fix, 4), round(frac_std, 4),
                    0, "res_thresh"
                ])
                continue     # not near integer

            # Integer is taken from the bias-corrected value.  Filter state is NOT modified.
            N1_int = int(round(N1_corr))
            N2_int=N1_int-NWL_m

            # Log successful candidate (selected=1)
            _nl_debug_w.writerow([
                nproc, sid_m, sid_m[0],
                round(sigma_N1_m, 3), round(_rng_sat, 3),
                round(frac_for_fix, 4), round(frac_std, 4),
        ...
        # v82 PART 2+4: compute per-sat phase RMS and write CSV.
        # Done AFTER NL candidates are committed so used_for_NL is accurate.
        # PART 3: no filtering — satellites are not skipped here.
        for _m_pq in geom:
            _sid_pq = _m_pq['sid']
            _hist_pq = _phase_res_hist[_sid_pq]
            if len(_hist_pq) >= 10:
                _arr_pq = np.array(_hist_pq)
                _prms_pq = math.sqrt(float(np.mean(_arr_pq**2)))
                _maxr_pq = float(np.max(np.abs(_arr_pq)))
            else:
                _prms_pq = float('nan')
                _maxr_pq = float('nan')
            _used_nl_pq = 1 if _sid_pq in nl_fixed else 0
            _phase_qual_csv_w.writerow([
                nproc,
                _sid_pq,
                _sid_pq[0],
                f"{_prms_pq*1e3:.3f}" if not math.isnan(_prms_pq) else "nan",
                f"{_maxr_pq*1e3:.3f}" if not math.isnan(_maxr_pq) else "nan",
                _used_nl_pq,
            ])

        # Step 3 — per-epoch SUMMARY row
        _total_selected = _ep_nl_count
        _gps_count = sum(1 for s in nl_fixed if s.startswith('G'))
        _gal_count = sum(1 for s in nl_fixed if s.startswith('E'))
        _nl_debug_w.writerow([
            nproc, "SUMMARY", "",
            "", "", "", "",
            _total_selected,
            f"GPS={_gps_count},GAL={_gal_count}"
        ])
        ...
        _nl_skip_phase_accum += _ep_skip_phase   # v81 STEP 2
        ...
        if nproc % 300 == 0 and sidx:
            ...
            print(f"[NL_STATS] SOD={sod:.0f}"
                  ...
                  f"  skipped_phase={_nl_skip_phase_accum}"
            ...
            _nl_skip_phase_accum = 0   # v81 STEP 2
            _nl_count_accum      = 0
            reject_due_to_innov  = 0
            # PART 6 — SIG_LOCK debug: print locked signal set every 300 epochs
            for _sid_sl in sorted(_sat_signal_map.keys()):
                _sigs_sl = _sat_signal_map[_sid_sl]
                print(f"[SIG_LOCK] {_sid_sl} using {_sigs_sl[0]}/{_sigs_sl[2]} + {_sigs_sl[1]}/{_sigs_sl[3]}")
            # v82 PART 5: per-satellite phase RMS summary (no spam — every 300 ep)
            print("[PHASE_QUALITY]")
            for _sid_prt in sorted(sidx.keys()):
                _hist_prt = _phase_res_hist[_sid_prt]
                if len(_hist_prt) >= 10:
                    _arr_prt = np.array(_hist_prt)
                    _rms_prt = math.sqrt(float(np.mean(_arr_prt**2))) * 1e3
                    print(f"  {_sid_prt}  rms={_rms_prt:.2f}mm"
                          f"{'  [NL_FIXED]' if _sid_prt in nl_fixed else ''}")
    ...
    _nl_debug_fh.flush()
    _nl_debug_fh.close()
    print(f"[CSV]  nl_debug_log.csv → {_nl_debug_path}")
    _phase_qual_csv_fh.flush()
    _phase_qual_csv_fh.close()
    print(f"[CSV]  phase_quality_log.csv → {_phase_qual_csv_path}")
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation (v82) — NL Stability: Phase Quality Gate + Best-4, Tighter Constraint, Cooldown',
