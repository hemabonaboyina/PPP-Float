...
def _mw_cyc(P1, P2, L1_cyc, L2_cyc):
    ...
    # MW in meters first
    mw_m = (L1_m - L2_m) - (P1_m - P2_m)

    # Convert to WL cycles
    return mw_m / LAMBDA_WL
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx):
    ...
    MW_cyc = _mw_cyc(P1, P2, L1, L2)   # NOTE: uses corrected P1,P2 (with OSB)
    if abs(MW_cyc) > 100:
       print(f"[DEBUG MW] {sid} abnormal MW = {MW_cyc:.2f}")
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            ...
            if sid not in wl_fixed and len(mw_hist[sid]) >= 10:
                ...
                NWL = round(mn)
                print(f"[DEBUG WL CHECK] {sid} len={len(mw_hist[sid])} std={sd:.3f}")
                if sd < 0.25 and abs(mn - NWL) < 0.25:
                    print(f"[DEBUG WL PASS] {sid}")
                    wl_fixed[sid] = NWL
                    print(f"[WL] {sid} fixed: N_WL={wl_fixed[sid]}")
                    print(f"  [WL]  {sid} fixed: N_WL={wl_fixed[sid]}  mean={mn:.3f}  std={sd:.3f} cyc")
            if len(mw_hist[sid]) >= 20:
                print(f"[MW] {sid} mean={np.mean(mw_hist[sid]):.2f} std={np.std(mw_hist[sid]):.2f}")
            # ── AMBIGUITY STATE ─────────────────────────────────────────
            if sid not in sidx:
            ...
            if sid in wl_fixed:
                ...
                N_if_float = x[ki] / LAMBDA_IF
                  # smoothing factor
                wl_value = NWL * LAMBDA_WL   # correct WL constraint
                ...
                P[ki, ki] = max(P[ki, ki]*0.3, (0.02)**2)
    ...
    print(f"[DEBUG WL_DICT] size={len(wl_fixed)} keys={list(wl_fixed.keys())[:5]}")
