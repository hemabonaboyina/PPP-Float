"""
ppp_gps_only.py  — GPS-only version derived from ppp_gps.py (v40)
==================================================================
Changes from ppp_gps.py:
  - Only GPS (G) satellites are processed; Galileo (E) is skipped entirely.
  - _proc_gal() and all Galileo-specific constants/helpers are retained but
    never called, so the file stays easy to diff against the original.
  - b_rec is now GPS-only; the 'E' constellation branch in b_rec_frozen is
    never exercised.
  - Output label changed to "GPS-only PPP v40".
  - Epoch print shows G{n_gps} only.
  - Output CSV filename defaults to ppp_results_gps.csv.

Soft ZWD anchor (defaults ZWD_ANCHOR_* below):
  One weak pseudo-observation per epoch on zenith wet delay x[4], variance
  sigma^2, plus a slow EMA so the anchor tracks the filter without going stale.
  Motivation: height and ZWD are coupled in GPS-only PPP; a loose absolute
  constraint limits troposphere drift without dominating real observations.
  Calibrate sigmas / EMA on your data. Background: Kouba & Héroux (2001),
  GPS Solutions 5(2), 12–28 (PPP with ZTD estimation); integer AR e.g. Teunissen
  (LAMBDA) for WL/NL fixing context.
"""
...
# Soft ZWD anchor in _ppp_pass: pseudo-obs z = zwd_anchor - x[4], R = sigma^2
ZWD_ANCHOR_SIGMA_M = 0.12
# Post-filter EMA: zwd_anchor <- RETAIN*zwd_anchor + BLEND*x[4] (sum = 1)
ZWD_ANCHOR_RETAIN = 0.997
ZWD_ANCHOR_BLEND = 0.003

# Galileo constants kept for reference — NOT used in GPS-only mode
FREQ_E1   = FREQ1
FREQ_E5A  = 1176.45e6
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx):
    ...
    L1=so.get('L1C',0.)
    L2_civil = so.get('L2L',0.)
    L2_semi  = so.get('L2W',0.)
...
def _lambda_ils(a_float, Q):
    ...
    try:
        ...
        Z = np.eye(n, dtype=int); a_z = a_float.copy()
        for i in range(1,n):
...
# ==============================================================================
#  PPP Kalman filter pass  — GPS ONLY
# ==============================================================================
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    NL_EXCL_THRESH  = 0.25
    NL_R_TIGHT      = (0.005)**2
    # GPS-only is more fragile than multi-GNSS; use a slightly looser entry gate.
    NL_PHASE_THRESH = 0.012
    _nl_diag_done   = False
    _phase_rms_prev = 999.

    zwd_anchor = float(izwd)
    ...
    _nl_bad_nwl = set()
    _wl_history_ptrace = {}
    _nl_gate_stats = defaultdict(int)
    _nl_gate = _nl_gate_stats
    _wl_persist_stats = defaultdict(int)
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
                        ...
                        if sys_id not in b_rec_frozen:
                            ...
                            if sid in _wl_history:
                                ...
                                if diff == 0:
                                    ...
                                    # Only tiny jumps are considered same-arc jitter.
                                    # Larger jumps are treated as new-arc evidence.
                                    print(f"[WL PERSIST] {sid} using prev NWL={hist_NWL} "
                                          f"(new arc gave NWL={NWL}, diff={diff}<=2->keep)")
                                    NWL_to_use = hist_NWL
                                    _wl_persist_stats['kept_hist'] += 1
                                else:
                                    print(f"[WL NEWARK] {sid} using new NWL={NWL} "
                                          f"(prev={hist_NWL}, diff={diff}>2->new arc)")
                                    _wl_history[sid] = NWL
                                    _wl_history_ptrace[sid] = pt_now
                                    NWL_to_use = NWL
                                    _wl_persist_stats['used_new'] += 1
                            else:
                                _wl_history[sid] = NWL
                                _wl_history_ptrace[sid] = pt_now
                                _wl_persist_stats['first_fix'] += 1
            ...
            if slip:
                ...
                _sat_age[sid]=0
                # FIX 5: no permanent blacklist — reconsidered every epoch

            wu=_wu(m['sat_xyz'],m['rec_apc'],sun,wum.get(sid,0.))
            wum[sid]=wu; LIFc=m['LIF']-wu*LAMBDA_IF; m['LIFc']=LIFc
            ...
            if not phi.get(sid,False):
                if sid in _amb_init:
                    ...
                    _sat_age[sid]=0; pt_now=P[0,0]+P[1,1]+P[2,2]
        ...
        # FIX 6: only cull when N>5 to protect NL candidate pool in GPS-only
        if len(geom)>5:
            pdop=_pdop(geom)
            if pdop>6.0:
        ...
        # FIX 7: add 1 soft ZWD pseudo-observation
        n_soft=1
        H=np.zeros((2*ns+n_wl+n_nl+n_soft, nst))
        z=np.zeros(2*ns+n_wl+n_nl+n_soft)
        Rd=np.zeros(2*ns+n_wl+n_nl+n_soft)
        xs=x.copy()
        ...
        for ri,m in enumerate(geom):
            ...
            age=m.get('age',99); phase_sig=_sig(m['el'],SP)
            if age<=3: phase_sig*=5.
            # Softly downweight low-elevation phase in weak-geometry windows
            if m['el']<math.radians(20.): phase_sig*=1.8
            Rd[rl]=phase_sig**2
        ...
        for ni,m in enumerate(nl_in_geom):
            ...
            z[2*ns+n_wl+ni]=N_IF_fix-xs[m['ki']]; Rd[2*ns+n_wl+ni]=NL_R_TIGHT

        # Soft ZWD anchor — see module ZWD_ANCHOR_* constants
        i_soft=2*ns+n_wl+n_nl
        sig2 = ZWD_ANCHOR_SIGMA_M ** 2
        H[i_soft,4]=1.0; z[i_soft]=zwd_anchor-xs[4]; Rd[i_soft]=sig2
        ...
        # Slowly track anchor toward solution (avoids anchor becoming stale)
        zwd_anchor = (ZWD_ANCHOR_RETAIN * zwd_anchor + ZWD_ANCHOR_BLEND * float(x[4]))

        # ── LAMBDA NL FIXING (GPS only) ──────────────────────────────────────
        # FIX 8: count every gate for diagnostics
        wl_avail=[m for m in geom if m['sid'] in wl_fixed and m['sid'] not in nl_fixed
                  and phi.get(m['sid'],False)]
        _nl_gate['wl_avail']+=len(wl_avail)
        nl_cands=[]
        for m in wl_avail:
            v=P[m['ki'],m['ki']]
            if v>=NL_VAR_THRESH:     _nl_gate['blk_var']+=1; continue
            if phase_rms_now>=NL_PHASE_THRESH: _nl_gate['blk_phase']+=1; continue
            nl_cands.append(m)
        ...
        if len(nl_cands)>=2:
            ...
            N1_floats=np.array([
                _nl_float(x[m['ki']],wl_fixed[m['sid']],
                          ob.get('L1C',ob.get('L1W',0.)),
                          ob.get('L2W',ob.get('L2C',0.)))
            ...
            # FIX 5: per-epoch exclusion only — do NOT permanently blacklist
            ...
            _nl_gate['frac_excl']+=len(excl_sids)
            ...
            if not _nl_diag_done and nproc>100:
                ...
                for i,m in enumerate(nl_cands):
                    ...
                    print(f"    {m['sid']} frac={fracs_corr[i]:+.4f}  "
                          f"NWL={wl_fixed[m['sid']]}  var={P[m['ki'],m['ki']]:.1f}  [{tag}]")
                if excl_sids:
                    print(f"    Excluded (|frac|>{NL_EXCL_THRESH}): {excl_sids}")

            # FIX 3: minimum clean candidates lowered to 2 (was 3)
            if len(clean_idx)>=2:
                _nl_gate['clean_ge2']+=1
                ...
                # Tighter ratio for pairs (n=2) than for triples+ (n>=3)
                ratio_req=4.0 if len(clean_idx)==2 else NL_RATIO_THRESH
                if N1_fixed is not None and ratio>=ratio_req:
                    newly=[]
                    ...
                    if newly:
                        _nl_gate['fix_events']+=1
                        _nl_gate['fixed_sats']+=len(newly)
                        print(f"  [NL FIXED] SOD={sod:.0f} ratio={ratio:.2f} "
                              f"sats={newly} excl={excl_sids}")
                else:
                    _nl_gate['ratio_rej']+=1
        # ─────────────────────────────────────────────────────────────────────
        ...
        results[sod]={'xyz':pos.copy(),'dx':dx.copy(),'p_trace':P[0,0]+P[1,1]+P[2,2],
                      ...
                      'code_rms':code_rms,'phase_rms':phase_rms_now*1e3,
                      'zhd':ZHD,'zwd':ZWD,
        ...
        if nproc<=3 or nproc%240==0:
            print(f"  [{label}] SOD={sod:6.0f}  N={len(geom):2d}(G{len(geom)})  "
                  f"3D={d3:8.1f}mm  WL={len(wl_fixed)}  NL={len(nl_fixed)}"
                  f"  ZHD={ZHD:.3f}m  ZWD={ZWD:.4f}m  ZTD={TROPO:.4f}m"
                  f"  CodeRMS={code_rms:.1f}mm  PhsRMS={phase_rms_now*1e3:.2f}mm")

    # FIX 8: print gate breakdown to diagnose bottleneck
    print(f"[WL_DICT] size={len(wl_fixed)} keys={list(wl_fixed.keys())}")
    print(f"[NL GATE STATS]  wl_avail={_nl_gate['wl_avail']}  "
          f"blk_var={_nl_gate['blk_var']}  blk_phase={_nl_gate['blk_phase']}  "
          f"cand_ge2={_nl_gate['cand_ge2']}  frac_excl={_nl_gate['frac_excl']}  "
          f"clean_ge2={_nl_gate['clean_ge2']}  ratio_rej={_nl_gate['ratio_rej']}  "
          f"fix_events={_nl_gate['fix_events']}  fixed_sats={_nl_gate['fixed_sats']}")
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS-only PPP-AR v40r — GPS-only NL fixes applied (8 improvements)")
    print("="*72)
    ...
    print(f"[NL CFG] PHASE_THRESH={1000*0.015:.0f}mm  VAR_THRESH={20.:.0f}m(sigma)  "
          f"MIN_CLEAN=2  WL_PERSIST_MAX=2cyc  no_permanent_blacklist")
    print()
    ...
    if best60:
        ...
        print(f"  Diff XYZ     = [{(avg_xyz[0]-REF[0])*1e3:+.2f}, "
              f"{(avg_xyz[1]-REF[1])*1e3:+.2f}, {(avg_xyz[2]-REF[2])*1e3:+.2f}] mm")
        print(f"  Rotated ENU  : dE={dE_mm:+.1f} mm  dN={dN_mm:+.1f} mm  dU={dU_mm:+.1f} mm")
        print(f"  3D = {diag3d:.1f} mm")

    print("\n[PASS 2] SKIPPED — FWD-only mode")
    ...
    print("\n"+"="*72)
    print(f"[RESULT] GPS-only FWD  ({len(fwd_list)} epochs total)")
    ...
    for thresh in [50,100,200]:
        ...
        re=math.sqrt(np.mean(enu[:,0]**2)); rn=math.sqrt(np.mean(enu[:,1]**2))
        ru=math.sqrt(np.mean(enu[:,2]**2))
        nl_epochs=sum(1 for _,r in conv if r.get('nl_fixed',0)>0)
        print(f"  Converged <{thresh:3d}mm: {len(conv):4d} epochs  "
              f"RMS E/N/U={re:.1f}/{rn:.1f}/{ru:.1f}mm  3D={rms3d:.1f}mm  "
              f"NL-fixed epochs={nl_epochs}")

    bs,br=min(fwd_list,key=lambda x:np.linalg.norm(x[1]['dx']))
    b3=np.linalg.norm(br['dx'])*1e3
    print(f"  Best epoch: SOD={bs:.0f}  3D={b3:.1f}mm  WL={br.get('wl_fixed',0)}  "
          f"NL={br.get('nl_fixed',0)}")
    ...
    if len(fwd_list)>120:
        ...
        print(f"  Last 120:  RMS E/N/U="
              f"{math.sqrt(np.mean(enu_t[:,0]**2)):.1f}/"
              f"{math.sqrt(np.mean(enu_t[:,1]**2)):.1f}/"
              f"{math.sqrt(np.mean(enu_t[:,2]**2)):.1f}mm  3D={r3t:.1f}mm  "
              f"Bias E={bias_enu[0]:+.1f} N={bias_enu[1]:+.1f} U={bias_enu[2]:+.1f}mm")

    if best60:
        nl_fix_total=sum(1 for _,r in fwd_list if r.get('nl_fixed',0)>0)
        print(f"")
        print(f"  [ACCURACY SUMMARY]")
        print(f"  DIAG 3D  = sqrt({dE_mm:.1f}² + {dN_mm:.1f}² + {dU_mm:.1f}²) = {diag3d:.1f} mm")
        print(f"  Dominant : East {dE_mm:.1f} mm  N={dN_mm:.1f} mm  U={dU_mm:.1f} mm")
        if len(fwd_list)>120: print(f"  Last-120 : RMS {r3t:.1f} mm")
        print(f"  NL-fixed : {nl_fix_total}/{len(fwd_list)} epochs "
              f"({100*nl_fix_total/len(fwd_list):.1f}%)")
    ...
    if fwd_list:
        hourly=[]
        for h in range(24):
            chunk=[(s,r) for s,r in fwd_list if h*3600<=s<(h+1)*3600]
            if not chunk: continue
            dx_h=np.array([r['dx'] for _,r in chunk])
            enu_h=(Re@dx_h.T).T*1e3
            zwd_h=np.array([r.get('zwd',0.) for _,r in chunk])
            n_h=np.array([r.get('n',0) for _,r in chunk],dtype=float)
            nl_h=np.array([r.get('nl_fixed',0) for _,r in chunk],dtype=float)
            hourly.append((h,len(chunk),
                           float(np.mean(enu_h[:,0])),float(np.mean(enu_h[:,1])),
                           float(np.mean(enu_h[:,2])),
                           float(math.sqrt(np.mean(enu_h[:,2]**2))),
                           float(np.mean(zwd_h)),float(np.mean(n_h)),
                           float(np.mean(nl_h>0)*100)))
        print("")
        print("  [HOURLY DIAG] hour  nEp   meanE  meanN  meanU  rmsU  meanZWD  meanNsat  NL%")
        for row in hourly:
            h,nep,e_m,n_m,u_m,u_r,z_m,ns_m,nl_p=row
            print(f"    {h:02d}   {nep:4d}  {e_m:+6.1f} {n_m:+6.1f} {u_m:+7.1f} "
                  f"{u_r:6.1f}  {z_m:8.4f}   {ns_m:6.2f}   {nl_p:5.1f}")
        if outfile:
            diag_csv=(outfile[:-4] if outfile.lower().endswith('.csv') else outfile)+"_hourly_diag.csv"
            with open(diag_csv,'w') as fh:
                fh.write("hour,epochs,meanE_mm,meanN_mm,meanU_mm,rmsU_mm,"
                         "meanZWD_m,meanNsat,NL_frac_percent\n")
                for h,nep,e_m,n_m,u_m,u_r,z_m,ns_m,nl_p in hourly:
                    fh.write(f"{h},{nep},{e_m:.3f},{n_m:.3f},{u_m:.3f},{u_r:.3f},"
                             f"{z_m:.6f},{ns_m:.3f},{nl_p:.3f}\n")
            print(f"  [CSV]  Hourly diag: {diag_csv}")
...
if __name__=='__main__':
    ...
    postpos(None,None,0.,0.,PrcOpt(),SolOpt(),FilOpt(),
            INFILES,os.path.join(DATA,'ppp_results_gps.csv'))
