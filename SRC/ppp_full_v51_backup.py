"""
ppp_ar.py  v56 — NL selection / WL gate / per-const b_rec / threshold fixes
=============================================================================

PATCHES IN v56 (builds on v55 windwup-lambda fix)
===================================================

PATCH-A  Windwup lambda: GPS _lam_if=_DENOM_G (0.107m), Gal _lam_if=_DENOM_E (0.109m).
         Previously LAMBDA_IF (0.142m) caused 0.33 NL-cycle bias per revolution.

PATCH-B  WL b_rec freeze gate removed.
         "if sys_id not in b_rec_frozen: pass" blocked ALL GPS WL fixing until
         ≥5 sats agreed (never happened for GPS with noisy C1W/C2W).
         Fix: allow WL fixing with soft b_rec from ≥2 sats before freeze.
         Freeze threshold lowered 5→3 sats, 60%→50% agreement.

PATCH-C  b_rec estimation uses np.median not np.mean throughout WL block.

PATCH-D  _nl_bad_nwl permanent blacklist removed from NL candidate filter.
         Exclusion is now epoch-local only; satellites can retry next epoch.

PATCH-E  Per-constellation b_rec_nl (GPS and Galileo separate medians).
         Mixed median corrupted integer recovery for both systems.
         Guard: if |b_nl_sys| > 0.40 cyc → set to 0.0 (filter not converged).

PATCH-F  NL threshold relaxation:
           NL_RES_THRESH    0.10 → 0.15 cyc
           NL_EXCL_THRESH   0.25 → 0.30 cyc
           NL_INNOV_GATE    80mm → 115mm  (> 1 GPS NL cycle = 107mm)
           NL_RELEASE_THRESH 60mm → 115mm (was 0.56 cycles — released correct fixes)
           NL_PHASE_THRESH  15mm → 20mm   (tropical PhsRMS can be 8–15mm)
           NL_MIN_OBS  GE:8→6, single:5→4

PATCH-G  PDOP guard 3.0 → 5.0; duplicate phase_rms_now check removed from
         _skip_nl (already enforced in nl_cands filter).

PATCH-H  Q_nl symmetrized and regularized (+1e-10·I) before LAMBDA call.

PATCH-I  _nl_diag_done resets every 360 epochs for visibility of later state.

CUMULATIVE EARLIER FIXES (v50–v54, all retained):
  v50: WL persist bug, proper ILS LAMBDA, Galileo Q_nl denom, ocean tide loading
  v51: ratio gate 4.5, NL innovation gate 80mm, post-update release 60mm
  v52: ZWD watchdog, ZWD soft prior, NL_PHASE_THRESH 15mm
"""
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    # Phase OSBs used only for MW satellite-bias removal.
    # The IF combination (LIF) intentionally does NOT have phase OSBs subtracted
    # here; the satellite IF phase bias is absorbed into x[ki] and explicitly
    # removed inside _nl_float() when computing the NL float ambiguity. ✓
    ...
    return dict(sid=sid,unit=unit,mh=mh,mw=mw,el=el,
                ...
                _lam_if=_DENOM_G)   # PATCH-A: wu*(α·λ1−β·λ2)=_DENOM_G, NOT LAMBDA_IF
...
def _proc_gal(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    return dict(sid=sid,unit=unit,mh=mh,mw=mw,el=el,
                ...
                sat_xyz=sva,rec_apc=ra,
                _lam_if=_DENOM_E,_alfa=ALFA_E,_beta=BETA_E,   # PATCH-A Gal
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    # ── v53 parameters ────────────────────────────────────────────────────────
    NL_RATIO_THRESH   = 3.0           # balanced LAMBDA ratio (was 4.5)
    NL_VAR_THRESH     = (10.0)**2
    NL_RES_THRESH     = 0.15          # PATCH-F: 0.10→0.15 cyc (avoids spurious EXCL)
    NL_EXCL_THRESH    = 0.30          # PATCH-F: 0.25→0.30 cyc (epoch-local exclusion)
    NL_R_TIGHT        = (0.005)**2
    NL_INNOV_GATE     = 0.115         # PATCH-F: 80mm→115mm (>1 GPS NL cycle=107mm)
    NL_RELEASE_THRESH = 0.115         # PATCH-F: 60mm→115mm (was 0.56 cyc — too aggressive)
    NL_PHASE_THRESH   = 0.020         # PATCH-F: 15mm→20mm (tropical station PhsRMS can be 8-15mm)
    NL_MIN_SATS       = 3             # min clean candidates for LAMBDA
    NL_MIN_OBS        = 6 if constellation == 'GE' else 4  # PATCH-F: GE:8→6, E:5→4
    ...
    for epoch in eplist:
        ...
        for k in range(namb):
            ...
            if sid_k and sid_k in nl_fixed:
                Q[5+k,5+k]=0.0   # strict hold — integer constraint is active
            else:
                Q[5+k,5+k]=q_amb
        P+=Q
        ...
        for sid,so in sorted(sobs.items()):
                    ...
                    if n_hist>=min_n:
                        if sys_id not in b_rec_frozen:
                            ...
                            for s2,h2 in mw_hist.items():
                                ...
                                all_fracs.append(np.median(h2)-round(np.median(h2)))  # PATCH-C: median
                            # PATCH-B: lower freeze from 5 sats/60% → 3 sats/50%
                            if len(all_fracs)>=3:
                                bc=float(np.median(all_fracs))
                                agr=sum(1 for f in all_fracs if abs(f-bc)<0.25)
                                if agr>=max(2,0.5*len(all_fracs)):
                        ...
                        if sys_id in b_rec_frozen:
                            ...
                            for s2,h2 in mw_hist.items():
                                if s2[0]!=sys_id or len(h2)<min_n: continue
                                if (np.std(h2) if len(h2)>1 else 999.)>0.45: continue
                                fracs.append(np.median(h2)-round(np.median(h2)))  # PATCH-C: median
                            # PATCH-B: allow soft b_rec with >=2 sats (don't block on freeze)
                            if len(fracs)<2: continue  # truly not enough data yet
                            b_rec=float(np.median(fracs)); tag=sys_id+'E'
                        ...
                        # PATCH-B: removed "if sys_id not in b_rec_frozen: pass"
                        # Soft b_rec (tag='E') is now allowed above if >=2 sats present
                        if sd<=0.25 and residual<0.25:   # PATCH-B: residual 0.20→0.25 (slightly looser)
                            NWL_to_use=NWL
            ...
            lam_if=m.get('_lam_if',_DENOM_G)   # PATCH-A: _DENOM_* not LAMBDA_IF
            ...
            if not phi.get(sid,False):
                if sid in _amb_init:
                    ...
                    rp0=_rp(m,x[3],x[4]); x[ki]=LIFc-rp0; P[ki,ki]=300.**2
        ...
        for ri,m in enumerate(geom):
            ...
            Rd[rl]=phase_sig**2
            # residual gate: pre-update phase residual > 30mm → downweight heavily
            if abs(z[rl]) > PHASE_RES_GATE:
        ...
        zwd_before=x[4]
        if filter_standard(x,P,H_p.T,z_p,np.diag(Rd_p))!=0: continue
        ...
        for sid_chk in list(nl_fixed.keys()):
            ...
            if post_innov>NL_RELEASE_THRESH:
                # Hold ambiguity: downweight measurements, release only on clear drift
                # (do NOT release immediately — absorb via measurement downweighting)
                nl_fixed.pop(sid_chk,None)
                print(f"  [NL RELEASE] {sid_chk} SOD={sod:.0f} "
                      f"post-update drift={post_innov*1e3:.1f}mm>{NL_RELEASE_THRESH*1e3:.0f}mm — released")
            elif post_innov > 0.030:
                # instability but below release threshold — hold ambiguity,
                # inflate its variance slightly to allow gentle re-convergence
                P[ki_chk,ki_chk]=max(P[ki_chk,ki_chk],(0.010)**2)
        ...
        nl_cands=[m for m in geom
                  ...
                  # PATCH-D: removed "m['sid'] not in _nl_bad_nwl" — no permanent blacklist
        ...
        if len(nl_cands)>=NL_MIN_SATS and len(geom)>=NL_MIN_OBS:
            # PATCH-G: PDOP guard 3.0→5.0; removed duplicate phase_rms_now>0.015
            # (phase_rms_now already enforced in nl_cands filter via NL_PHASE_THRESH)
            pdop_now = _pdop(geom)
            _skip_nl = (pdop_now > 5.0)
            # PATCH-I: re-arm diag every 360 epochs (~3 h) so we see later convergence state
            if nproc % 360 == 0: _nl_diag_done = False
            if not _skip_nl:
                ...
                # PATCH-E: per-constellation receiver NL bias.
                # GPS and Galileo have independent NL hardware delays.
                # A single median over mixed fracs corrupts both.
                N1_corr=N1_floats.copy()
                for sys_c in ('G','E'):
                    idx_sys=[i for i,m in enumerate(nl_cands) if m['sid'][0]==sys_c]
                    if len(idx_sys)<2: continue   # need ≥2 sats to estimate bias
                    fracs_sys=np.array([N1_floats[i]-round(N1_floats[i]) for i in idx_sys])
                    b_nl_sys=float(np.median(fracs_sys))
                    if abs(b_nl_sys)>0.40:
                        # bias > 0.40 cyc: filter not converged for this constellation
                        # zero out correction to avoid flipping integer rounding
                        b_nl_sys=0.0
                    for i in idx_sys:
                        N1_corr[i]=N1_floats[i]-b_nl_sys
                ...
                # PATCH-D: no _nl_bad_nwl.add() — exclusion is epoch-local only

                if not _nl_diag_done and nproc>100:
                    _nl_diag_done=True
                    fracs_diag=np.array([N1_corr[i]-round(N1_corr[i]) for i in range(len(nl_cands))])
                    print(f"  [NL DIAG] SOD={sod:.0f} n_all={len(nl_cands)} "
                          f"n_clean={len(clean_idx)} "
                          f"PhsRMS={phase_rms_now*1e3:.2f}mm (per-const b_rec applied)")
                    for i,m in enumerate(nl_cands):
                        tag='OK  ' if i in clean_idx else 'EXCL'
                        print(f"    {m['sid']} frac={fracs_diag[i]:+.4f} "
                ...
                if len(clean_idx)>=NL_MIN_SATS:
                    ...
                    # PATCH-H: ensure Q_nl is symmetric and positive-definite.
                    # Ambiguity states share pos/clock/ZWD correlations; floating-point
                    # asymmetry + near-rank-deficiency can cause LAMBDA to fail silently.
                    Q_nl=0.5*(Q_nl+Q_nl.T)
                    Q_nl+=np.eye(len(idxs_c))*1e-10
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS+Galileo PPP v56 — windwup-λ | WL-gate-off | per-const-b_rec | "
          "NL-thresh-relax | no-blacklist | Q_nl-reg")
    print("="*72)
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation Comparison (v54-NL-enabled)',
