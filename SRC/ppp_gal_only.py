"""
ppp_gal_only.py  — Galileo-only version derived from ppp_gps.py (v40)
=====================================================================
Changes from ppp_gps.py:
  - Only Galileo (E) satellites are processed; GPS (G) is skipped entirely.
  - _proc() (GPS) is retained but never called.
  - All Galileo signal processing uses E1 (C1C/L1C) + E5a (C5Q/L5Q) via _proc_gal().
  - b_rec is Galileo-only ('E' key in b_rec_frozen).
  - NL fixing uses Galileo IF/WL wavelengths (_nl_float_gal, _nl_if_value_gal).
  - Output label changed to "Galileo-only PPP v40".
  - Epoch print shows E{n_gal} only.
  - Output CSV filename defaults to ppp_results_gal.csv.
"""
...
# Galileo E1/E5a constants
FREQ_E1   = FREQ1          # 1575.42 MHz  (same as GPS L1)
...
def _ifc_e(a, b): return ALFA_E*a - BETA_E*b
...
def parse_bia(fp):
        ...
        for ln in fh:
            ...
            if unit=='ns':
                ...
                if obs in ('L1C','L1W','L1P'):
                    B[prn][obs]=val*LAMBDA_E1
                elif obs in ('L5Q','L5I','L5X'):
    ...
    if tot>0:
        e=B.get('E01',{})
        print(f"       E01 C1C={e.get('C1C',float('nan')):+.4f}m  "
              f"C5Q={e.get('C5Q',float('nan')):+.4f}m  "
              f"L1C={e.get('L1C',float('nan')):+.6f}m  "
              f"L5Q={e.get('L5Q',float('nan')):+.6f}m")
    return dict(B)
...
# ==============================================================================
#  Melbourne-Wubbena (Galileo E1/E5a) and geometry-free
# ==============================================================================
def _mw_cyc_gal(P1, P5, L1_cyc, L5_cyc):
    """Galileo MW in WL cycles using E1 + E5a."""
    L1_m = L1_cyc * LAMBDA_E1;  L5_m = L5_cyc * LAMBDA_E5A
    phi_WL_m = (FREQ_E1*L1_m  - FREQ_E5A*L5_m) / (FREQ_E1 - FREQ_E5A)
    P_NL_m   = (FREQ_E1*P1    + FREQ_E5A*P5)   / (FREQ_E1 + FREQ_E5A)
    return (phi_WL_m - P_NL_m) / LAMBDA_WL_E

def _gf_m_gal(L1_cyc, L5_cyc):
    return L1_cyc*LAMBDA_E1 - L5_cyc*LAMBDA_E5A
...
# ==============================================================================
#  Galileo satellite geometry  (unchanged from ppp_gps.py _proc_gal)
# ==============================================================================
def _proc_gal(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx):
    ...
    MW_cyc = _mw_cyc_gal(P1-b1, P5-b5, L1, L5) - b_wl_sat_cyc
    GF_m   = _gf_m_gal(L1, L5)
...
# ==============================================================================
#  PPP Kalman filter pass  — GALILEO ONLY
# ==============================================================================
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            ...
            wum[sid]=wu
            lam_if = LAMBDA_IF_E   # Galileo IF wavelength
        ...
        for ri,m in enumerate(geom):
            ...
            if age<=3: phase_sig=phase_sig*5.
            Rd[rl]=phase_sig**2

        for wi,m in enumerate(wl_in_geom):
            H[2*ns+wi,m['ki']]=0.; z[2*ns+wi]=0.; Rd[2*ns+wi]=1e10

        # NL pseudo-obs — Galileo wavelengths
        for ni,m in enumerate(nl_in_geom):
            ob=osb.get(m['sid'],{})
            bl1=ob.get('L1C',0.); bl5=ob.get('L5Q',0.)
            N_IF_fix=_nl_if_value_gal(nl_fixed[m['sid']], wl_fixed[m['sid']], bl1, bl5)
        ...
        if nproc<=3 or nproc%240==0:
            n_gal=len(geom)   # all sats are Galileo
            print(f"  [{label}] SOD={sod:6.0f}  N={len(geom):2d}(E{n_gal})  3D={d3:8.1f}mm"
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("Galileo-only PPP v40 — correct NL cascade, no WL validation (Geng 2019)")
    print("="*72)
    ...
    if best60:
        ...
        print(f"  Diff XYZ     = [{(avg_xyz[0]-REF[0])*1e3:+.2f}, {(avg_xyz[1]-REF[1])*1e3:+.2f}, {(avg_xyz[2]-REF[2])*1e3:+.2f}] mm")
    ...
    print("\n"+"="*72)
    print(f"[RESULT] Galileo-only FWD  ({len(fwd_list)} epochs total)")
    ...
    if best60:
        diag_3d = diag3d
        dE_diag = dE_mm; dN_diag = dN_mm; dU_diag = dU_mm
        ...
        if len(fwd_list) > 120:
            print(f"  Last-120 : RMS {r3t:.1f} mm")
    ...
    print(f"  3D error  : mean={np.mean(all_3d):.1f} mm  median={np.median(all_3d):.1f} mm  max={np.max(all_3d):.1f} mm  min={np.min(all_3d):.1f} mm")
    print(f"  Code  RMS : {math.sqrt(np.mean(all_code**2)):.1f} mm  (post-conv={math.sqrt(np.mean(code_post**2)):.1f} mm)")
    print(f"  Phase RMS : {math.sqrt(np.mean(all_phase**2)):.2f} mm  (post-conv={math.sqrt(np.mean(phase_post**2)):.2f} mm)")
...
if __name__=='__main__':
    ...
    postpos(None,None,0.,0.,PrcOpt(),SolOpt(),FilOpt(),
            INFILES,os.path.join(DATA,'ppp_results_gal.csv'))
