"""
ppp.py  v89 — Fix all 8 convergence blockers: iono caps re-enabled, sigma gates tightened, integer-flip prevention
===========================================================================
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    # v84 PART 1: only log when AR eligibility state CHANGES (suppresses per-epoch spam)
    _sig_pair_str = f"{code1_type}/{phase1_type}+{code2_type}/{phase2_type}"
    _has_valid_osb = (ar_skip_reason is None) and bool(ob)
    if _prev_gps_ar_state.get(sid) != no_AR:
        print(f"[GPS_AR_CHANGE] {sid} signals={_sig_pair_str} "
              f"OSB_ok={_has_valid_osb} -> AR={'ON' if not no_AR else 'OFF'}"
              + (f" reason={ar_skip_reason}" if ar_skip_reason else ""))
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    for epoch in eplist:
            ...
            for _obs_sid in sorted(sobs.keys()):
                ...
                if _obs_sid[0] == 'G':
                    ...
                    if _c1w != 0. and _l1w != 0. and _c2w != 0. and _l2w != 0.:
                        ...
                        # v83: AR status depends on OSB, not signal label
                        _geom_entry = next((m for m in geom if m['sid'] == _obs_sid), None)
                        _ar_status = 'AR=ON' if (_geom_entry and _geom_entry.get('use_for_ar', False)) else 'AR=off(no OSB)'
                        print(f"  {_obs_sid} → C1C/L1C + C2W/L2W ✔ [PPP+{_ar_status}]")
                    else:
        ...
        for m in geom:
            ...
            # PART 2 — SINGLE RELAXED SIGMA GATE (0.40 m — only filter allowed)
            _sigma_gate = 0.40
            if sigma_N1_m > _sigma_gate:
                _ep_skip_sigma += 1
                reject_due_to_sigma += 1
                print(f"[NL_REJECT] sat={sid_m}  sigma={sigma_N1_m:.4f}  reason=sigma_high")
                print(f"[SIGMA_CHECK] sid={sid_m} sigma={sigma_N1_m:.3f}")
                continue
            ...
            # FIX 2 — REQUIRE FRACTIONAL STABILITY: frac_std must be < 0.02 cyc.
            # v86 PART 2: hard 0.02 for both constellations (was 0.04/0.05).
            # Require strict stability before attempting any fix.
            if frac_std >= 0.02:
        ...
        # v88 PART 1 — NL_FLOW debug throttled: print every 100 epochs only
        _after_sigma = _nl_epoch_candidates   # PART 3: no second filtering
        _nl_fixed_this_ep = []
        if nproc % 100 == 0:
            print(f"[NL_FLOW] candidates={len(_nl_preselect)}"
                  f"  selected={len(_nl_eligible_sids)}"
                  f"  after_sigma={len(_after_sigma)}"
                  f"  fixed={len(nl_fixed)}")
            print(f"[NL_FLOW_CHECK] "
                  f"cand={len(_nl_preselect)} "
                  f"sel={len(_nl_eligible_sids)} "
                  f"after_sigma={len(_after_sigma)} "
                  f"fixed={len(nl_fixed)}")
        # PART 5 — HARD DEBUG CHECK every 50 epochs
        if nproc % 50 == 0:
            print(f"[CHECK] candidates={len(_nl_preselect)} "
                  f"selected={len(_nl_eligible_sids)} "
                  f"after_sigma={len(_after_sigma)}")
        ...
        if nl_fixed and len(geom) >= 4:
            ...
            for sid_nl,(N1_i,N2_i) in list(nl_fixed.items()):
                ...
                nl_pairs.append((h1, float(N1_i), NL_R_eff))
                # N2 pseudo-obs
                h2=np.zeros(len(x)); h2[ki_nl+2]=1.
                nl_pairs.append((h2, float(N2_i), NL_R_eff))
            if nl_pairs:
        ...
        if nproc % 300 == 0 and sidx:
            ...
            # v85 PART 4 — NL quality summary (mandatory debug)
            print(f"[NL_QUALITY] mean_sigma={_mean_sig_dbg:.4f} max_sigma={_max_sig_dbg:.4f}")
            # v84 PART 1: periodic GPS AR summary (replaces per-epoch per-sat spam)
            nGPS_AR_capable = sum(1 for m in geom if m['sid'][0]=='G' and not m.get('no_AR', False))
            print(f"[GPS_SUMMARY] SOD={sod:.0f}  AR_capable={nGPS_AR_capable}"
                  f"  NL_total={_nl_count_dbg}")
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation (v89) — iono caps + sigma gates + integer-flip fix',
