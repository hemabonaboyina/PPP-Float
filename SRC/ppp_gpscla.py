...
v2 fixes: Joseph form KF, no WL pseudo-obs, PDOP-aware sat selection,
          post-fit outlier rejection, age gate, noisy sat exclusion,
          PERSIST threshold=5, WL residual threshold=0.25.
v3 fixes: NL probation system (wrong integers auto-released after 20 epochs),
          minimum 3 clean sats required for any NL fix,
          adaptive ratio threshold (6.0 for 3-sat fix, 4.0 for 4-sat, 3.0 for 5+),
          ZWD clamp at [-0.05, 0.40]m, NL_PHASE_THRESH=0.020m,
          removed permanent _nl_bad_nwl blacklist.
v4-v5 fixes: ZWD init 0.35, prior P=0.10^2, process noise 5e-7,
             probation threshold scales with convergence,
             early NL ratio thresh 8.0 (first 30 min).
v6 fixes (4 targeted corrections):
  Fix 1: Two-stage position process noise (1e-8 pre-conv → 1e-9 post-conv)
          Reduces ZWD/East coupling that was causing ~20mm East bias.
  Fix 2: Two-stage ZWD process noise (5e-7 → 2e-7 after nproc>200)
          Prevents late-day ZWD drift from corrupting East at day's end.
  Fix 3: NL_PHASE_THRESH raised 20mm → 25mm to allow 4-5 sats into NL pool.
  Fix 4: Backward smoother pass added; FWD+BWD merged by p_trace per epoch.
          Expected to recover H00-H08 dead zone and reduce early-session bias.
"""
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              satx,att,recx,noisy_sats=None,elm=math.radians(10.),SC=1.50,SP=0.003,
    ...
    NL_RES_THRESH=0.15
    NL_EXCL_THRESH=0.35   # FIX: was 0.25, pre-screening was too aggressive
    NL_R_TIGHT=(0.005)**2
    # Fix 3: Raise NL_PHASE_THRESH from 20mm to 25mm.
    # Post-convergence PhsRMS sits at 10-20mm — 20mm was a borderline gate
    # restricting NL pool to 3 sats. 25mm allows 4-5 sats in, improving
    # azimuthal spread and LAMBDA redundancy. Ratio thresholds (8.0/6.0/4.0)
    # still guard against bad fixes, so loosening the PhsRMS pre-screen is safe.
    NL_PHASE_THRESH=0.025   # 25mm — allows 4-5 sats into NL pool
    NL_PROBATION_EPOCHS=20  # epochs a new fix must survive before being trusted
    _nl_diag_done=False

    x=np.zeros(5); x[3]=iclk; x[4]=izwd
    P=np.zeros((5,5))
    P[0,0]=P[1,1]=P[2,2]=100.**2; P[3,3]=3000.**2
    P[4,4]=0.10**2   # ZWD prior sigma 0.10m — tighter around good GPT3w a priori
    ...
    for epoch in eplist:
        ...
        # Fix 1: Two-stage position process noise.
        # Pre-convergence (nproc<120, ~1hr): 1e-8*dt allows filter to move freely.
        # Post-convergence: tighten to 1e-9*dt — receiver is static, ZWD changes
        # must NOT bleed into East/North via the troposphere-position coupling.
        # This directly attacks the ~20mm persistent East bias seen in the data.
        q_pos = 1e-8*dt if nproc < 120 else 1e-9*dt
        Q[0,0]=Q[1,1]=Q[2,2]=q_pos
        Q[3,3]=1e4*dt
        # Fix 2: Two-stage ZWD process noise.
        # Tropical IISC (13°N, 844m) has wet delay variability ~30-50mm/hr early-session,
        # but stabilises in the second half of the day.
        # 5e-7*dt pre-convergence tracks rapid early ZWD changes.
        # 2e-7*dt post-convergence (nproc>200, ~1.7hr) prevents late-day ZWD drift
        # from corrupting the East component as satellite geometry rotates.
        Q[4,4] = 5e-7*dt if nproc < 200 else 2e-7*dt
        ...
        # Threshold scales with convergence: loose early (filter still noisy),
        # tight after 120 epochs (converged). Prevents revoking correct fixes
        # due to unconverged position/ZWD inflating phase RMS.
        prob_thresh = (NL_PHASE_THRESH*3.0 if nproc<120 else NL_PHASE_THRESH*1.5)
        for sid in list(_nl_probation.keys()):
            _nl_probation[sid]-=1
            if _nl_probation[sid]<=0:
                del _nl_probation[sid]  # probation passed — fix trusted
            elif phase_rms_now>prob_thresh:
        ...
        if len(nl_cands)>=2:
            ...
            # Require >= 3 clean sats — 2-sat fixes have no geometric redundancy
            if len(clean_idx)>=3:
                ...
                # Adaptive ratio: stricter when fewer sats (less redundancy)
                # Also stricter in first 60 epochs (nproc<60, i.e. first 30 mins):
                # position error is still >1000mm, a marginal fix destabilises ZWD.
                # After convergence, relax back to normal adaptive thresholds.
                n_clean=len(clean_idx)
                if nproc<60:
                    ratio_thresh=8.0   # very strict until filter is stable
                elif n_clean<=3: ratio_thresh=6.0
                elif n_clean==4: ratio_thresh=4.0
        ...
        nproc+=1
        # ZWD clamp: floor at -0.05m for filter recovery headroom.
        # Ceiling 0.50m: IISC is tropical (13N, 844m), ZWD can reach 0.40m+.
        # Prior ceiling of 0.22m was suppressing 100-200mm of real wet delay.
        # Ref: Kouba & Heroux (2001) GPS Solutions 5:12-28.
        x[4]=max(x[4], -0.05)
        x[4]=min(x[4], 0.50)
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS-only PPP-AR v6 — Fix1:pos-noise Fix2:ZWD-noise Fix3:NL-thresh Fix4:BWD-smoother")
    print("="*72)

    _,epochs,ah,ak=parse_obs(obs_f)
    sp3t,sp,sc=parse_sp3(sp3_f)
    clkd=parse_clk(clk_f) if clk_f else {}
    noisy_sats=_noisy_sats(clkd)
    ...
    fwd,ex,ec,ez,wl_fwd,fwd_amb,fwd_snapshots=_ppp_pass(
        epochs,sp3t,sp,sc,clkd,osb,ah,APX.copy(),0.,0.35,
    ...
    # Fix 4: Backward smoother pass.
    # The forward-only filter accumulates systematic errors (ZWD/East coupling,
    # PCO residuals) that grow forward in time. A backward pass starting from the
    # end-of-day converged state propagates corrections back through early epochs
    # where the forward filter was unconverged (H00-H08 dead zone).
    # Merge strategy: at each SOD, take whichever pass has lower p_trace
    # (smaller position uncertainty), which automatically selects the converged
    # solution. FWD wins in the second half; BWD wins in the first half.
    print("\n[PASS 2] Backward (smoother) ...")
    # Use FWD end-state as BWD start: converged position ex, clock ec, ZWD ez.
    # Pass FWD WL fixes and converged ambiguities so BWD doesn't re-converge from scratch.
    bwd,_bx,_bc,_bz,_bwl,_,_=_ppp_pass(
        epochs,sp3t,sp,sc,clkd,osb,ah,ex.copy(),ec,ez,
        lat0,DOY,zhd,tref,satx,att,recx,
        noisy_sats=noisy_sats,direction=-1,label="BWD",
        wl_init=wl_fwd,amb_init=fwd_amb)
    print(f"  {len(bwd)} epochs  end_3D={np.linalg.norm(_bx-REF)*1e3:.1f}mm  ZWD={_bz:.3f}m")

    # Merge FWD and BWD: pick best p_trace epoch-by-epoch
    merged={}
    all_sods=set(fwd.keys()) | set(bwd.keys())
    fwd_wins=0; bwd_wins=0
    for sod in all_sods:
        if sod in fwd and sod in bwd:
            if fwd[sod]['p_trace'] <= bwd[sod]['p_trace']:
                merged[sod]={**fwd[sod],'pass':'FWD'}; fwd_wins+=1
            else:
                merged[sod]={**bwd[sod],'pass':'BWD'}; bwd_wins+=1
        elif sod in fwd:
            merged[sod]={**fwd[sod],'pass':'FWD'}; fwd_wins+=1
        else:
            merged[sod]={**bwd[sod],'pass':'BWD'}; bwd_wins+=1
    print(f"  [MERGE] FWD won {fwd_wins} epochs, BWD won {bwd_wins} epochs")
    # Use merged for all downstream reporting and CSV output
    fwd=merged
    ...
    # Build result list from merged FWD+BWD (fwd is already merged at this point)
    rl=[(s,r) for s,r in sorted(fwd.items())]
