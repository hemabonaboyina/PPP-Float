...
KEY FIXES vs v40:
  ...
  1. GALILEO ISB STATE ADDED — a 6th Kalman state estimates the Galileo
     inter-system clock bias (ISB) relative to GPS time. CODE precise clock
     products reference GPS system time; Galileo satellite clocks include a
     residual system-time offset that must be estimated per-session (typically
     1–15 mm in phase). Without this state, Galileo pseudoranges/phases are
     systematically miscorrected, pushing Galileo ambiguities away from integer
     values and making GPS+Galileo combined mode perform no better than GPS-only.

  2. NL_VAR_THRESH RAISED — from (10m)² to (20m)². Ambiguity states initialise
     at 300² = 90000 m² and take hours to converge. The old 100 m² threshold
     excluded most sats from LAMBDA, keeping NL count frozen at 4 for the
     entire session after the initial fix.

  3. b_rec FREEZE THRESHOLD LOWERED FOR GALILEO — from ≥5 to ≥3 sats.
     Galileo typically has only 4–6 satellites above 10° elevation. With the
     old threshold, Galileo b_rec never froze, blocking all Galileo WL fixing.

  4. _amb_conv_sods THRESHOLD FIXED — from p_trace < 0.30 m² (never satisfied)
     to p_trace < 9.0 m² (satisfied within ~30 min). This gate controls BWD
     ambiguity inheritance. With the old value the BWD pass always inherited
     zero ambiguities and reconverged from scratch, wasting the backward pass.

  5. ZWD PROCESS NOISE RAISED — from 1e-8 to 1e-7 m²/s. For a 24-hour
     session, ZWD varies ~15–25 mm due to diurnal troposphere change. The old
     value was too tight and caused the growing Up error seen late in the day.
     RTS smoother updated to match.

  6. NL_EXCL_THRESH RAISED — from 0.25 to 0.30 cycles. Galileo NL fracs
     computed with the per-constellation b_rec_gal land in 0.26–0.29 range
     due to IFB residuals. The old threshold excluded valid Galileo sats.

  7. _nl_bad_nwl AGE GUARD ADDED — sats must have age > 30 epochs before they
     can be permanently blacklisted. Prevents freshly-initialised sats from
     being blacklisted before their ambiguities converge.

  8. NL_R_TIGHT TIGHTENED — from (5mm)² to (3mm)². Tighter pseudo-obs noise
     keeps the fixed ambiguity closer to the integer constraint.

  9. ISB INCLUDED IN PHASE/CODE RESIDUALS — phase RMS gate, code RMS log,
     and ambiguity initialisation all correctly subtract the ISB for Galileo
     satellites so the residuals reflect true measurement noise only.
"""
...
# ==============================================================================
class _rts_store:
    """Simple namespace to accumulate (sod, x, P) tuples during the FWD pass.
    NOTE: must be reset explicitly before each pass via _rts_store._data = []
    The class-level list is intentionally mutable so all code refers to the
    same object; the reset in postpos() before each _ppp_pass call is the
    correct pattern.
    """
    _data = []  # reset to [] before every _ppp_pass call in postpos()

def _rts_smooth(fwd_results, nom, REF):
    """
    Rauch–Tung–Striebel fixed-interval smoother applied to the position /
    clock / ZWD sub-states stored during the forward Kalman pass.

    The ambiguity states are *not* smoothed — they are treated as pseudo-
    constants that were already fixed by the NL step.  Only states 0-4
    (dX, dY, dZ, dT, ZWD) are smoothed.

    Parameters
    ----------
    fwd_results : dict  {sod: result_dict} from _ppp_pass
    nom         : np.ndarray  nominal position used in _ppp_pass

    Returns
    -------
    smoothed : dict  {sod: result_dict}  same keys/format, positions updated
    """
    data = _rts_store._data          # list of (sod, x, P) in time order
    if len(data) < 3:
        return fwd_results           # too few epochs — nothing to smooth

    N   = len(data)
    # Determine state dimension from stored data: 5 for single-constellation, 6 for GE.
    # The smoother only updates the base states (pos + clock + ZWD [+ ISB]).
    # Ambiguity states (indices >= dim) are not smoothed.
    dim = min(6, data[0][1].shape[0])  # at most 6, but respect actual state size

    # Extract the position sub-states (indices 0-4) in forward order
    sods  = [d[0] for d in data]
    xs    = [d[1][:dim].copy() for d in data]  # list of length-5 vectors
    ...
    for k in range(N-2, -1, -1):
        ...
        # Transition matrix F (identity for walk model over short dt)
        F = np.eye(dim)

        # Process noise Q for the dt step (must match _ppp_pass)
        Q_k             = np.zeros((dim, dim))
        Q_k[0,0]        = 1e-8 * dt
        Q_k[1,1]        = 1e-8 * dt
        Q_k[2,2]        = 1e-8 * dt
        Q_k[3,3]        = 1e4  * dt
        Q_k[4,4]        = 1e-7 * dt  # must match _ppp_pass ZWD noise
        if dim == 6:
            Q_k[5,5]    = 1e-10 * dt  # ISB: near-constant hardware bias (GE mode only)

        P_k   = Ps[k]
        P_k1  = F @ P_k @ F.T + Q_k          # predicted covariance at k+1

        try:
            G_k = P_k @ F.T @ np.linalg.inv(P_k1)  # smoother gain
    ...
    # REF is passed in from postpos — do not hardcode here

    # Build smoothed results dict — copy everything from FWD, update position
    smoothed = {}
    sod_to_idx = {d[0]: i for i, d in enumerate(data)}
    for sod, r in fwd_results.items():
        if sod not in sod_to_idx:
            smoothed[sod] = {**r, 'pass': 'FWD'}
            continue
        i       = sod_to_idx[sod]
        dx_sm   = xs_s[i][:3]            # smoothed position offset
        pos_sm  = nom + dx_sm
        smoothed[sod] = {
            **r,
            'xyz'   : pos_sm.copy(),
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              ...
              constellation='GE'):
    """
    constellation : 'G'  = GPS only
                    'E'  = Galileo only
                    'GE' = GPS + Galileo (default)
    """
    ...
    NL_RATIO_THRESH = 3.0
    # Raise NL_VAR_THRESH from 100 m² (10m) to 400 m² (20m): after b_rec
    # is frozen and WL is fixed, the ambiguity variance correctly reflects
    # float uncertainty. The old threshold excluded sats that were at the
    # 1–5 m² level (perfectly suitable for LAMBDA) and is the main reason
    # NL count stays flat at 4 after initial fix.
    NL_VAR_THRESH   = (20.0)**2
    NL_RES_THRESH   = 0.15
    # Raised from 0.25 to 0.30: Galileo NL fracs computed with b_rec_gal
    # sometimes land at 0.26–0.29 due to IFB residuals; 0.25 was excluding
    # valid Galileo sats unnecessarily (Geng & Shi 2017).
    NL_EXCL_THRESH  = 0.30
    # 3mm pseudo-obs noise: tighter than previous 5mm. Keeps fixed ambiguity
    # within 3mm of integer without over-constraining noisy sats.
    NL_R_TIGHT      = (0.003)**2
    NL_PHASE_THRESH = 0.020  # 20mm: tolerates 1-2 freshly initialised sats without blocking all NL fixing
    _nl_diag_done   = False
    _phase_rms_prev = 999.

    # ── ISB state is only meaningful in combined GPS+Galileo mode ─────────────
    # In GPS-only or Galileo-only mode there is no inter-system bias to estimate:
    # - GPS-only  : all observations are GPS; no Galileo system-time offset exists.
    # - Galileo-only: only one system is present; the clock state already absorbs
    #   any receiver hardware bias.  Adding an unobservable ISB state inflates the
    #   covariance, introduces a rank-deficiency, and corrupts ambiguity floats.
    # Solution: use a 5-state vector for single-constellation, 6-state for GE.
    _USE_ISB = (constellation == 'GE')
    _N_BASE  = 6 if _USE_ISB else 5   # number of base states before ambiguities
    _ISB_IDX = 5                       # index within the 6-state vector (unused when _USE_ISB=False)

    x = np.zeros(_N_BASE); x[3] = iclk; x[4] = izwd
    P = np.zeros((_N_BASE, _N_BASE))
    P[0,0]=P[1,1]=P[2,2]=100.**2; P[3,3]=3000.**2; P[4,4]=0.5**2
    if _USE_ISB:
        # Galileo inter-system clock bias (ISB): initialise with 1000m² (≈30m)
        # uncertainty. In GPS+Galileo combined mode, CODE clocks refer to GPS
        # time; Galileo satellite clocks include a system-time offset that isn't
        # fully absorbed by the satellite clock products. This ISB must be
        # estimated per-session (≈1–5 m in code, ≈3–15 mm in phase).
        x[5] = 0.0
        P[5,5] = 1000.**2
    ...
    for epoch in eplist:
        ...
        Q[3,3]=1e4*dt
        # ZWD random-walk: 1e-7 m²/s ≈ 0.3 mm/√s which is realistic for a
        # mid-latitude station with diurnal ZWD variation of ~20 mm over 24 h.
        # The old value 1e-8 was too tight and caused ZWD to drift away from
        # the true value by end-of-day, producing the growing Up error.
        Q[4,4]=1e-7*dt
        # Galileo ISB: near-constant hardware bias — only estimated in GE mode.
        # In single-constellation modes the ISB slot does not exist in the state vector.
        if _USE_ISB:
            Q[5,5]=1e-10*dt
        ...
        # Ambiguity states start at _N_BASE (5 for single-const, 6 for GE)
        for k in range(namb): Q[_N_BASE+k,_N_BASE+k]=q_amb
        P+=Q
        ...
        for sid,so in sorted(sobs.items()):
                ...
                if n_hist >= 15:
                    ...
                    if n_hist >= min_n:
                        ...
                        if sys_id not in b_rec_frozen:
                            ...
                            # GPS needs >=5 sats; Galileo typically has only 4-6
                            # visible so lower threshold to 3 for 'E'.
                            min_freeze = 3 if sys_id=='E' else 5
                            if len(all_fracs_sys)>=min_freeze:
                                ...
                                if agreement >= max(2, 0.6*len(all_fracs_sys)):
                        ...
                        if sys_id not in b_rec_frozen:
                            ...
                            if sid in _wl_history:
                                ...
                                if diff == 0:
                                    ...
                                    # Small diff (<=4 cyc): mid-arc multipath with frozen b_rec.
                                    # Keep history NWL — genuine new arcs produce diffs >> 4 cyc.
                                    print(f"[WL PERSIST] {sid} using prev NWL={hist_NWL} "
                                          f"(new arc gave NWL={NWL}, diff={diff}<=4->keep)")
                                    NWL_to_use = hist_NWL
                                else:
                                    # Large diff (>4 cyc): genuine new orbital pass — use new NWL.
                                    print(f"[WL NEWARK] {sid} using new NWL={NWL} "
                                          f"(prev={hist_NWL}, diff={diff}>4->new arc)")
                                    _wl_history[sid] = NWL
                                    _wl_history_ptrace[sid] = pt_now
                                    NWL_to_use = NWL
                            else:
            ...
            if not phi.get(sid,False):
                if sid in _amb_init:
                    ...
                    # Subtract ISB for Galileo so ambiguity absorbs only the
                    # integer cycle offset, not the system-time bias.
                    # ISB is only in the state vector when _USE_ISB is True (GE mode).
                    isb_now = (x[_ISB_IDX] if (_USE_ISB and m.get('_sys')=='E') else 0.)
                    rp0=_rp(m,x[3],x[4])+isb_now; x[ki]=LIFc-rp0
                    P[ki,ki]=300.**2; phi[sid]=True
                    _sat_age[sid]=0  # new sat starts at age 0
                    pt_now = P[0,0]+P[1,1]+P[2,2]
                    _amb_init_ptrace[sid] = pt_now
                    # Mark sat as having been initialised during a converged epoch.
                    # p_trace < 9.0 m² (3m per axis) is the threshold — this is
                    # satisfied well before cm-level, around the 200-500mm phase.
                    # The old threshold of 0.30 m² was never reached, so BWD
                    # inheritance was always empty, crippling backward passes.
                    if pt_now < 9.0:
        ...
        if nproc==0:
            ...
            for m in geom:
                ...
                if m['sid'] in _amb_seeded:
                    ...
                    isb_now = (x[_ISB_IDX] if (_USE_ISB and m.get('_sys')=='E') else 0.)
                    rp0=_rp(m,x[3],x[4])+isb_now; x[ki]=m['LIFc']-rp0; P[ki,ki]=300.**2
        ...
        for ri,m in enumerate(geom):
            ...
            is_gal = (m.get('_sys')=='E')
            rp=_rp(m,xs[3],xs[4])
            # ISB contribution to predicted range: subtract current ISB estimate
            # for Galileo sats only — and only when ISB state exists (GE mode).
            if _USE_ISB and is_gal:
                rp = rp + xs[_ISB_IDX]
            rr=2*ri
            H[rr,0]=-u[0]; H[rr,1]=-u[1]; H[rr,2]=-u[2]; H[rr,3]=1.; H[rr,4]=mw
            if _USE_ISB and is_gal: H[rr,_ISB_IDX]=1.   # ISB partial for Galileo code
            z[rr]=m['PIF']-rp; Rd[rr]=_sig(m['el'],SC)**2
            rl=2*ri+1
            H[rl,0]=-u[0]; H[rl,1]=-u[1]; H[rl,2]=-u[2]; H[rl,3]=1.; H[rl,4]=mw; H[rl,ki]=1.
            if _USE_ISB and is_gal: H[rl,_ISB_IDX]=1.   # ISB for Galileo phase
        ...
        phase_res_now=[m['LIFc']-(_rp(m,x[3],x[4])+(x[_ISB_IDX] if (_USE_ISB and m.get('_sys')=='E') else 0.)+x[m['ki']]) for m in geom]
        # For the NL gate, compute phase RMS only on sats with converged ambiguities
        # (age > 5 epochs). Fresh sats have inflated residuals that would block NL
        # fixing for all other satellites even when the solution is healthy.
        phase_res_mature=[m['LIFc']-(_rp(m,x[3],x[4])+(x[_ISB_IDX] if (_USE_ISB and m.get('_sys')=='E') else 0.)+x[m['ki']])
                          for m in geom if m.get('age',0)>5]
        phase_rms_now=(math.sqrt(np.mean(np.array(phase_res_mature)**2))
                       if phase_res_mature else
                       math.sqrt(np.mean(np.array(phase_res_now)**2)) if phase_res_now else 999.)
        ...
        if len(nl_cands) >= 2:
            ...
            # STEP 1: b_rec_nl per constellation (Collins 2010, Geng 2010)
            # GPS and Galileo have different hardware NL fractional biases.
            # Pooling them into one median contaminates both corrections and
            # pushes fracs of both constellations past the 0.25 exclusion
            # threshold, preventing LAMBDA from ever firing in combined mode.
            fracs_raw = np.array([f - round(f) for f in N1_floats])
            gps_fracs = [fracs_raw[i] for i,m in enumerate(nl_cands) if m.get('_sys')!='E']
            gal_fracs = [fracs_raw[i] for i,m in enumerate(nl_cands) if m.get('_sys')=='E']
            b_rec_gps = float(np.median(gps_fracs)) if gps_fracs else float(np.median(fracs_raw))
            b_rec_gal = float(np.median(gal_fracs)) if gal_fracs else b_rec_gps
            b_rec_nl  = b_rec_gps  # kept for DIAG print reference
            N1_corr   = np.array([
                N1_floats[i] - (b_rec_gal if nl_cands[i].get('_sys')=='E' else b_rec_gps)
                for i in range(len(N1_floats))])
            ...
            # Only permanently exclude a sat whose own ambiguity has converged
            # (P[ki,ki] < 1.0 m²) AND whose frac is still bad. A large P[ki,ki]
            # means the ambiguity hasn't settled yet — bad frac is a float
            # artifact, not a wrong NWL. Blacklisting unconverged sats blocks
            # valid Galileo candidates for the entire session.
            # Only permanently exclude a sat whose own ambiguity has converged
            # (P[ki,ki] < 1.0 m²) AND whose frac is still bad AND which has
            # been tracked for >30 epochs (age guard prevents fresh sats from
            # being blacklisted before their ambiguity has had time to converge).
            for sid in excl_sids:
                ki_excl = sidx.get(sid)
                sat_age_excl = _sat_age.get(sid, 0)
                if ki_excl is not None and P[ki_excl, ki_excl] < 1.0 and sat_age_excl > 30:
                    _nl_bad_nwl.add(sid)
            ...
            if len(clean_idx) >= 2:
                ...
                # Per-satellite IF denominator: GPS and Galileo use different
                # frequency pairs so their ambiguity states have different units.
                # Using a single GPS denom for Galileo sats corrupts Q_nl and
                # excludes Galileo from LAMBDA. Build a per-sat scale vector.
                denom_gps = ALFA*LAMBDA1    - BETA*LAMBDA2
                denom_gal = ALFA_E*LAMBDA_E1 - BETA_E*LAMBDA_E5A
                scales = np.array([denom_gal if m.get('_sys')=='E' else denom_gps
                                   for m in nl_cands_c])
                Q_nl = P[np.ix_(idxs_c, idxs_c)] / np.outer(scales, scales)
        ...
        # ── RTS: store full state after each update ───────────────────────────
        # We keep (sod, x_copy, P_copy, H, z, Rd) so the smoother can replay.
        # Only store when direction==1 (FWD); RTS runs post-FWD.
        if direction == 1:
        ...
        code_res=[(m['PIF']-_rp(m,x[3],x[4])-(x[_ISB_IDX] if (_USE_ISB and m.get('_sys')=='E') else 0.))
                  for m in geom]
        ...
        ISB=x[_ISB_IDX] if _USE_ISB else 0.   # Galileo ISB — zero in single-const modes
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS+Galileo PPP v50 — RTS smoother + multi-constellation framework")
    print("="*72)
    ...
    # ── Common arguments for every _ppp_pass call ─────────────────────────────
    ...
    # ── Run all three constellations ──────────────────────────────────────────
    mode_labels = [('G','GPS-only'), ('E','Galileo-only'), ('GE','GPS+Galileo')]
    all_fwd     = {}   # label -> fwd dict
    all_rts     = {}   # label -> smoothed dict
    all_meta    = {}   # label -> {wl_fixed, fwd_converged}

    for const, label in mode_labels:
        print(f"\n{'='*72}")
        print(f"[MODE] {label}  (constellation='{const}')")
        print(f"[PASS FWD] {label} ...")
        # Reset the RTS accumulator for this run
        _rts_store._data = []
        ...
        rts = _rts_smooth(fwd, APX.copy(), REF)

        all_fwd[label]  = fwd
        all_rts[label]  = rts
        all_meta[label] = {'wl_fixed': wl_f}
        ...
        if best60:
            ...
            diff    = avg_xyz - REF; enu_d = Re @ diff
            dE,dN,dU = enu_d*1e3
            diag3d   = np.linalg.norm(diff)*1e3
            print(f"  DIAG 3D={diag3d:.1f}mm  dE={dE:+.1f}  dN={dN:+.1f}  dU={dU:+.1f} mm")

        # ── Per-mode metrics ───────────────────────────────────────────────
        _print_metrics(fwd, rts, REF, label)

    # ── Primary mode result list (GPS+Galileo FWD for CSV) ───────────────────
    ...
    if outfile and rl:
        ...
        with open(outfile,'w') as fo:
            fo.write(
                ...
                "3D_mm,"
                "N,WL_fixed,NL_fixed,"
                "ZHD_m,ZWD_m,ZTD_m,ISB_mm,CodeRMS_mm,PhsRMS_mm\n"
            )
            for sod,r in rl:
...
def _compute_metrics(results, REF):
    """
    Compute per-epoch ENU errors and aggregate statistics from a results dict.

    Returns
    -------
    dict with keys:
      sods, e_mm, n_mm, u_mm, d3_mm  – per-epoch arrays
      rms_e, rms_n, rms_u, rms_3d    – RMS over all converged epochs (3D<200mm)
      conv_time_10cm                  – first SOD where 3D < 100mm (or None)
      conv_time_5cm                   – first SOD where 3D < 50mm  (or None)
      wl_counts, nl_counts, sods_all  – per-epoch ambiguity counts
      n_wl_fix, n_nl_fix              – total WL/NL fixed epochs
      nl_first_sod                    – first SOD with NL > 0
    """
    if not results:
        return None
    lr,lo,_ = _lla(REF); Re = _enu(lr,lo)
    ...
    # Convergence time: first epoch where 3D stays < threshold for ≥5 epochs
    def _conv_time(threshold):
        for i, (sod, _) in enumerate(fwd_list):
            window = d3_all[i:i+5]
            if len(window) == 5 and np.all(window < threshold):
                return sod
        return None

    # NL first fix SOD
    nl_first_sod = None
    for sod, r in fwd_list:
        if r.get('nl_fixed', 0) > 0:
            nl_first_sod = sod
            break

    return dict(
        sods=sods_all, e_mm=enu_all[:,0], n_mm=enu_all[:,1], u_mm=enu_all[:,2],
        d3_mm=d3_all,
        rms_e=rms_e, rms_n=rms_n, rms_u=rms_u, rms_3d=rms_3d,
        conv_time_10cm=_conv_time(100.), conv_time_5cm=_conv_time(50.),
...
def _print_metrics(fwd, rts, REF, label):
    """Print convergence / accuracy metrics for one mode."""
    m = _compute_metrics(fwd, REF)
    if m is None:
        return
    print(f"\n  ── Metrics: {label} ──────────────────────────────────────────")
    print(f"  RMS (E/N/U/3D): {m['rms_e']:.1f} / {m['rms_n']:.1f} / "
          f"{m['rms_u']:.1f} / {m['rms_3d']:.1f} mm  (3D < 200 mm subset)")
    ...
    print(f"  Convergence (5-epoch sustain) < 10 cm: {ct10}")
    print(f"  Convergence (5-epoch sustain) <  5 cm: {ct5}")
    print(f"  WL-fixed epochs: {m['n_wl_fix']}/{len(m['sods'])}  "
          f"NL-fixed epochs: {m['n_nl_fix']}/{len(m['sods'])}  "
          f"First NL: {m['nl_first_sod']}")

    # RTS improvement
    mr = _compute_metrics(rts, REF)
    if mr:
        print(f"  RTS RMS (E/N/U/3D): {mr['rms_e']:.1f} / {mr['rms_n']:.1f} / "
              f"{mr['rms_u']:.1f} / {mr['rms_3d']:.1f} mm  (improvement over FWD)")
...
def _plot_comparison(all_fwd, all_rts, REF):
    ...
    Four-panel comparison plot:
      (a) 3D error vs time — FWD, one curve per mode
      (b) ENU error vs time — GPS+Galileo FWD
      (c) Number of fixed ambiguities vs time
      (d) 3D error FWD vs RTS for GPS+Galileo
    """
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    fig.suptitle('PPP-AR Multi-Constellation Comparison', fontsize=14, fontweight='bold')

    # ── (a) 3D error vs time — all modes FWD ──────────────────────────────
    ax = axes[0, 0]
    for label, fwd in all_fwd.items():
        m = _compute_metrics(fwd, REF)
        if m is None: continue
        sod_h = m['sods'] / 3600.
        ax.plot(sod_h, m['d3_mm'], color=colors.get(label,'k'), alpha=0.8,
    ...
    ax.set_xlabel('Time (hours of day)'); ax.set_ylabel('3D Error (mm)')
    ax.set_title('(a) 3D Positioning Error — FWD')
    ax.set_ylim(0, 500); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # ── (b) ENU errors — GPS+Galileo FWD ─────────────────────────────────
    ax = axes[0, 1]
    m = _compute_metrics(all_fwd.get('GPS+Galileo', {}), REF)
    if m is not None:
        sod_h = m['sods'] / 3600.
        ax.plot(sod_h, m['e_mm'], color='#e6194b', linewidth=0.8, label='East')
        ax.plot(sod_h, m['n_mm'], color='#3cb44b', linewidth=0.8, label='North')
        ax.plot(sod_h, m['u_mm'], color='#4363d8', linewidth=0.8, label='Up')
        ax.axhline(0, color='black', linewidth=0.5)
    ax.set_xlabel('Time (hours of day)'); ax.set_ylabel('Error (mm)')
    ax.set_title('(b) ENU Errors — GPS+Galileo FWD')
    ax.set_ylim(-300, 300); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # ── (c) Ambiguity counts vs time ──────────────────────────────────────
    ax = axes[1, 0]
    for label, fwd in all_fwd.items():
        m = _compute_metrics(fwd, REF)
        if m is None: continue
        sod_h = m['sods'] / 3600.
        ax.plot(sod_h, m['nl_counts'], color=colors.get(label,'k'),
                linewidth=0.9, label=f'{label} NL')
    ax.set_xlabel('Time (hours of day)'); ax.set_ylabel('# NL-fixed satellites')
    ax.set_title('(c) NL-Fixed Ambiguities vs Time')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # ── (d) FWD vs RTS — GPS+Galileo ─────────────────────────────────────
    ax = axes[1, 1]
    fwd = all_fwd.get('GPS+Galileo', {}); rts = all_rts.get('GPS+Galileo', {})
    mf = _compute_metrics(fwd, REF); mr = _compute_metrics(rts, REF)
    if mf is not None:
        ax.plot(mf['sods']/3600., mf['d3_mm'], color='#4363d8', linewidth=0.8,
                alpha=0.8, label='FWD')
    if mr is not None:
        ax.plot(mr['sods']/3600., mr['d3_mm'], color='#f58231', linewidth=0.8,
                alpha=0.8, label='RTS smoothed')
    ax.axhline(50,  color='gray', linestyle='--', linewidth=0.7)
    ax.set_xlabel('Time (hours of day)'); ax.set_ylabel('3D Error (mm)')
    ax.set_title('(d) FWD vs RTS Smoothed — GPS+Galileo')
    ax.set_ylim(0, 300); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    plt.tight_layout()

    # Derive output path from outfile or use DATA dir
