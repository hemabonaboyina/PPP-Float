"""
ppp_postpos.py  v11  --  GPS PPP  <2 cm goal
Two-pass forward-backward, ppp_ar filter_standard, correct BIA parser.
"""
...
def parse_bia(fp):
    """
    Robust token-based SINEX-BIA parser.
    Line example:
     OSB  G080 G01           C1W       2026:038:00000 2026:039:00000 ns   -5.2816  0.0000
    PRN = cols 9:12,  OBS1 = cols 25:29
    Unit+value found by scanning tokens after col 29 for 'ns'/'cyc'/'m'.
    """
    ...
    with open(fp,'r',errors='replace') as fh:
        for ln in fh:
            ...
            # Use token split — far more robust than fixed columns
            # Format: <space>OSB  <site> <PRN> <...> <obs1> <...> <unit> <value>
            parts=ln.split()
            # parts[0]='OSB', parts[1]=site e.g.'G080', parts[2]=PRN e.g.'G01'
...
def parse_atx(fp, ant_type='LEIAR25.R4', dome='NONE'):
    """
    Parse IGS ANTEX file. Returns dict:
      {'rec': {'L1': {'pco': [N,E,U], 'pcv_noazi': {zen: val}},
               'L2': {'pco': [N,E,U], 'pcv_noazi': {zen: val}}},
       'sat': {prn: {'L1': {'pco': [N,E,U], 'pcv_noazi': {nadir: val}},
                     'L2': {...}}}}
    ...
    target = f"{ant_type:<16}{dome:<4}"   # 20-char key as in ATX cols 0-19
    in_ant = False; in_freq = False; cur_sys = None
    ...
    with open(fp, 'r', errors='replace') as fh:
        for ln in fh:
            ...
            if 'TYPE / SERIAL NO' in label:
                # cols 0-19: antenna type+dome  cols 20-39: serial / SVN
                ant_key = ln[0:20].strip()
                svn     = ln[20:40].strip()   # e.g. "G01" for satellites
                if ant_key == target.strip():
                    in_ant = True             # receiver antenna match
                    cur_block = 'rec'
                elif len(svn) == 3 and svn[0] in 'GREJCIS':
                    in_ant = True             # satellite antenna
                    cur_block = svn           # e.g. "G01"
                else:
                    in_ant = False
            ...
            if 'START OF FREQUENCY' in label:
                freq_label = ln[3:6].strip()  # e.g. "G01" or "G02"
                in_freq = True; cur_pco = None; cur_pcv = {}
            ...
            if 'END OF FREQUENCY' in label:
                ...
                if band and cur_pco is not None:
                    if cur_block == 'rec':
                        result['rec'].setdefault(band, {})
                        result['rec'][band] = {'pco': cur_pco, 'pcv_noazi': cur_pcv}
                    else:
                        result['sat'].setdefault(cur_block, {})
                        result['sat'][cur_block][band] = {'pco': cur_pco, 'pcv_noazi': cur_pcv}
                in_freq = False
            ...
            if in_freq and 'NORTH / EAST / UP' in label:
                try:
                    cur_pco = [float(ln[0:10])*1e-3,   # N → metres
                               float(ln[10:20])*1e-3,   # E → metres
                               float(ln[20:30])*1e-3]   # U → metres
                except: pass

            if in_freq and ln[3:8].strip().replace('.','').lstrip('-').isdigit():
                # NOAZI or azimuth-elevation grid row
                parts = ln.split()
                if parts[0] == 'NOAZI':
                    # zen values: 0°→90° in DZEN steps (typically 5°)
                    vals = [float(v)*1e-3 for v in parts[1:]]
                    dzen = 5.0; zen = 0.0
                    for v in vals:
                        cur_pcv[round(zen, 1)] = v; zen += dzen
...
def _pcv_noazi(pcv_dict, zen_deg):
    """Interpolate NOAZI PCV table at zenith angle zen_deg (degrees)."""
...
def _compute_sat(sid,so,tow,rec_xyz,ant_h,sp3t,sp3p,sp3c,
    ...
    # ---- FIX NaN / missing bias ----
    if b1 is None or np.isnan(b1):
       b1 = 0.0

    if b2 is None or np.isnan(b2):
       b2 = 0.0
    ...
    if sc is None:
        # fallback instead of dropping satellite
        sc = 0.0
    ...
    # Step 1: preliminary APC (ant_h only) to get elevation angle
    rec_apc0=rec_xyz+ant_h*er
    el,az=_elaz(rec_apc0,sv)
    if el is None or el<ELM: return None
    # Step 2: apply PCO+PCV using elevation from step 1
    ...
    # Recompute el/az with corrected APC (small update, improves consistency)
    ...
    mw_raw=(FREQ1*L1c-FREQ2*L2c)/(FREQ1-FREQ2)-(FREQ1*P1c+FREQ2*P2c)/((FREQ1+FREQ2)*LAMBDA1)
...
def run_ppp_pass(epochs,sp3t,sp3p,sp3c,clkd,code_osb,atx,
    ...
    P_p[3,3]=3000.**2; P_p[4,4]=0.30**2
    P_p[5,5]=P_p[6,6]=0.01**2              # gradient initial sigma 10 mm
    ...
    for epoch in ep_list:
        ...
        Q[0,0]=Q[1,1]=Q[2,2]=1e-10*dt      # position (static)
        Q[3,3]=9e4*dt                        # clock — 300m sigma/sqrt(s), sensible
        Q[4,4]=1e-8*dt                       # ZTD wet
        Q[5,5]=Q[6,6]=5e-10*dt              # trop gradient
        for k in range(na): Q[7+k,7+k]=1e-12*dt  # ambiguities start at index 7
        P_p+=Q

        sun=_sun(tow)
        rec_xyz=nom+x_p[:3]; ztd_wet=x_p[4]; gn=x_p[5]; ge=x_p[6]
        geom=[]
        ...
        for sid,so in sorted(sobs.items()):
            ...
            # IF residual slip check (keep as secondary guard)
        ...
        if len(geom) < 6:
           print(f"[WARN] Too few satellites ({len(geom)}) → skipping epoch")
           continue
        ...
        for ri,m in enumerate(geom):
            ...
            mf_g=mw*cot_el                        # gradient mapping function
            dmn=mf_g*math.cos(az_m)               # north partial
            dme=mf_g*math.sin(az_m)               # east partial
            H[rr,0]=-u[0]; H[rr,1]=-u[1]; H[rr,2]=-u[2]; H[rr,3]=1.; H[rr,4]=mw
            H[rr,5]=dmn; H[rr,6]=dme             # gradient partials
            z[rr]=m['PIF']-rp
            rl=2*ri+1
            H[rl,0]=-u[0]; H[rl,1]=-u[1]; H[rl,2]=-u[2]; H[rl,3]=1.; H[rl,4]=mw
            H[rl,5]=dmn; H[rl,6]=dme; H[rl,ki]=1.
            z[rl]=m['LIFc']-(rp+x_snap[ki])
            Rd[rr]=_sig(m['el'],SC)**2; Rd[rl]=_sig(m['el'],SP)**2

        # ---- SAFETY: residual gating ----
        if np.any(np.isnan(z)) or np.any(np.isinf(z)):
            continue

        if np.max(np.abs(z)) > 200:
            print("[WARN] Large residual → skipping epoch")
            continue
        if filter_standard(x_p,P_p,H.T,z,np.diag(Rd))!=0: continue
        nproc+=1
        # Re-centre only after 10 epochs and only if position shift > 1 cm
        # Cap the shift to 10 m to avoid recentring on a bad epoch
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*65); print("GPS PPP v11 (forward-backward)  IISC DOY038/2026"); print("="*65)
