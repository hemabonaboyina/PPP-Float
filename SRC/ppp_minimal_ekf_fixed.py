...
def _ppp_pass(epochs, sp3t, sp, sc, clkd, osb, ah, nom, iclk, izwd,
    ...
    _AMB_MIN_EPOCHS = 3     # consecutive valid epochs required before init
    ...
    for epoch in eplist:
        ...
        for sid, so in sorted(sobs.items()):
            ...
            # ── Birth queuing: require ≥ _AMB_MIN_EPOCHS consecutive valid epochs
            if not amb_active.get(sid, False):
                if _amb_valid_streak[sid] < _AMB_MIN_EPOCHS:
                    ...
                    geom.append(m)
                    _sat_age[sid] += 1
                    continue   # code row will be added below via geom build
        ...
        for ri, m in enumerate(geom):
            ...
            # ── Phase row ────────────────────────────────────────────────────
            # Include if and only if ambiguity state is valid (NO lifecycle gating)
            if m['sid'] in _newborn_pending:
                # Not yet born — phase row excluded (x[ki] uninitialised)
                _reject_obs(rl, H, z, Rd,
                            "newborn_phase_excluded_birth_pending",
                            m['sid'], sod, startup=True)
            elif not amb_active.get(m['sid'], False):
                # State not yet valid (< _AMB_MIN_EPOCHS streak)
                _reject_obs(rl, H, z, Rd,
                            "phase_excluded_insufficient_streak",
                            m['sid'], sod, startup=True)
            else:
                ...
                if not math.isfinite(phase_res):
                    ...
                    # Old code: Rd[rl] = max(Rd[rl], 1.0**2)
                    # This inflated phase sigma from ~3 mm to 1000 mm, collapsing
                    # phase/code information ratio from ~250 000× to ~2×.
                    # Now: only log large residuals; information dominance preserved.
        ...
        # ── NL release check (disabled — kept to avoid crash if nl_fixed non-empty) ─
