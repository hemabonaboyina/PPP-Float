...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    ob=osb.get(sid,{})
    # STRICT OSB selection: all 4 OSBs must be present — no raw fallback.
    ...
    if (code1_type not in ob or code2_type not in ob or
        ...
        if sid not in _osb_dbg_printed:
            ...
            print(f"{sid}: signals={_sig_pair_str}, osb_ok=False, used=False  (missing_osb)")
        return None  # STRICT: skip satellite — OSB incomplete, no raw fallback
    b_C1 = ob.get(code1_type, 0.)
    b_C2 = ob.get(code2_type, 0.)
    b_L1 = ob.get(phase1_type, 0.)
    b_L2 = ob.get(phase2_type, 0.)
    # Reject out-of-range OSB values — skip satellite entirely.
    if abs(b_C1) > 10.0 or abs(b_C2) > 10.0 or abs(b_L1) > 1.0 or abs(b_L2) > 1.0:
        if sid not in _osb_dbg_printed:
            _osb_dbg_printed.add(sid)
            print(f"{sid}: signals={_sig_pair_str}, osb_ok=False, used=False  (bad_bias)")
        return None  # skip satellite — bad OSB value

    ar_skip_reason = None
    no_AR = False
    use_for_ar = True

    # v84 PART 1: track AR state changes
    _has_valid_osb = True
    _prev_gps_ar_state[sid] = no_AR  # update silently; summary printed per 300 ep

    if sid not in _osb_dbg_printed:
        _osb_dbg_printed.add(sid)
        print(f"{sid}: signals={_sig_pair_str}, osb_ok=True, used=True")
...
def _proc_gal(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    ob=osb.get(sid,{})
    # STRICT: all 4 OSBs must be present — no raw fallback.
    _gal_sig_str = f"{code1_type}/{phase1_type}+{code2_type}/{phase2_type}"
    if (code1_type not in ob or code2_type not in ob or
            phase1_type not in ob or phase2_type not in ob):
        if sid not in _osb_dbg_printed:
            _osb_dbg_printed.add(sid)
            print(f"{sid}: signals={_gal_sig_str}, osb_ok=False, used=False  (missing_osb)")
        return None  # STRICT: skip satellite — OSB incomplete
    b_C1 = ob.get(code1_type, 0.)
    b_C5 = ob.get(code2_type, 0.)
    b_L1 = ob.get(phase1_type, 0.)
    b_L5 = ob.get(phase2_type, 0.)
    if abs(b_C1) > 10.0 or abs(b_C5) > 10.0 or abs(b_L1) > 1.0 or abs(b_L5) > 1.0:
        if sid not in _osb_dbg_printed:
            _osb_dbg_printed.add(sid)
            print(f"{sid}: signals={_gal_sig_str}, osb_ok=False, used=False  (bad_bias)")
        return None  # skip satellite — bad OSB value
    ...
    if sid not in _osb_dbg_printed:
        _osb_dbg_printed.add(sid)
        print(f"{sid}: signals={_gal_sig_str}, osb_ok=True, used=True")
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              satx,att,recx,elm=math.radians(10.),SC=0.1667,SP=0.0144,
    ...
    ZWD_PRIOR         = 0.12
    ZWD_PRIOR_SIGMA   = 0.05   # v111: relaxed 0.02→0.05 m — allows ZWD to adapt without forcing height drift
    ZWD_CLAMP         = 0.015
    _zwd_prev         = None
    _nl_diag_done     = False
    ...
    _HEIGHT_R = (0.5)**2    # 50 cm sigma → variance (10× relaxed, non-distorting)
    ...
    for epoch in eplist:
        ...
        Q[IDX_CLK,IDX_CLK]=(0.01)**2*dt*pos_clk_q_scale*clk_q_scale
        Q[IDX_ISB_GAL,IDX_ISB_GAL]=(0.001)**2*dt  # ISB near-constant (tightened from 0.01)
        # ZWD process noise: baseline (no ×1.5 scaling).
        # ZWD_Q_SCALE: Run-L scalar multiplier — only ZWD Q is affected.
        Q[IDX_ZWD,IDX_ZWD]=2.5e-9*dt*ZWD_Q_SCALE
        ...
        for ri,m in enumerate(geom):
            ...
            _code_sig = max(_sig_exp(m['el'],SC,2.0) * _code_scale * _cst_scale, 0.3)
            Rd[r]=_code_sig**2
            ...
            z[r]=m['L1mc']-pred_L1
            _sin_el_ph = max(math.sin(m['el']), 0.1)
            _SP_cst = SP * _cst_scale
            phase_sig = max(_SP_cst / (_sin_el_ph ** 2) * (1.0 + 0.5 / _sin_el_ph) * (5. if m.get('age',99)<=3 else 1.), 0.003)
            Rd[r]=phase_sig**2
            if abs(z[r])>PHASE_RES_GATE: Rd[r]=max(Rd[r], 1.0**2)
            # Galileo hard phase outlier gate: residual > 0.05 m → near-zero weight
            if is_gal and abs(z[r]) > 0.05:
                Rd[r] = 1e6
            ...
            z[r]=m['L2mc']-pred_L2
            phase_sig2 = max(_SP_cst / (_sin_el_ph ** 2) * (1.0 + 0.5 / _sin_el_ph) * (5. if m.get('age',99)<=3 else 1.), 0.003)
            Rd[r]=phase_sig2**2
            if abs(z[r])>PHASE_RES_GATE: Rd[r]=max(Rd[r], 1.0**2)
            # Galileo hard phase outlier gate: residual > 0.05 m → near-zero weight
            if is_gal and abs(z[r]) > 0.05:
        ...
        z_p[n_total]=ZWD_PRIOR-xs[IDX_ZWD]
        ...
        # height constraint removed — let filter estimate height freely
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print(f"\n{'='*72}")
    print("[v110] GPS+Galileo PPP — ZWD Q×1.5 + elev floor + height constraint (pure float)")
    print(f"{'='*72}")
    ...
    # ── Results summary ────────────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print("[v110] RESULTS SUMMARY  (pure float, v110: ZWD Q×1.5 + elev floor 0.05 + height σ=8cm)")
    print(f"{'='*72}")
    ...
    try:
        ...
        fig_v.suptitle(
            'PPP v110 — ZWD Q×1.5 + Elev Floor + Height Constraint  (pure float)',
            fontsize=11, fontweight='bold')
        ...
        fig_a.suptitle('PPP v109 — Full Analysis  (pure float)',
        ...
        print(f"[v110] Analysis plot saved: {_ap}")
        plt.close(fig_a)

    except Exception as _ve:
        print(f"[v106] Plot failed: {_ve}")

    print(f"\n[v110] Wall time: {_time.time() - t0:.1f}s")
    print("="*72)
