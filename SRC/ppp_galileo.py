"""
ppp_galileo_only.py  v1
========================
Galileo-only PPP with Ambiguity Resolution.
Derived from ppp_gpsonly.py v3 — GPS code replaced with Galileo E1+E5a.
Signals: E1(C1C/L1C) + E5a(C5Q/L5Q), ionosphere-free combination.
Frequencies: E1=1575.42MHz, E5a=1176.45MHz.
All v3 fixes retained: probation system, adaptive ratio, ZWD clamp.
"""
...
from constants import CLIGHT, OMGE, RE_WGS84
from kf import filter_standard

# Galileo E1 + E5a frequencies
FREQ1     = 1575.42e6   # E1
FREQ2     = 1176.45e6   # E5a
    ...
    with open(fp, 'r', errors='replace') as fh:
        ...
        for raw in fh:
            ...
            if 'END OF ANTENNA' in lb:
                if cur:
                    if isat and cprn:
                        p1=np.array(pf.get('E01',pf.get('G01',[0,0,0])),float)
                        p2=np.array(pf.get('E05',pf.get('G05',[0,0,0])),float)  # E5a=1176.45 MHz → G05, NOT G02
                        v1=pv.get('E01',pv.get('G01',[])); v2=pv.get('E05',pv.get('G05',[]))
                        ...
                        rec_atx[cant]={'L1':np.array(pf.get('E01',pf.get('G01',[0,0,0])),float),
                                       'L2':np.array(pf.get('E05',pf.get('G05',[0,0,0])),float),  # E5a→G05
                                       'v1':list(pv.get('E01',pv.get('G01',[]))),'v2':list(pv.get('E05',pv.get('G05',[]))),
                                       'z1':z1,'dz':dz}
                cur=None; continue
        ...
        for raw in f:
            ...
            if hdr:
                ...
                if ln.startswith('>'):
                    ...
                    e['sats'][sid]=obs
    print(f"[OBS]  {len(ep)} epochs  ant_h={ah:.4f}m  ant={ak}  (Galileo only)")
    return ot, ep, ah, ak
...
def parse_bia(fp):
    ...
    with open(fp, 'r', errors='replace') as fh:
        for ln in fh:
            ...
            # Reject malformed PRNs — must be exactly 3 chars: one letter + two digits
            if len(prn)!=3 or not prn[0].isalpha() or not prn[1:].isdigit(): continue
            if not obs: continue
            ...
            if unit=='ns': B[prn][obs]=val*1e-9*CLIGHT
                ...
                elif obs in ('L2W','L2C','L2P','L2X','L5Q'): B[prn][obs]=val*LAMBDA2
                else: B[prn][obs]=val*LAMBDA1
            else: B[prn][obs]=val
    tot=sum(len(v) for v in B.values())
    print(f"[BIA]  {tot} OSB entries  {len(B)} PRNs")
    if tot>0:
        first_gal=next((p for p in sorted(B) if p.startswith('E')),None)
        if first_gal:
            e=B[first_gal]
            print(f"       {first_gal} C1C={e.get('C1C',float('nan')):+.4f}m  C5Q={e.get('C5Q',float('nan')):+.4f}m  "
                  f"L1C={e.get('L1C',float('nan')):+.6f}m  L5Q={e.get('L5Q',float('nan')):+.6f}m")
        no_c1=[p for p in B if p.startswith('E') and 'C1C' not in B[p] and 'C1X' not in B[p]]
        no_c5=[p for p in B if p.startswith('E') and 'C5Q' not in B[p] and 'C5X' not in B[p] and 'C5I' not in B[p]]
        bad_osb=sorted(set(no_c1)|set(no_c5))
        if bad_osb: print(f"       [WARN] {len(bad_osb)} Galileo PRNs lack code OSB → excluded from AR: {bad_osb}")
    return dict(B)
...
def _proc(sid, so, tow, rxyz, ah, sp3t, sp, sc, clkd, osb,
    ...
    # Galileo E1 + E5a — try multiple RINEX obs code variants
    P1=so.get('C1C',so.get('C1X',0.))
    P2=so.get('C5Q',so.get('C5X',so.get('C5I',0.)))
    L1=so.get('L1C',so.get('L1X',0.))
    L2=so.get('L5Q',so.get('L5X',so.get('L5I',0.)))
    if P1==0 or P2==0 or L1==0 or L2==0: return None

    ob=osb.get(sid,{})
    # --- OSB completeness gate -------------------------------------------
    # If a satellite has NO code bias entry it was not calibrated in this
    # product.  Using b=0 would mix uncorrected pseudoranges into the IF
    # observable and corrupt WL/NL — exclude the satellite entirely.
    b1=ob.get('C1C',ob.get('C1X',None))
    b2=ob.get('C5Q',ob.get('C5X',ob.get('C5I',None)))
    if b1 is None or b2 is None: return None   # no OSB → skip
    # Phase OSB: default 0 if absent (affects NL integer but not geometry)
    PIF=_ifc(P1-b1,P2-b2); LIF=_ifc(L1*LAMBDA1,L2*LAMBDA2)
    bl1=ob.get('L1C',ob.get('L1X',0.))
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              ...
              direction=1,label="FWD",wl_init=None,amb_init=None,b_rec_init=None,
              noisy_wl_init=None,nl_init=None,izwd_var=0.04):
    ...
    _amb_seeded=set(); nl_fixed=dict(nl_init) if nl_init else {}
    if nl_init: print(f"[NL SEEDED] {len(nl_fixed)} integers inherited: {sorted(nl_fixed.keys())}")
    ...
    NL_EXCL_THRESH=0.25
    NL_R_TIGHT=(0.005)**2
    NL_PHASE_THRESH=0.020   # 20mm — tight gate prevents bad early fixes
    NL_PROBATION_EPOCHS=20  # epochs a new fix must survive before being trusted
    _nl_diag_done=False

    x=np.zeros(5); x[3]=iclk; x[4]=izwd
    P=np.zeros((5,5))
    P[0,0]=P[1,1]=P[2,2]=100.**2; P[3,3]=3000.**2; P[4,4]=izwd_var
    ...
    # If a pre-calibrated receiver WL fractional bias is supplied (e.g. from
    # the forward pass), freeze it immediately so BWD uses the same integer
    # reference as FWD and avoids off-by-one WL mismatches.
    if b_rec_init is not None:
        b_rec_frozen['E']=b_rec_init
        print(f"[B_REC SEEDED] E: b_rec={b_rec_init:+.4f} cyc (inherited from FWD)")
    _wl_noisy=set(noisy_wl_init) if noisy_wl_init else set()
    # Sats in wl_fixed (inherited) are already resolved — no need to re-check them
    # as noisy. Noisy seed only prevents fresh WL attempts on known-bad sats.
    _wl_noisy-=set(wl_fixed)
    if _wl_noisy: print(f"[WL_NOISY SEEDED] {sorted(_wl_noisy)} (inherited from FWD)")
    _nl_probation={}   # sid -> epochs_remaining before fix is trusted
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            if sid[0]!='E': continue  # Galileo only
            ...
            # WL fixing
            if sid not in wl_fixed and sid not in _wl_noisy:
                n_hist=len(mw_hist[sid])
                # Early-exit for pathological MW noise (multipath, signal corruption).
                # std > 2.0 cyc at any check point is unambiguously broken — no satellite
                # has a legitimate MW std above ~0.6 cyc; 2 cyc means the observations
                # are corrupted and will never produce a valid WL integer.
                if n_hist>=5:
                    sd_early=np.std(mw_hist[sid]) if n_hist>1 else 999.
                    if sd_early>2.0:
                        _wl_noisy.add(sid); mw_hist[sid].clear()
                        print(f"[WL NOISY EARLY] {sid} excluded: MW std={sd_early:.3f}>2.0 cyc at n={n_hist}")
                if sid not in _wl_noisy and n_hist>=15:
                    mn=np.mean(mw_hist[sid]); sd=np.std(mw_hist[sid])
                    min_n=30 if sd>0.30 else 15
                    if n_hist>=min_n:
                        if 'E' not in b_rec_frozen:
                            all_fracs=[]
                            for s2,h2 in mw_hist.items():
                                if s2[0]!='E': continue
                                if len(h2)<min_n: continue
                                sd2=np.std(h2) if len(h2)>1 else 999.
                                if sd2>0.35: continue   # tightened from 0.45
                            ...
                            if len(all_fracs)>=5:
                                ...
                                if agreement>=max(3,0.6*len(all_fracs)):
                                    b_rec_frozen['E']=b_candidate; b_rec_n['E']=len(all_fracs)
                                    print(f"[B_REC FROZEN] E: b_rec={b_candidate:+.4f} cyc "
                                          f"median of {len(all_fracs)} sats agree={agreement}")
                        if 'E' in b_rec_frozen: b_rec=b_rec_frozen['E']; tag='EF'
                        else:
                            fracs=[]
                            for s2,h2 in mw_hist.items():
                                if s2[0]!='E' or len(h2)<min_n: continue
                                sd2=np.std(h2) if len(h2)>1 else 999.
                                if sd2>0.35: continue   # tightened from 0.45
                                m2=np.mean(h2); fracs.append(m2-round(m2))
                            b_rec=np.mean(fracs) if fracs else 0.0; tag='EE'
                        ...
                        # Flag satellites that are persistently unresolvable after 100 epochs
                        if n_hist>=100 and residual>=0.40:
                            _wl_noisy.add(sid)
                            print(f"[WL NOISY] {sid} excluded: residual={residual:.3f}>=0.40 after n={n_hist} epochs")
                            mw_hist[sid].clear()
                        if 'E' not in b_rec_frozen: pass
                        elif sd<0.35 and residual<0.25:  # tightened from 0.45 to 0.35
            ...
            if slip:
                ...
                _nl_probation.pop(sid,None)

            # Phase wind-up
            wu=_wu(m['sat_xyz'],m['rec_apc'],sun,wum.get(sid,0.)); wum[sid]=wu
            # Phase wind-up on IF combo: δL_IF = α·λ1·δ - β·λ2·δ = δ·λ_NL  (NOT λ_IF)
            LIFc=m['LIF']-wu*LAMBDA_NL; m['LIFc']=LIFc
        ...
        # PDOP-aware satellite selection
        if len(geom)>4:
            pdop=_pdop(geom)
            if pdop>4.0:
        ...
        # NL fixing — require age >= 5 epochs for stability
        _nl_newly=[]   # reset each epoch; populated only when new integers are resolved
        nl_cands=[m for m in geom
                  ...
                  and m.get('age',0)>=5]   # reduced from 10 — Galileo signals converge faster

        # Probation: un-fix sats whose integer is not holding after fixing
        for sid in list(_nl_probation.keys()):
            _nl_probation[sid]-=1
            if _nl_probation[sid]<=0:
                del _nl_probation[sid]  # probation passed — fix trusted
            elif phase_rms_now>NL_PHASE_THRESH*1.5:
                nl_fixed.pop(sid,None); del _nl_probation[sid]
                print(f"  [NL UNFIX] SOD={sod:.0f} {sid} failed probation (PhsRMS={phase_rms_now*1e3:.1f}mm)")
        ...
        if len(nl_cands)>=2:
            ...
            # No permanent blacklist — excluded sats retry each epoch
            ...
            # Allow >=2 clean sats for Galileo-only (low SV count); strict ratio gates 2-sat case
            if len(clean_idx)>=2:
                ...
                # Adaptive ratio: very strict for 2 sats (no redundancy), relaxed for 3+
                n_clean=len(clean_idx)
                if   n_clean==2: ratio_thresh=12.0  # very strict — no geometric redundancy
                elif n_clean==3: ratio_thresh=5.5   # was 4.0; restored toward original 6.0
                elif n_clean==4: ratio_thresh=4.0   # was 3.5
                else:            ratio_thresh=NL_RATIO_THRESH
                if N1_fixed is not None and ratio>=ratio_thresh:
                    newly=[]
                    for i,(m,ob) in enumerate(zip(nl_cands_c,ob_list_c)):
                        if abs(N1_corr_c[i]-int(N1_fixed[i]))<NL_RES_THRESH:
                            ...
                            # Skip probation for integers inherited (seeded) from the other pass:
                            # they've already survived probation in FWD. Fresh ones still serve it.
                            if m['sid'] not in _amb_seeded:
                                _nl_probation[m['sid']]=NL_PROBATION_EPOCHS
                    if newly:
                        _nl_newly=newly  # record freshly fixed sats for this epoch
                        print(f"  [NL FIXED] SOD={sod:.0f} ratio={ratio:.2f} thresh={ratio_thresh:.1f} sats={newly} excl={excl_sids}")

        nproc+=1
        nl_newly_this_epoch=_nl_newly  # sats freshly fixed in THIS epoch (not carried)
        # ZWD clamp: -0.05m floor for filter recovery; 0.60m ceiling for tropical ZWD (Bangalore ~13°N)
        x[4]=max(x[4], -0.05)
        x[4]=min(x[4],  0.60)  # was 0.40 — tropical sites can reach 0.45-0.55 m ZWD
        ...
        results[sod]={'xyz':pos.copy(),'dx':dx.copy(),'p_trace':P[0,0]+P[1,1]+P[2,2],
                      ...
                      'nl_genuine':len(nl_newly_this_epoch),  # sats newly fixed THIS epoch
        ...
        if nproc<=3 or nproc%240==0:
            print(f"  [{label}] SOD={sod:6.0f}  N={len(geom):2d}(E{len(geom)})  3D={d3:8.1f}mm"
    ...
    # Ambiguity inheritance gate: use end-of-pass variance, not init-time convergence.
    # A sat initialised mid-session (high init pt) is still well-determined if its
    # ambiguity variance shrank to < 0.01 m² by end of pass.
    AMB_VAR_THRESH=0.01  # ~0.1m ambiguity std — well-determined
    fwd_amb={sid:(x[ki],P[ki,ki]) for sid,ki in sidx.items() if phi.get(sid,False)}
    fwd_amb_out={sid:v for sid,v in fwd_amb.items() if v[1]<=AMB_VAR_THRESH}
    excluded={sid:f"var={v[1]:.4f}" for sid,v in fwd_amb.items() if v[1]>AMB_VAR_THRESH}
    print(f"[AMB INHERIT] {len(fwd_amb_out)}/{len(fwd_amb)} sats (excluded: {excluded})")
    b_rec_out=b_rec_frozen.get('E',None)
    return results,nom+x[:3],x[3],x[4],wl_fixed,fwd_amb_out,nl_fixed,_amb_snapshots,b_rec_out,_wl_noisy
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("Galileo-only PPP-AR v6 — no nl_init in BWD, PhsRMS-gated DIAG, robust merge")
    print("="*72)

    _,epochs,ah,ak=parse_obs(obs_f)
    sp3t,sp,sc=parse_sp3(sp3_f)
    clkd=parse_clk(clk_f) if clk_f else {}
    noisy_sats=set()  # Galileo clocks are stable — no noisy-sat filter needed
    ...
    fwd,ex,ec,ez,wl_fwd,fwd_amb,nl_fwd,fwd_snapshots,b_rec_fwd,wl_noisy_fwd=_ppp_pass(
    ...
    print(f"  b_rec frozen = {b_rec_fwd:+.4f} cyc" if b_rec_fwd is not None else "  b_rec not frozen in FWD")
    if wl_noisy_fwd: print(f"  WL noisy (excluded): {sorted(wl_noisy_fwd)}")
    print(f"  NL integers at FWD end: {sorted(nl_fwd.keys())} ({len(nl_fwd)} sats)")

    # ── Backward pass: seeds WL + b_rec + float ambiguities + noisy-sat set from FWD ──
    # NOTE: nl_init is intentionally NOT passed to BWD. NL integers are tied to the
    # satellite arc at the time they were fixed — an integer valid at day-end (FWD)
    # does not describe the correct N1 at day-start (BWD start). Seeding them creates
    # a ~450mm systematic Up bias in early BWD epochs (confirmed in v5 data).
    # BWD finds its own integers from the inherited float ambiguity state + WL dict.
    print("\n[PASS 2] Backward (seeded from FWD) ...")
    bwd,bx_end,_,bz_end,wl_bwd,_,_,_,_,_=_ppp_pass(
        epochs,sp3t,sp,sc,clkd,osb,ah,ex.copy(),ec,ez,
        lat0,DOY,zhd,tref,satx,att,recx,
        noisy_sats=noisy_sats,direction=-1,label="BWD",
        wl_init=wl_fwd,amb_init=fwd_amb,b_rec_init=b_rec_fwd,
        noisy_wl_init=wl_noisy_fwd)  # nl_init omitted: wrong-epoch integers hurt BWD
    print(f"  {len(bwd)} epochs  end_3D={np.linalg.norm(bx_end-REF)*1e3:.1f}mm  ZWD={bz_end:.3f}m")
    print(f"  WL fixed in BWD: {list(wl_bwd.keys())}  ({len(wl_bwd)} sats)")

    # ── Merge: quality score = p_trace × (1 + phase_rms/10mm)²
    # This penalises high phase RMS (bad fit) so a BWD epoch with artificially
    # tight p_trace but 200 mm phase RMS cannot beat a FWD epoch with good fit.
    def _qscore(r):
        phs=max(r.get('phase_rms',999.),0.001)
        return r['p_trace']*(1.+(phs/10.0))**2

    merged={}
    for sod in set(fwd)|set(bwd):
        fr=fwd.get(sod); br=bwd.get(sod)
        if   fr is None: merged[sod]={**br,'pass':'BWD'}
        elif br is None: merged[sod]={**fr,'pass':'FWD'}
        else:
            use_fwd=_qscore(fr)<=_qscore(br)
            best=fr if use_fwd else br
            tag='FWD' if use_fwd else 'BWD'
            genuine=max(fr.get('nl_genuine',0), br.get('nl_genuine',0))
            merged[sod]={**best,'pass':tag,'nl_genuine':genuine}

    # ── DIAG: best-60 by quality score, requiring good phase fit ──
    # Only consider epochs where phase RMS < 20mm — this excludes epochs where
    # wrong NL integers are held (they produce low p_trace but wrong position).
    fwd_converged=sorted(
        [(s,r) for s,r in merged.items() if r.get('phase_rms',999.)<20.0],
        key=lambda kv: _qscore(kv[1]))
    ...
    if best60:
        ...
        print(f"\n[DIAG] Best-60 FWD+BWD merged average converged position:")
    ...
    rl=[(s,r) for s,r in sorted(merged.items())]
    lr,lo,_=_lla(REF); Re=_enu(lr,lo); fwd_list=sorted(merged.items())
    print("\n"+"="*72)
    print(f"[RESULT] FWD+BWD merged  ({len(fwd_list)} epochs total)")
    for thresh in [50,100,200]:
        ...
        # nl_active = epochs where >=2 NL integers are held in the filter
        # nl_genuine = epochs where integers were *newly* resolved (not just carried)
        nl_active=sum(1 for _,r in conv if r.get('nl_fixed',0)>=2)
        nl_genuine=sum(1 for _,r in conv if r.get('nl_genuine',0)>=2)
        print(f"  Converged <{thresh:3d}mm: {len(conv):4d} epochs  RMS E/N/U={re:.1f}/{rn:.1f}/{ru:.1f}mm  "
              f"3D={rms3d:.1f}mm  NL-active={nl_active}  NL-genuine={nl_genuine}")
    ...
    total_ep=len(fwd_list)
    # NL-active: >=2 integers held in filter (includes inherited from other pass)
    nl_active_total=sum(1 for _,r in fwd_list if r.get('nl_fixed',0)>=2)
    # NL-genuine: integers were freshly resolved in at least one pass for that epoch
    nl_genuine_total=sum(1 for _,r in fwd_list if r.get('nl_genuine',0)>=2)
    if best60:
        print(f"\n  [ACCURACY SUMMARY]")
        ...
        print(f"  NL-active (filter holds ≥2 ints):  {nl_active_total}/{total_ep} ({100*nl_active_total/total_ep:.1f}%)")
        print(f"  NL-genuine (freshly resolved ≥2):  {nl_genuine_total}/{total_ep} ({100*nl_genuine_total/total_ep:.1f}%)")
    ...
    if outfile and rl:
        ...
        with open(outfile,'w') as fo:
            fo.write("SOD,pass,Computed_X,Computed_Y,Computed_Z,REF_X,REF_Y,REF_Z,"
                     ...
                     "N,WL_fixed,NL_fixed,NL_genuine,ZHD_m,ZWD_m,ZTD_m,CodeRMS_mm,PhsRMS_mm\n")
            for sod,r in rl:
...
if __name__=='__main__':
    ...
    postpos(None,None,0.,0.,PrcOpt(),SolOpt(),FilOpt(),
            INFILES,os.path.join(DATA,'ppp_gal_results.csv'))