...
def _ppp_pass(epochs, sp3t, sp, sc, clkd, osb, ah, nom, iclk, izwd,
    ...
    namb      = 0
    amb_active= {}   # sid → bool: True = has valid state, participates in EKF
    _amb_saved_state = {}   # sid → {x_ki, P_ki, last_sod} for gap recovery
    _amb_birth_sigma = {}   # sid → sigma at first birth (for contraction stats)
    _amb_reset_count = 0    # cumulative resets (slips)
    ...
    # ── AUDIT: per-satellite participation tracking ───────────────────────────
    _audit_accepted_ph   = defaultdict(int)   # total accepted phase rows
    _audit_rejected_ph   = defaultdict(int)   # total rejected phase rows
    _audit_last_accept   = {}                 # sat → epoch of last accepted update
    _audit_birth_epoch   = {}                 # sat → epoch of first birth
    _audit_last_H_norm   = {}                 # sat → last ||H[ki,:]|| seen
    _audit_last_Kg_norm  = {}                 # sat → last Kalman gain norm for ki
    _audit_recent_ph_res = defaultdict(list)  # sat → ring-buffer of recent phase res
    _audit_recent_rej    = defaultdict(list)  # sat → ring-buffer of recent rej flags
    _audit_RING          = 10                 # ring-buffer width for recent stats
    ...
    for epoch in eplist:
        ...
        for sid, so in sorted(sobs.items()):
            ...
            if slip:
                ...
                _recent_res   = _audit_recent_ph_res.get(sid, [])
                _recent_rej   = _audit_recent_rej.get(sid, [])
                ...
                _ep_since_birth = nproc - _audit_birth_epoch.get(sid, nproc)
                ...
                if reset_audit_fh is not None:
                    ...
                    reset_audit_fh.write(
                        ...
                        f"{_amb_reset_count+1}\n"
                    )
                # clear audit ring-buffers for this sat on reset
                _audit_recent_ph_res[sid].clear()
                _audit_recent_rej[sid].clear()
                # ─────────────────────────────────────────────────────────────
                x[ki] = 0.; P[ki, ki] = 20.**2
                amb_active[sid] = False
                ...
                print(f"[AMB-RESET] sat={sid}  SOD={sod:.0f}  reason=cycle_slip  "
                      f"cumulative_resets={_amb_reset_count}")
            ...
            if not amb_active.get(sid, False):
                _saved = _amb_saved_state.get(sid)
                if _saved is not None:
                    _gap = sod - _saved['last_sod']
                    if 0 < _gap < 300.:
                        x[ki]   = _saved['x_ki']
                        P[ki,ki]= min(_saved['P_ki'] * 2.0, (50.)**2)
                        P[ki, :ki] = 0.; P[:ki, ki] = 0.
                        amb_active[sid] = True
            ...
            if not amb_active.get(sid, False):
                ...
                if sid in _amb_init:
                    ...
                    if sid not in _amb_birth_sigma:
                        _amb_birth_sigma[sid] = math.sqrt(P[ki,ki])
                else:
        ...
        if nproc == 0:
            ...
            for m in geom:
                ...
                if m['sid'] in _newborn_pending:
                    ...
                    x[ki] -= dclk
                elif amb_active.get(m['sid'], False):
                    rp0 = _rp(m, x[3], x[4])
                    x[ki] = m['LIFc'] - rp0
                    P[ki, ki] = 20.**2
        ...
        for ri, m in enumerate(geom):
            ...
            if m['sid'] in _newborn_pending or not amb_active.get(m['sid'], False):
                ...
                _NIS = (_nu_mm**2) / max(_S_mm2, 1e-12)
                ...
                _is_nb_aud    = 0
                _ep_birth_aud = nproc - _audit_birth_epoch.get(m['sid'], nproc)
                _phs_hard_mm  = _phs_hard * 1e3
                # ─────────────────────────────────────────────────────────────
                ...
                if not math.isfinite(phase_res):
                    ...
                    # ring-buffer update
                    _audit_rejected_ph[m['sid']] += 1
                    _audit_recent_rej[m['sid']].append(1)
                    if len(_audit_recent_rej[m['sid']]) > _audit_RING:
                        _audit_recent_rej[m['sid']].pop(0)
                    ...
                    if innov_audit_fh is not None:
                        innov_audit_fh.write(
                            ...
                            f"{_fmtf(_S_mm2,prec=4)},{_fmtf(_NIS,prec=6)},"
                            f"{_fmtf(_amb_sig_aud,prec=4)},"
                            f"{_fmtf(_pos_sig_aud,prec=4)},{_fmtf(_clk_sig_aud,prec=4)},"
                            f"{_fmtf(_zwd_sig_aud,prec=4)},{_fmtf(_isb_sig_aud,prec=4)},"
                            f"{_is_nb_aud},{_ep_birth_aud},"
                    ...
                    _audit_accepted_ph[m['sid']] += 1
                    _audit_last_accept[m['sid']] = nproc
                    _audit_recent_rej[m['sid']].append(0)
                    if len(_audit_recent_rej[m['sid']]) > _audit_RING:
                        _audit_recent_rej[m['sid']].pop(0)
                    _audit_last_H_norm[m['sid']] = float(np.linalg.norm(_h_ph))
                    # update recent residual ring
                    _audit_recent_ph_res[m['sid']].append(phase_res * 1e3)
                    if len(_audit_recent_ph_res[m['sid']]) > _audit_RING:
                        _audit_recent_ph_res[m['sid']].pop(0)
        ...
        _P_before_diag = {sid: P[sidx[sid], sidx[sid]]
                          for sid in sidx if amb_active.get(sid, False)}
        ...
        for _esid, _ephi in list(amb_active.items()):
        ...
        if amb_part_fh is not None:
            for _aps, _apki in sidx.items():
                if not amb_active.get(_aps, False):
                    continue
                _ap_alive    = nproc - _audit_birth_epoch.get(_aps, nproc)
                _ap_sig      = math.sqrt(max(P[_apki, _apki], 0.)) * 1e3
                _ap_acc      = _audit_accepted_ph.get(_aps, 0)
                _ap_rej      = _audit_rejected_ph.get(_aps, 0)
                # Did this sat appear in geom this epoch?
                _ap_present  = int(any(m['sid'] == _aps for m in geom))
                _ap_missing  = 1 - _ap_present
                _ap_last_Hn  = _audit_last_H_norm.get(_aps, float('nan'))
                # Participated in EKF update if H[rl,ki] != 0 this epoch
                # (we check via accepted phase counter change this epoch)
                _ap_in_upd   = int(_aps in _newborn_pending is False
                                   and _audit_last_accept.get(_aps, -1) == nproc)
                ...
                _audit_last_Kg_norm[_aps] = _ap_last_Kg
                _last_acc_ep = _audit_last_accept.get(_aps, -1)
        ...
        for _nsid, _nd in sorted(_newborn_pending.items()):
            ...
            amb_active[_nsid] = True

            if _nsid not in _amb_birth_sigma:
                _amb_birth_sigma[_nsid] = 20.0   # 20 m birth

            # AUDIT: record birth epoch
            _audit_birth_epoch[_nsid] = nproc
            ...
            if birth_trace_fh is not None:
                ...
                birth_trace_fh.write(
                    ...
                    f"{_fmtf(_birth_NIS,prec=6)},"
                    f"{_fmtf(_birth_pred_sig_mm,prec=4)}\n"
                )
            # ─────────────────────────────────────────────────────────────────
        for sid_chk in list(nl_fixed.keys()):
            if sid_chk not in sidx or not amb_active.get(sid_chk, False):
                continue
        ...
        #             and amb_active.get(m['sid'], False)
        ...
        if trace_fh is not None:
            ...
            for _tm in geom:
                ...
                if amb_active.get(_tsid, False) and _tsid not in _newborn_pending:
                    _tki   = _tm['ki']
        ...
        # Ambiguity sigma statistics
        _all_sigs = [math.sqrt(max(P[ki2, ki2], 0.))
                     for sid2, ki2 in sidx.items() if amb_active.get(sid2, False)]
        ...
        if state_trace_fh is not None:
            ...
            _gps_amb_sigs3=[math.sqrt(max(P[sidx[s],sidx[s]],0.))*1e3
                            for s in sidx if s[0]=='G' and amb_active.get(s)]
            _gal_amb_sigs3=[math.sqrt(max(P[sidx[s],sidx[s]],0.))*1e3
                            for s in sidx if s[0]=='E' and amb_active.get(s)]
    ...
    print(f"  Total ambiguity resets : {_amb_reset_count}")
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation Comparison (v63-phase14)',
