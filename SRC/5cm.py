...
def try_nl_fix(x, P, amb_indices, LAMBDA_NL, phs_rms):

    global nl_fixed_done

    # stop if already fixed
    if nl_fixed_done:
        return False

    # allow fixing only after convergence
    if phs_rms > 0.02:   # 20 mm threshold
        return False
    ...
    for ki in amb_indices:
        Nf = x[ki] / LAMBDA_NL
        var = P[ki, ki] / (LAMBDA_NL**2)
    ...
    # integer fixing
    N_fixed = np.round(N_float)

    # ratio test
    v = N_float - N_fixed
    s1 = v.T @ np.linalg.inv(Q) @ v

    v2 = N_float - (N_fixed + 1)
    s2 = v2.T @ np.linalg.inv(Q) @ v2

    ratio = s2 / s1 if s1 > 0 else 999

    if ratio > 3:
        print(f"[NL FIXED] {len(N_fixed)} ambiguities ratio={ratio:.2f}")

        for i, ki in enumerate(idx):
            delta = (N_fixed[i] - N_float[i]) * LAMBDA_NL
            x[ki] += delta
            P[ki, ki] = (0.01)**2
...
def _sig(el, s0): return s0 / max(math.sin(el)**2, 0.1)
...
def _lambda_ils(a_float, Q):
    ...
    import itertools
    base = np.round(a_float).astype(int)
    rng = range(-2, 3) if n <= 6 else range(-1, 2)
    best = []
    for deltas in itertools.product(rng, repeat=n):
        cand = base + np.array(deltas)
        r = cand.astype(float) - a_float
        qf = float(r @ Qi @ r)
        best.append((qf, cand.copy()))
    best.sort(key=lambda x: x[0])
    if len(best) < 2 or best[0][0] < 1e-12: return None, 0.0
    ratio = best[1][0] / best[0][0]
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            ...
            m['ki']=ki; m['NWL']=wl_fixed.get(sid,None); geom.append(m)
        ...
        # ==============================
        # NL FIXING CALL (ADD THIS)
        # ==============================

        # collect ambiguity indices (VERY IMPORTANT)
        amb_indices = []

        for i in range(len(x)):
        # ambiguities are large (~meters)
           if abs(x[i]) > 1 and abs(x[i]) < 50:
               amb_indices.append(i)

        # try fixing
        try_nl_fix(x, P, amb_indices, LAMBDA_NL)
        ...
        if len(nl_cands)>=2:
            ...
            b_rec_nl = 0.0
            N1_floats_corr = N1_floats
            if not _nl_diag_done and nproc > 100:
                _nl_diag_done = True
                print(f"  [NL DIAG] SOD={sod:.0f} n_cands={len(nl_cands)} PhsRMS={phase_rms_now*1e3:.2f}mm")
                for i,(m,ob) in enumerate(zip(nl_cands,ob_list)):
                    ki=m['ki']; res=N1_floats[i]-round(N1_floats[i])
                    print(f"    {m['sid']} N1_float={N1_floats[i]:.4f} frac={res:+.4f} NWL={wl_fixed[m['sid']]} "
                          f"x[ki]={x[ki]:.4f}m  P[ki]={P[ki,ki]:.6f}m^2")
                fracs=[N1_floats[i]-round(N1_floats[i]) for i in range(len(nl_cands))]
                print(f"    mean_frac={np.mean(fracs):+.4f}  std_frac={np.std(fracs):.4f}")
            denom=ALFA*LAMBDA1-BETA*LAMBDA2
            idxs=[m['ki'] for m in nl_cands]
            Q_nl=P[np.ix_(idxs,idxs)]/(denom**2)
            N1_fixed,ratio=_lambda_ils(N1_floats_corr, Q_nl)
            if N1_fixed is not None and ratio>=NL_RATIO_THRESH:
                newly=[]
                for i,(m,ob) in enumerate(zip(nl_cands,ob_list)):
                    if abs(N1_floats_corr[i]-int(N1_fixed[i]))<NL_RES_THRESH:
                        nl_fixed[m['sid']]=int(N1_fixed[i]); newly.append(m['sid'])
                if newly:
                    print(f"  [NL FIXED] SOD={sod:.0f} ratio={ratio:.2f} sats={newly}")
    ...
    return results, nom+x[:3], x[3], x[4], wl_fixed, fwd_amb
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print("="*72)
    print("GPS+Galileo PPP v23 — L2C preferred over L2W (SDB fix)")
    print("="*72)
    ...
    fwd,ex,ec,ez,wl_fwd,fwd_amb=_ppp_pass(
    ...
    converged = sorted(fwd.items(), key=lambda kv: kv[1]['p_trace'])
    best60 = converged[:min(60, len(converged))]
    if best60:
        ...
        diff = avg_xyz - REF
        enu_diff = Re @ diff
        print(f"\n[DIAG] Best-60 average converged position:")
        print(f"  XYZ  = [{avg_xyz[0]:.4f}, {avg_xyz[1]:.4f}, {avg_xyz[2]:.4f}]")
        print(f"  dE/N/U from REF = {enu_diff[0]*1e3:+.1f} / {enu_diff[1]*1e3:+.1f} / {enu_diff[2]*1e3:+.1f} mm")
        print(f"  3D   = {np.linalg.norm(diff)*1e3:.1f} mm from REF")

    print("\n[PASS 2] Backward — inheriting WL fixes + ambiguities from FWD ...")
    _conv2 = sorted(fwd.items(), key=lambda kv: kv[1]['p_trace'])
    _best2 = _conv2[:min(30, len(_conv2))]
    bwd_start = np.mean([r['xyz'] for _,r in _best2], axis=0) if _best2 else ex
    print(f"  BWD start pos: 3D={np.linalg.norm(bwd_start-REF)*1e3:.1f}mm from REF")
    print(f"  Inheriting {len(fwd_amb)} FWD ambiguity states into BWD")
    bwd,_,_,_,wl_bwd,_=_ppp_pass(
        epochs,sp3t,sp,sc,clkd,osb,ah,bwd_start.copy(),0.,0.20,
        lat0,DOY,zhd,tref,satx,att,recx,direction=-1,label="BWD",
        wl_init=wl_fwd,amb_init=fwd_amb)
    print(f"  {len(bwd)} epochs  WL in BWD: {len(wl_bwd)} sats")

    all_sods=sorted(set(list(fwd)+list(bwd))); combined={}
    for sod in all_sods:
        fo=fwd.get(sod); bo=bwd.get(sod)
        if bo and bo.get('phase_rms',999)>25.: bo=None
        if   not fo and not bo: continue
        elif not fo: combined[sod]={**bo,'pass':'BWD'}
        elif not bo: combined[sod]={**fo,'pass':'FWD'}
        else: combined[sod]=({**fo,'pass':'FWD'} if fo['p_trace']<=bo['p_trace']
                             else {**bo,'pass':'BWD'})

    if len(fwd)>120:
        fwd_tail=[(s,r) for s,r in sorted(fwd.items()) if s>=sorted(fwd.keys())[-120]]
        da_f=np.array([r['dx'] for _,r in fwd_tail])
        lr,lo,_=_lla(REF); Re=_enu(lr,lo)
        enu_f=(Re@da_f.T).T*1e3
        print(f"[FWD-ONLY] Last 120 epochs  "
              f"RMS E/N/U={math.sqrt(np.mean(enu_f[:,0]**2)):.1f}/"
              f"{math.sqrt(np.mean(enu_f[:,1]**2)):.1f}/"
              f"{math.sqrt(np.mean(enu_f[:,2]**2)):.1f}mm")
    print()

    rl=[(s,combined[s]) for s in sorted(combined)]
    print(f"\n[INFO] {len(rl)} epochs  FWD={sum(1 for _,r in rl if r['pass']=='FWD')}  BWD={sum(1 for _,r in rl if r['pass']=='BWD')}")

    print("\n"+"="*72)
    if len(rl)>60:
        tail=[(s,r) for s,r in rl if s>=rl[-120][0]]
        conv_all=[(s,r) for s,r in rl if np.linalg.norm(r['dx'])*1e3 < 200]
        ...
        if conv_all:
            da2=np.array([r['dx'] for _,r in conv_all])
            r3c=math.sqrt(np.mean(np.sum(da2**2,axis=1)))*1e3
            print(f"  RMS  3D (converged, <200mm, {len(conv_all)} epochs): {r3c:.1f}mm")
    ...
    if outfile and rl:
        with open(outfile,'w') as fo:
            fo.write("SOD,pass,X,Y,Z,dX_mm,dY_mm,dZ_mm,3D_mm,N,WL_fixed,NL_fixed,ZHD_m,ZWD_m,ZTD_m,CodeRMS_mm,PhsRMS_mm\n")
            for sod,r in rl:
                dx=r['dx']*1e3
                fo.write(f"{sod:.1f},{r['pass']},{r['xyz'][0]:.4f},"
                         ...
                         f"{r.get('wl_fixed',0)},{r.get('nl_fixed',0)},"
...
if __name__=='__main__':
    try:
        sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ppp_ar_python'))
        from ppp_ar.structures import PrcOpt, SolOpt, FilOpt
    except:
        class PrcOpt: pass
        class SolOpt: pass
        class FilOpt: pass
