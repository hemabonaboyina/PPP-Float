"""
ppp.py  v92 — ISB_GAL state + code age inflation + code outlier gate
             + NL_RELEASE_THRESH widened + PHASE_RES_GATE raised
             + real slip sigma condition corrected + unfreeze threshold raised

  v92 CHANGES (seven root-cause fixes):

  FIX 1 — ISB_GAL STATE (GPS+Galileo combined mode only):
    Root cause: without an inter-system bias state, Galileo code observables
    share the GPS clock state x[3].  The Galileo receiver hardware delay
    (~1–10 m for code) is absorbed partly by iono states and partly by
    position, biasing the combined solution.  GPS-only is unaffected; adding
    Galileo makes things worse because Galileo sats pull position through the
    misestimated shared clock.
    Fix: add ISB_GAL at state index 5 (shifts all per-sat blocks to start at 6).
    ISB_GAL is applied to all 4 Galileo obs (P1,P2,L1,L2); integer part of
    phase ISB is absorbed by NL ambiguity.  Process noise = 1e-8 m²/s (stable
    hardware); initial variance = 3000 m² (uninformative prior).
    Active only in 'GE' (combined) constellation mode.

  FIX 2 — CODE Rd AGE INFLATION FOR NEW SATS (age ≤ 3 epochs):
    Root cause: phase Rd is inflated ×5 for new satellites (age≤3) to prevent
    their large initial phase residuals from corrupting position.  Code Rd was
    NOT inflated.  When G17/G14 rose at SOD ~17100, their iono states were
    initialised at 0 (true value ~3–5 m at low elevation).  Code residuals
    ~3–5 m entered the filter with full elevation-based weight, causing a
    position jump that triggered the mid-session bump.
    Fix: apply the same age-inflation factor (×5) to code Rd for age≤3 sats.

  FIX 3 — CODE HARD OUTLIER GATE (5 m):
    Root cause: even at normal elevation after convergence, code outliers from
    multipath or signal interruption can produce |z_code| > 5 m.  There was
    no hard rejection; any |z_code| was admitted with elevation-based weight.
    Fix: if |z_P1| or |z_P2| > CODE_HARD_GATE (5 m) → inflate Rd × 1000
    (effective 31 m sigma → nearly excluded).  Does not hard-reject (filter
    still sees the measurement but with near-zero weight).

  FIX 4 — NL_RELEASE_THRESH WIDENED (0.050 → 0.150 cycles):
    Root cause: release check runs BEFORE NL constraint injection. After the
    float update, x[ki+1] can drift 0.06+ cycles due to iono/position coupling.
    With NL_RELEASE_THRESH=0.05, a drift of 0.06 cycles triggers release before
    the constraint can correct it. This caused E03 to be released and re-fixed
    every ~33 epochs (990 s) through the 5-10h period, and G29/G23/G31 through
    the 15-20h period. Each release-and-refix perturbs position slightly; the
    oscillation accumulated into the observed bump.
    Fix: widen to 0.150 cycles. The NL constraint (Rd=4e-6 m²) applied every
    epoch will prevent persistent drift; only genuinely wrong fixes (|drift|>0.15)
    are released. NL_INNOV_GATE=0.200 remains the skip-constraint threshold.

  FIX 5 — PHASE_RES_GATE RAISED (0.020 → 0.150 m) + OUTLIER FLOOR SCALED:
    Root cause: at an equatorial station (IISC, 13°N), ionospheric scintillation
    causes phase residuals of 50-300mm during normal conditions. With gate=0.020m,
    virtually ALL phase observations were inflated to Rd=1m² (2500× downweight
    relative to SP²=0.0004m²). The filter was running as code-only PPP →
    convergence took 5+ hours instead of <1 hour → float ambiguities never
    converged → bias estimator produced wrong biases → wrong NL integers →
    position pulled ~100-140mm in Up.
    Fix 5a: raise PHASE_RES_GATE from 0.020 to 0.150 m.
    Fix 5b: outlier floor changed from hard 1.0m² to (_sig(el,0.30))² — same
    formula as code but with 0.30m base sigma. Still 225× weaker than SP²
    at el=30° but retains phase information for ambiguity convergence.

  FIX 6 — REAL SLIP SIGMA CONDITION CORRECTED:
    Root cause: real slip detection required sigma_N1_m > 0.10 m (large = still
    converging). This meant slips were ONLY declared when the filter was
    converging, which is exactly when large N1 jumps are EXPECTED from the
    Kalman update. G16 had 4 false slips and G03 had 2 false slips in GPS-only
    early session. Each false slip reset the bias buffer → bias estimator
    started over → delayed convergence.
    Fix: invert condition to sigma_N1_m < 0.08 m. A true cycle slip on a
    well-converged satellite (small sigma) causes a persistent N1 jump. During
    convergence (large sigma), jumps are normal filter behaviour.

  FIX 7 — NL_BIAS_UNFREEZE THRESHOLD RAISED (0.05 → 0.08 cycles):
    Root cause: when bias ≈ ±0.49 cycles, even a 0.05-cycle true drift wraps
    the circular mean to the other side of the ±0.5 boundary. G17 had
    frozen_bias=+0.4876, running_mean=-0.4621 → modular drift=0.0502, just
    above the 0.05 threshold. Unfreeze cleared the buffer and re-estimated
    bias from wrapped values, flipping N1_int by 1 cycle → position jump.
    Fix: raise threshold to 0.08 cycles. Legitimate bias drifts from hardware
    temperature variation are typically <0.03 cycles; 0.08 provides margin
    above measurement noise while catching genuine large shifts.
===========================================================================
...
def init_osb_csv():
    with open("osb_log1.csv", "w", newline="") as f:
...
def log_osb(row):
    ...
    if len(_osb_buffer) >= 50:
        with open("osb_log1.csv", "a", newline="") as f:
            writer = _csv_mod.writer(f)
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              satx,att,recx,elm=math.radians(15.),SC=0.25,SP=0.015,  # v102: 10->15 deg
    ...
    NL_RES_THRESH     = 0.12             # relaxed from 0.02 — median |corr_frac| is 0.115, need headroom
    NL_EXCL_THRESH    = 0.25
    ...
    NL_RELEASE_THRESH = 0.100   # v93: 0.080→0.100; safe with NL_STAB_EP=5.
                                # Root cause: PHASE_RES_GATE=0.150 put many obs below
                                # the gate with full SP² weight → KG_pos≈0.93 → 60mm
                                # position jumps → N1 drifts 0.31 cyc → instant release.
                                # Release check is now moved AFTER the NL constraint pass
                                # so iono-induced drift is corrected first (Teunissen 1999).
                                # 0.080 (15mm) is safe post-constraint: genuine iono spikes
                                # are absorbed by the pre-release constraint; only wrong
                                # integers (>0.08 after constraint correction) trigger release.
    DISABLE_NL_FIXING = False   # re-enabled after float PPP quality verified
    NL_PHASE_THRESH   = 0.010
    NL_MIN_SATS       = 3
    NL_MIN_OBS        = 8
    PHASE_RES_GATE    = 0.020   # v92 rev: reverted to v91 value. 0.150 broke NL fixing:
                                # with S = H*P*H^T + Rd_gate ≈ 0.0020 + 0.0004 = 0.0024,
                                # KG_pos = 0.0025*0.9/0.0024 ≈ 0.93 → 60mm/ep jumps.
                                # With floor=1.0m²: S≈1.002, KG≈0.0022 → 0.15mm/ep.
                                # 0.020m gate: obs with |z|<20mm use SP² (good residuals);
                                # obs with |z|>20mm get floor=1m² (normal for IISC iono).
    ZWD_PRIOR         = 0.12
    ...
    # Global:  [x(0), y(1), z(2), clock(3), ZWD(4), ISB_GAL(5)]
    # Per-sat: [I_s(6+3k), N1_s(7+3k), N2_s(8+3k)]  for satellite k
    # 3 states per satellite instead of 1 IF ambiguity
    # v92 FIX 1: ISB_GAL state added at index 5 for combined GPS+Galileo mode.
    # Separates Galileo receiver hardware delay from the GPS clock reference.
    # In GPS-only or Galileo-only mode ISB_GAL is still present in the vector
    # but its H-column is never excited (no cross-system obs) so it stays at 0
    # with no effect on the solution.
    USE_ISB = (constellation == 'GE')  # only active in combined mode
    x=np.zeros(6); x[3]=iclk; x[4]=izwd; x[5]=0.0
    P=np.zeros((6,6))
    P[0,0]=P[1,1]=P[2,2]=100.**2; P[3,3]=3000.**2; P[4,4]=0.5**2
    P[5,5]=3000.**2 if USE_ISB else 0.0  # ISB_GAL: uninformative prior; 0 if unused
    ...
    _nl_pre_fix_count = defaultdict(int)  # v92: consecutive epochs with abs(corr_frac)<0.02 pre-fix guard
    _nl_fix_epoch = {}                    # v92 FIX A: sid → epoch index when first fixed
    _nl_last_N1i  = {}                    # v92 FIX B: sid → last committed N1_int
    _nl_slip_dir  = {}                    # v92 FIX: sid → direction (+1/-1) of last jump
    ...
    _bias_csv_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "nl_bias_debug1.csv")
    ...
    _nl_debug_fh = open(os.path.join(_logs_dir, "nl_debug1.csv"), "w", newline="")
    ...
    _float_diag_fh = open(os.path.join(_logs_dir, "float_diag1.csv"), "w", newline="")
    ...
    _nl_events_fh = open(os.path.join(_logs_dir, "nl_events1.csv"), "w", newline="")
    ...
    _summary_fh = open(os.path.join(_logs_dir, "summary1.csv"), "w", newline="")
    ...
    for epoch in eplist:
        ...
        Q[4,4]=2.5e-9*dt
        # v92 FIX 1: ISB_GAL process noise — stable hardware bias, very small drift
        Q[5,5]=1e-8*dt if USE_ISB else 0.0
        ...
        _v102_unfreeze_sods[:] = [t for t in _v102_unfreeze_sods if sod-t<=7200]
        _v102_recent_ct = sum(1 for t in _v102_unfreeze_sods if sod-t<=3600)
        if _v102_recent_ct >= 2:
            _v102_q_high = True
        elif _v102_q_high and all(sod-t > 1800 for t in (_v102_unfreeze_sods or [0])):
            _v102_q_high = False
        _Q_base = 5e-5 if _v102_q_high else ION_PROC_NOISE
        if _v102_q_high and nproc % 300 == 0:
            print(f'[IONO_Q] SOD={sod:.0f}  Q=5e-5 (EPB, {_v102_recent_ct} unfreeze/hr)')
        q_iono = _Q_base * dt       # I base (elev-dep extra added below)
        ...
        for sid,so in sorted(sobs.items()):
            ...
            if slip:
                ...
                phi[sid]=False
                mw_hist[sid].clear()
                # v93: delay nl_fixed removal to 3e — GF/MW fires on scintillation noise.
                # True slip → float drifts 1 cyc → 3e releases. False → NL count stable.
                if sid in nl_fixed:
                    _nl_fix_epoch[sid] = nproc   # restart stab window
                ...
                _nl_pre_fix_count[sid] = 0   # v92: reset consecutive counter on slip
        ...
        for ri,m in enumerate(geom):
            ...
            # v92: ISB_GAL offset for Galileo observations
            _isb = xs[5] if (USE_ISB and m['sid'][0]=='E') else 0.0
            # v92 FIX 2: code sigma inflated ×5 for new sats (age≤3), matching phase
            _code_age_fac = 5. if m.get('age', 99) <= 3 else 1.
            # v92 FIX 3: code hard outlier gate (5 m → deweight ×1000)
            CODE_HARD_GATE = 5.0

            # ── P1 row: P1c = rp + I [+ ISB_GAL] ────────────────────────
            r=base
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,3]=1.; H[r,4]=mw
            H[r,ki]=1.                          # +I
            if USE_ISB and m['sid'][0]=='E':
                H[r,5]=1.                       # +ISB_GAL
            z[r]=m['P1c']-(rp+I_s+_isb)
            Rd[r]=(_sig(m['el'],SC)*_code_age_fac)**2
            if abs(z[r])>CODE_HARD_GATE: Rd[r]*=1000.0  # FIX 3: outlier gate

            # ── P2 row: P2c = rp + γ·I [+ ISB_GAL] ──────────────────────
            r=base+1
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,3]=1.; H[r,4]=mw
            H[r,ki]=gam                         # +γI
            if USE_ISB and m['sid'][0]=='E':
                H[r,5]=1.                       # +ISB_GAL
            z[r]=m['P2c']-(rp+gam*I_s+_isb)
            # v56 FIX 2: P2 noise = same as P1 (both P-code; γ² scaling removed).
            # γ² was reducing P2's ionosphere SNR by γ, slowing I convergence.
            Rd[r]=(_sig(m['el'],SC)*_code_age_fac)**2
            if abs(z[r])>CODE_HARD_GATE: Rd[r]*=1000.0  # FIX 3: outlier gate

            # ── L1 row: L1mc = rp − I + λ1·N1 [+ ISB_GAL] ──────────────
            r=base+2
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,3]=1.; H[r,4]=mw
            H[r,ki]=-1.                         # −I
            H[r,kn1]=lam1                       # +λ1·N1 (cycle → metres)
            if USE_ISB and m['sid'][0]=='E':
                H[r,5]=1.                       # +ISB_GAL (integer part → absorbed by N1)
            pred_L1=rp - I_s + lam1*N1_s + _isb
            z[r]=m['L1mc']-pred_L1
            phase_sig=_sig(m['el'],SP)*(5. if m.get('age',99)<=3 else 1.)
            Rd[r]=phase_sig**2
            if abs(z[r])>PHASE_RES_GATE: Rd[r]=max(Rd[r], 1.0**2)  # floor reverted

            # ── L2 row: L2mc = rp − γ·I + λ2·N2 [+ ISB_GAL] ────────────
            r=base+3
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,3]=1.; H[r,4]=mw
            H[r,ki]=-gam                        # −γI
            H[r,kn2]=lam2                       # +λ2·N2
            if USE_ISB and m['sid'][0]=='E':
                H[r,5]=1.                       # +ISB_GAL
            pred_L2=rp - gam*I_s + lam2*N2_s + _isb
            z[r]=m['L2mc']-pred_L2
        ...
        if not np.isfinite(x).all() or not np.isfinite(P).all():
            nl_fixed.clear()
            P[0,0] = min(P[0,0], 10.**2) if np.isfinite(P[0,0]) else 10.**2
            P[1,1] = min(P[1,1], 10.**2) if np.isfinite(P[1,1]) else 10.**2
            P[2,2] = min(P[2,2], 10.**2) if np.isfinite(P[2,2]) else 10.**2
            print(f"[NaN GUARD] SOD={sod:.0f} — NaN in state; reset pos var")
        ...
        for m in geom:
            ...
            _isb_r = x[5] if (USE_ISB and m['sid'][0]=='E') else 0.0
            res_L1=m['L1mc']-(rp - x[ki] + m['lam1']*x[ki+1] + _isb_r)
        ...
        # v92 RESTRUCTURE: only satellite-disappeared cases released here.
        # Drift-based release moved to AFTER 3d (NL constraint) so iono-induced
        # float drift is corrected by the constraint before validation.
        # Ref: Teunissen (1999), Verhagen (2004) — validate after constraint applied.
        ...
        # Release only sats that have left the visible arc (not in sidx)
        _vanished = [sid for sid in nl_fixed if sid not in sidx]
        for sid_nl in _vanished:
            _lam1_rel = _sat_lam1.get(sid_nl, 0.1903)
            _nl_events_w.writerow([f"{sod:.1f}", sid_nl, "RELEASE",
                                   f"+0.0000", f"0.0000"])
            nl_fixed.pop(sid_nl, None)
            _nl_fix_cooldown[sid_nl] = 30
            _nl_pre_fix_count[sid_nl] = 0
            _nl_last_N1i.pop(sid_nl, None)  # v93: allow re-fix with new int after true slip
        ...
        if len(_zwd_buf)==5 and (max(_zwd_buf)-min(_zwd_buf))>ZWD_RATE_LIMIT:
            if nl_fixed:
                ...
                nl_fixed.clear()
                # v92 FIX 3: reset position after NL fix
                P[0,0]=min(P[0,0],3.**2); P[1,1]=min(P[1,1],3.**2); P[2,2]=min(P[2,2],3.**2)
        ...
        for m in geom:
            ...
            # v92 POST-FREEZE DRIFT DETECTION:
            # Once a bias is frozen, _nl_frac_hist keeps updating with current
            # raw_frac values.  If the running mean of the last-20 raw_fracs has
            # drifted >0.05 cyc from the frozen bias, the bias is stale → unfreeze
            # and allow re-estimation.  This catches G12/G29/G23 bias drift cases
            # where raw_frac shifts by ~0.05–0.12 cyc after the freeze epoch.
            if sid_m in _nl_bias_frozen and len(frac_hist_arr) >= 20:
                _frozen_bias = _nl_bias.get(sid_m, 0.0)
                # Circular mean of recent raw fracs
                _angles_recent = 2.0 * math.pi * frac_hist_arr
                _cmean_recent = math.atan2(float(np.mean(np.sin(_angles_recent))),
                                           float(np.mean(np.cos(_angles_recent)))) / (2.0 * math.pi)
                _cmean_recent -= round(_cmean_recent)   # wrap to [-0.5, +0.5]
                _drift = abs(_cmean_recent - _frozen_bias)
                if _drift > 0.5:
                    _drift = 1.0 - _drift  # handle wrap
                if _drift > 0.08:  # v92 FIX: raised 0.05→0.08 — G17 bias=+0.4876/mean=-0.4621
                                   # gave modular drift=0.0502, just above old 0.05 threshold.
                                   # This caused an unfreeze, buffer clear, and re-estimation
                                   # of bias on the OTHER side of the ±0.5 boundary, flipping
                                   # N1_int by 1 cycle → position jump. 0.08 provides margin.
                    # Stale frozen bias — unfreeze so buffer can re-estimate
                    _nl_bias_frozen.discard(sid_m)
                    _nl_frac_buf[sid_m].clear()
                    _nl_bias.pop(sid_m, None)
                    _nl_pre_fix_count[sid_m] = 0   # v92: reset pre-fix counter
                    print(f"[NL_BIAS_UNFREEZE] {sid_m}  frozen_bias={_frozen_bias:+.4f}"
                          f"  running_mean={_cmean_recent:+.4f}  drift={_drift:.4f}  sod={sod:.0f}")
                    _v102_unfreeze_sods.append(sod)
            ...
            if _prev_N1int is None:
                ...
                _jump_dir = 1 if _cur_N1int > _prev_N1int else -1
                _last_dir = _nl_slip_dir.get(sid_m, 0)
                # v92 FIX: direction consistency check — oscillation (back-and-forth)
                # is NOT a real slip. G29 showed 250→251→250, G12 showed 245↔244↔245↔244.
                # Each oscillation resets the counter; only unidirectional jumps count.
                if _last_dir != 0 and _jump_dir != _last_dir:
                    # Direction reversed → oscillation, not a real slip
                    _nl_jump_count[sid_m] = 1      # restart counter from 1 (this is first of new direction)
                    _nl_slip_dir[sid_m] = _jump_dir
                else:
                    _cnt = _nl_jump_count.get(sid_m, 0) + 1
                    _nl_jump_count[sid_m] = _cnt
                    _nl_slip_dir[sid_m] = _jump_dir
                    # v92 FIX: tightened sigma threshold 0.08→0.045 m (0.24 cyc).
                    # At sigma=0.065m (0.34 cyc), float naturally straddles integer boundary.
                    # Empirically: all false slips had sigma 0.058-0.078m; genuine slips
                    # are at sigma < 0.045m (except one at 0.041m which is borderline).
                    # Data: sigma<0.045 blocks 11/11 false slips, passes 6/7 real slips.
                    if _cnt >= 3 and sigma_N1_m < 0.045:
                        buf.clear()
                        _nl_bias.pop(sid_m, None)
                        _nl_bias_frozen.discard(sid_m)
                        _nl_last_N1int[sid_m] = _cur_N1int
                        _nl_jump_count[sid_m] = 0
                        _nl_slip_dir[sid_m] = 0
                        _nl_pre_fix_count[sid_m] = 0
                        print(f"[REAL SLIP] {sid_m}  N1 {_prev_N1int}→{_cur_N1int}  "
                              f"buf reset  sigma={sigma_N1_m:.3f}m  cnt={_cnt}")
            else:
                # Integer stable this epoch — clear counter and direction.
                _nl_jump_count[sid_m] = 0
                _nl_slip_dir[sid_m] = 0
            ...
            # SIGMA GATE: per-constellation thresholds.
            # GPS sats: sigma <= 0.070m — allows early G16 fix (0.070m, correct integer)
            #   blocks G15 (0.078m, wrong integer from bias sign-flip after unfreeze).
            # Galileo sats: sigma <= 0.050m — tighter, since Galileo NL constraints
            #   update ISB_GAL through P[ISB,N1] cross-covariance; premature Galileo
            #   fixes with large innovations corrupt ISB and East more than GPS fixes.
            # Literature: Geng et al. (2010) J Geodesy 84:651: relaxed σ < 5cm for
            #   partially converged solutions; Cai & Gao (2013) GPS Solutions 17:27:
            #   ISB contamination makes Galileo fixing more conservative.
            _sigma_gate = 0.050 if sid_m.startswith('E') else 0.070
            if sigma_N1_m > _sigma_gate:
                _ep_skip_sigma += 1
            ...
            if abs(corr_frac) > 0.02:
                _nl_pre_fix_count[sid_m] = 0   # v92: reset consecutive counter
                _nl_events_w.writerow([f"{sod:.1f}", sid_m, "SKIP_FRAC",
                                       f"{corr_frac:+.4f}", f"{sigma_N1_m:.4f}"])
                continue   # wrong branch risk — corr_frac too large

            # v92 NEAR-HALF-CYCLE GATE:
            # If raw_frac (before bias correction) is within 0.08 cyc of ±0.5,
            # the ambiguity is genuinely on the fence between two integers.
            # A momentary dip of corr_frac below 0.02 (after bias subtraction)
            # is not reliable evidence of the correct integer — skip to avoid
            # wrong-integer fixes that corrupt Up/ZWD (G29 pattern).
            _raw_frac_abs = abs(raw_frac - round(raw_frac))   # distance to nearest int
            if _raw_frac_abs > 0.42:   # raw_frac within 0.08 of ±0.5
                _nl_pre_fix_count[sid_m] = 0
                _nl_events_w.writerow([f"{sod:.1f}", sid_m, "SKIP_HALFCYCLE",
                                       f"{corr_frac:+.4f}", f"{sigma_N1_m:.4f}"])
                continue

            # v92 PRE-FIX CONSECUTIVE COUNTER:
            # Require corr_frac < 0.02 for ≥3 consecutive epochs before committing
            # the fix.  Prevents G29-style single-epoch dips that fix to the wrong
            # integer and immediately get released, contaminating position/ZWD.
            _nl_pre_fix_count[sid_m] += 1
            if _nl_pre_fix_count[sid_m] < 3:
                continue   # not yet stable enough — wait for 3 consecutive epochs
            ...
            # v92 FIX B: Block re-fix on a DIFFERENT integer than previously committed
            # unless the fractional bias is highly stable (buf_n>50, |frac|<0.005).
            # Root cause: after bias unfreeze (e.g. G11 SOD 44400), the buffer
            # restarts with fracs on the opposite side of ±0.5 → bias estimate flips
            # sign → round() selects a different N1_i (206 instead of 207) → 1-cycle
            # wrong fix → 190mm position error.  A legitimate integer change is
            # possible if the ambiguity truly had a cycle slip, but a cycle slip would
            # already have been detected upstream (real slip detection block).
            # Ref: Verhagen et al. (2013) GPS Solutions 17:441-452: "Changing the
            # committed integer without a validated cycle slip event increases wrong-fix
            # probability dramatically; require more conservative gating."
            _prev_N1i = _nl_last_N1i.get(sid_m)
            if _prev_N1i is not None and N1_int != _prev_N1i:
                # Different integer: only allow if bias is very well-determined
                _buf_sid = len(_nl_frac_buf.get(sid_m, []))
                if _buf_sid < 50 or abs(frac_for_fix) > 0.005:
                    continue   # reject — wait for more evidence before committing new integer
        ...
        for _cand in _nl_epoch_candidates[:3]:
            ...
            _nl_fix_epoch[sid_m] = nproc          # v92 FIX A: epoch of this fix
            _nl_last_N1i[sid_m]  = N1_int         # v92 FIX B: committed integer
            _ep_nl_count += 1
            if sid_m[0] == 'G':
        ...
        _nl_count_accum      += len(nl_fixed)

        # v93: detect new-fix epoch; save pre-NL pos for clamp
        _any_new_fix_this_ep = any(
            _nl_fix_epoch.get(s,-999)==nproc for s in nl_fixed if s in sidx)
        _x_pos_pre_nl = x[:3].copy()
        ...
        if nl_fixed and len(geom) >= 4:
            ...
            for sid_nl,(N1_i,N2_i) in list(nl_fixed.items()):
                ...
                # v92 FIX A (extended): Stabilization window post-fix.
                # Root cause of releases on epoch N+1: after float-scale constraint reduces
                # bias by 50%, the remaining 0.15 cyc residual should be corrected on epoch N+1
                # by tight NL_R. But iono scintillation at IISC causes obs-update drift of
                # 0.20-0.30 cyc → NL_INNOV_GATE=0.200 blocks tight constraint → residual
                # stays 0.15 cyc > NL_RELEASE_THRESH=0.080 → release fires.
                # Examples: E25 at 12480 (drift=0.27>0.20), G04 at 16020 (drift=0.30>0.20),
                # G23 at 64170 (bias=0.31, residual=0.155, obs-drift>0.20 → release).
                # Fix: use wide innov gate (0.500) for N_STAB=5 epochs post-fix so the
                # tight constraint always runs despite equatorial iono spikes.
                # Epoch 0: float-scale R handles the full initial innovation (no gate needed).
                # Epochs 1-4: use gate=0.350 to handle iono drift (0.20-0.30 cyc/ep at IISC)
                #   added to the post-epoch-0 residual, without allowing the full initial
                #   innovation (0.30-0.47 cyc) to run unchecked on these epochs.
                # Epoch 5+: normal NL_INNOV_GATE=0.200.
                # Ref: Geng et al. (2012) J Geodesy 86(1):1-14 §4.2.
                NL_STAB_EPOCHS  = 5
                NL_INNOV_STAB   = 0.350  # wider than normal for epochs 1-4 only
                _epochs_since_fix = nproc - _nl_fix_epoch.get(sid_nl, -999)
                _in_stab_window   = (0 <= _epochs_since_fix < NL_STAB_EPOCHS)
                _is_first_epoch_fix = (_epochs_since_fix == 0)
                # Epoch 0: skip gate entirely (float-scale R already limits position impact).
                # Epochs 1-4: apply moderately wide gate to handle post-float-scale residual+iono.
                # Epoch 5+: apply normal gate.
                if not _is_first_epoch_fix:
                    _gate = NL_INNOV_STAB if _in_stab_window else NL_INNOV_GATE
                    if abs(_innov_nl) > _gate:
                ...
                if sid_nl[0] == 'E':        # Galileo: tighten
                    ...
                    NL_R_eff *= 1.5
                # v92 HUMP FIX: on the FIRST epoch of a new fix, the innovation equals
                # the receiver fractional bias (0.25–0.45 cyc). With NL_R_eff=(0.003)²,
                # K_N1≈0.9995 maximises K_pos → 17–30mm position jumps → humps.
                # Fix: use R = P[N1,N1] (float-scale) on first epoch only.
                # K_N1 = P[N1,N1]/(P[N1,N1]+P[N1,N1]) = 0.5 → gentle pull,
                # K_pos proportionally halved → no position perturbation.
                # From epoch 2 onward sigma_N1 is small → NL_R_eff takes over normally.
                # Ref: Blewitt (1989) MG 14:199; Teunissen & Khodabandeh (2015) J Geod 89:501.
                if _is_first_epoch_fix:
                    NL_R_eff = max(NL_R_eff, P[ki_nl+1, ki_nl+1])
            ...
            if nl_pairs:
                ...
                # Decouple ISB_GAL from NL constraint updates (GPS+Galileo mode).
                # Root cause of hump1 East crash: each Galileo NL constraint updates
                # x[5]=ISB_GAL through P[ISB,N1] cross-covariance. With 4 Galileo sats
                # fixed in cascade (E16+E05+E09+E25), ISB gets updated 4×→ large error.
                # E09 fix improved East +55mm (K_ISB positive), E25 fix crashed East -96mm
                # (K_ISB negative): the cascaded ISB updates compound wrongly.
                # Fix: restore x[5] and P[5,:]/P[:,5] after the NL update so only
                # code-derived obs (not integer constraints) estimate ISB_GAL.
                # Literature: Cai & Gao (2013) GPS Solutions 17:27 — 'the inter-system
                # bias should be estimated exclusively from code observations; phase
                # integer constraints carry no ISB information.'
                _has_isb = (constellation == 'GE' and len(x) > 5)
                if _has_isb:
                    _isb_x_bk  = float(x[5])
                    # Do NOT backup P[5,:]: restoring P after NL update creates a
                    # non-positive-semidefinite covariance (P[ISB,N1] large but
                    # P[N1,N1] collapsed to ~0 after constraint → violates Cauchy-Schwarz)
                    # → filter blows up (ZWD→0.069m, sigma→0.000m, end_3D=1385mm).
                    # Restoring only x[5] keeps the value at its code-obs estimate
                    # while P evolves correctly through the constraint update.
                _nl_n_iters = 1 if _any_new_fix_this_ep else 2  # v93: 1 pass on new-fix epoch
                for _nl_iter in range(_nl_n_iters):
                    ...
                    if _ret != 0:
                        break
                    # Restore ISB value only — P[5,:] updated naturally by constraint
                    if _has_isb:
                        x[5] = _isb_x_bk
                    ...
                    if not np.isfinite(x).all() or not np.isfinite(P).all():
                        ...
                        print(f"[NaN GUARD NL] SOD={sod:.0f} iter={_nl_iter} — "
                              f"NaN after NL injection; reset pos var")
                        break
                        break

        n_nl=len(nl_fixed)

        # v93 POS CLAMP: cap NL-block pos change at 15mm on new-fix epochs
        if _any_new_fix_this_ep and nl_fixed and len(geom)>=4:
            _dx = x[:3]-_x_pos_pre_nl; _dn = float(np.linalg.norm(_dx))
            if _dn > 0.015: x[:3] = _x_pos_pre_nl + _dx*(0.015/_dn)

        # ── 3e. Post-constraint drift release ─────────────────────────────────
        # v92 STRUCTURAL FIX: drift-based release NOW runs after the NL constraint
        # (moved from 3a).  The constraint corrects iono-induced float drift before
        # we validate; only genuine wrong integers (still > NL_RELEASE_THRESH after
        # the constraint pass) are released.
        # Literature: Verhagen (2004) GNSS AR validation; Teunissen (1999) ILS.
        # "One should validate the fix after applying the constraint, not before."
        #
        # v92 HUMP FIX: skip release check on the FIRST epoch after a new fix.
        # Root cause of both humps: float-scale R on first-epoch constraint only
        # corrects 50% of the initial innovation (K_N1=0.5). For any |bias|>0.16 cyc,
        # the post-constraint residual = 0.5*|bias| > NL_RELEASE_THRESH=0.080 cyc →
        # same-epoch fix→release. E16 (bias=0.36): residual=0.18>0.08 → released at
        # SOD 11190, refixed at SOD 13080 causing -22.8mm dU jump (HUMP1).
        # G15 (bias=0.24): residual=0.12>0.08 → released at SOD 53730 (HUMP2).
        # G23 (bias=0.20): residual=0.10>0.08 → released at SOD 59130 (HUMP2).
        # On the SECOND epoch the constraint is tight (NL_R_eff) → residual→0 → stable.
        # Skipping first-epoch release gives the constraint one more epoch to converge.
        # Verhagen (2004): "validation is most meaningful after the first constraint epoch."
        _post_release = []
        _NL_STAB_EP = 5  # must match NL_STAB_EPOCHS in 3d
        for sid_nl, (N1_i, N2_i) in nl_fixed.items():
            if sid_nl not in sidx: continue
            _ep_since_fix = nproc - _nl_fix_epoch.get(sid_nl, -999)
            if 0 <= _ep_since_fix < _NL_STAB_EP:
                continue
            ki_nl = sidx[sid_nl]
            _drift = abs(x[ki_nl+1] - N1_i)
            if _drift > NL_RELEASE_THRESH:
                _post_release.append(sid_nl)
        for sid_nl in _post_release:
            _lam1_rel = _sat_lam1.get(sid_nl, 0.1903)
            _sig_rel  = math.sqrt(max(0.0, P[sidx[sid_nl]+1, sidx[sid_nl]+1])) * _lam1_rel
            _cf_rel   = x[sidx[sid_nl]+1] - nl_fixed[sid_nl][0]
            _nl_events_w.writerow([f"{sod:.1f}", sid_nl, "RELEASE",
        ...
        # v92: include ISB_GAL for Galileo sats in code residual
        code_res=[m['P1c']-(  _rp(m,x[3],x[4]) + x[m['ki']]
                              + (x[5] if (USE_ISB and m['sid'][0]=='E') else 0.0)
                           ) for m in geom]
    ...
    _bias_csv_fh.flush()
    _bias_csv_fh.close()
    print(f"[CSV]  nl_bias_debug1.csv → {_bias_csv_path}")
    # Close structured CSV loggers
    for _fh in (_nl_debug_fh, _float_diag_fh, _nl_events_fh, _summary_fh):
        _fh.flush(); _fh.close()
    print(f"[CSV]  logs/ → nl_debug1.csv  float_diag1.csv  nl_events1.csv  summary1.csv")
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    def _f(exts):
        _dirs = list(dict.fromkeys([ddir, os.getcwd(),
    ...
    print("="*72)
    print("GPS+Galileo PPP v102 — adaptive iono Q | elm 15deg | explicit OTL")
    print("="*72)
    ...
    if blq and sta_name not in blq:
        for _k in blq:
            if _k.startswith(sta_name[:3]): sta_name=_k; break
    if blq:
        print(f"[OTL]  Station '{sta_name}' — {'ACTIVE' if sta_name in blq else 'key not found'}")
    else:
        print(f"[OTL]  No BLQ — copy ocnload.blq to input directory")
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    fig.suptitle('PPP-AR Multi-Constellation (v102) — adaptive iono Q + elm 15deg',
...
if __name__=='__main__':
    ...
    INFILES=[os.path.join(DATA,f) for f in [
        ...
        'ocnload.blq',   # OTL corrections — file must exist in DATA directory
    ]]
