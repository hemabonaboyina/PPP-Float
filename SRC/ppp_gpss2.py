...
── v75 CHANGES ────────────────────────────────────────────────────────────
  ...
  * Elevation weighting: SC=0.20 m, SP=0.015 m  (v128: SC 0.30→0.20 code strengthening, SP 0.010→0.015 phase relaxation)
  * Bias estimation & freeze logic  (unchanged)
  * corr_frac logic  (unchanged)
  * ION_PROC_NOISE = 3e-5  (v128: 1e-5→3e-5)
...
# ==============================================================================
#  v124 GPS SIGNAL PATH — IISC RINEX DIAGNOSIS (from file inspection)
#
#  RINEX (Septentrio receiver, IISC00IND):
#    PRESENT : C1C, C1W, L1C, C2W, L2W   (L1W is NOT recorded)
#    MISSING : L1W
#
#  OSB product (COD0MGXFIN), clock reference: C1W/C2W:
#    GPS C1C OSBs  — present all PRNs
#    GPS C1W OSBs  — present all PRNs
#    GPS L1C OSBs  — present all PRNs (= L1W values; phase OSB is signal-independent)
#    GPS L1W OSBs  — present all PRNs (identical to L1C)
#
#  Because L1W = 0 in RINEX, the original primary path (C1W+L1W) is NEVER taken.
#  All GPS sats currently fall through to C1C+L1C, with C1C code OSB applied.
#  OSB application is self-consistent (C1C code vs C1C OSB), so there is NO
#  frame mismatch. The 20-30 m L1m-P1c values are NORMAL float PPP behaviour
#  (integer ambiguity + iono, not an OSB bug).
#
#  GPS_DISABLE_FALLBACK = True would be CATASTROPHIC here: it would drop ALL GPS
#  sats (L1W is missing for every satellite). DO NOT set True on this dataset.
#  The flag is retained for potential use with other receivers that do record L1W.
#
#  CORRECT FIX (v124): add an intermediate path — C1W code + L1C phase + C2W/L2W.
#  This places the code in the exact C1W reference frame of the clock product while
#  using the only available L1 phase (L1C). Phase OSB is identical for L1C and L1W.
#  The residual DCB between C1C and C1W after OSB correction is 0.3-0.8 m, which
#  is a genuine improvement for AR and code weighting.
# ==============================================================================
GPS_DISABLE_FALLBACK: bool = False  # v124: MUST be False — L1W absent from IISC RINEX
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    if _P1W != 0. and _L1W != 0. and _P2W != 0. and _L2W != 0.:
        # PRIMARY (strict): C1W code + L1W phase + C2W/L2W — full W-band
        # Clock ref = C1W/C2W: code and phase in exact OSB product frame.
        ...
        # v124 INTERMEDIATE: C1W code + L1C phase + C2W/L2W
        # Applies when receiver records C1W code but only L1C phase (e.g. Septentrio).
        # Code is in C1W frame — exact match for COD0MGXFIN clock reference (C1W/C2W).
        # Phase OSB for L1C == L1W in CODE product (signal-independent) — no penalty.
        # Eliminates the residual ~0.3-0.8 m DCB vs the C1C fallback.
        # AR-eligible: C1W code OSB fully supported.
        code1_type, code2_type, phase1_type, phase2_type = 'C1W', 'C2W', 'L1C', 'L2W'
        P1_val, P2_val, L1_val, L2_val = _P1W, _P2W, _L1C, _L2W
        use_for_ppp = True
        use_for_ar  = True
        # _fallback_used stays False — primary-grade: C1W code frame is correct

    else:
        # GPS_DISABLE_FALLBACK guard: if set, exclude any sat that lacks C1W
        # (retained for use with receivers that provide L1W but not C1W).
        if GPS_DISABLE_FALLBACK:
            return None  # strict mode — no C1W present, skip satellite

        # FALLBACK (last resort): C1C code + L1C phase + C2W/L2W
        # Only reached when C1W is entirely absent from the RINEX.
        # v83: AR eligibility determined by OSB check below, not signal label.
        if _P1C != 0. and _L1C != 0. and _P2W != 0. and _L2W != 0.:
            ...
            # STRICT: no raw fallback — only OSB-consistent signals allowed
            return None  # skip satellite — no C1W/L1W/C2W/L2W or C1C/L1C+C2W/L2W
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              satx,att,recx,elm=math.radians(15.),SC=0.20,SP=0.015,   # v128: SC→0.20 (strengthen code weight), SP→0.015 (phase weight relaxation; break iono-ambiguity coupling)
    ...
    ZWD_CLAMP         = 0.015
    GF_CODE_SCALE     = 0.4   # v127: 0.6→0.4 (CRITICAL — GF must compete with phase so iono does not float freely)
    ...
    # [CODE_REJECT] 3σ code prefit gate counters
    ...
    _iono_dI_adapt    = defaultdict(float)  # sid → |dI_meas| used for Q scaling
    ...
    for epoch in eplist:
        ...
        ION_PROC_NOISE = 3e-5 * ion_q_scale         # v128 PART3: base 5e-4×2.0 → 3e-5 (reduce iono dynamics; tighter iono state tracks code innovation)
        if ep_idx == 0:
            print(f"[IONO_Q_UPDATE] base={3e-5*ion_q_scale:.2e}  (v128: 5e-4x2→3e-5)")
        ...
        for sid_k,ki in sidx.items():
            ...
            _scale_k = min(1.0 + 5.0 * _dI_k, 20.0)
        ...
        if nproc % 300 == 0:
            ...
            for _m_sig in geom:
                ...
                print(f"[SIG] SOD={sod:.0f} sat={_sid_sig} "
                      f"code={_c1_sig}/{_c2_sig} phase={_p1_sig}/{_p2_sig} "
                      f"fallback={_fb_sig} ar_eligible={_ar_sig} "
        ...
        for ri,m in enumerate(geom):
            ...
            # ── P2 row: P2c = rp + γ·I + ISB(Gal) + grad ────────────────
            r=base+1
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,IDX_CLK]=1.; H[r,IDX_ZWD]=mw
            H[r,ki]=gam
            H[r,IDX_ISB_GAL] = 1. if is_gal else 0.
            H[r,IDX_Gn]=_h_gn; H[r,IDX_Ge]=_h_ge
            z[r]=m['P2c']-(rp+gam*I_s+isb_s+grad_s)
            # C2 sigma: GPS uses C2W (semi-codeless) → 1.0 m; Galileo C5Q uses SC
            _sigma_c2 = 1.0 if (not is_gal) else SC  # C2W semi-codeless noise
            ...
            _ph_rd = float(np.clip(_ph_rd_raw, 0.005**2, 0.1**2))   # v126: upper cap 0.05²→0.1² to allow SP=0.06 elevation-dependent weighting
        ...
        # ── [CODE_REJECT] 3σ code prefit gate ────────────────────────────────
        ...
        for _cr_ri, _cr_m in enumerate(geom):
            ...
            for _cr_off, _cr_label in ((0, 'P1'), (1, 'P2')):
                ...
                if _cr_sigma > 0.0 and abs(_cr_res) > 3.0 * _cr_sigma:
                    ...
                    print(f"[CODE_REJECT] SOD={sod:.0f} sat={_cr_sid} "
                          ...
                          f"ratio={abs(_cr_res)/_cr_sigma:.1f}σ")
                elif _cr_sigma > 0.0:          # v122 PART3: accumulate accepted code ratio
                    _cr_ratio_sum += abs(_cr_res) / _cr_sigma
                    _cr_ratio_cnt += 1
        ...
        if nproc > 0 and nproc % 300 == 0:
            ...
            # v122 PART3: [CODE_USAGE] — checks if code is silently ignored
        ...
        n_total=n_obs
        # ── GF pseudo-measurement rows — built BEFORE filter_standard (Part 1 fix) ──
        # Observation:  z_GF = P2c − P1c  (geometry-free code combination)
        # Model:        z_GF = (γ−1)·I_s  →  H[ki] = (γ−1), all other cols = 0
        # Noise:        R_GF = GF_CODE_SCALE·(Rd_P1+Rd_P2)  (R_GF = 3×sum, tightened from 9×)
        # Pre-update prediction uses xs[ki] — the state snapshot BEFORE this epoch's update.
        # GF rows are stacked into the same measurement system as main obs (single filter call).
        _gf_H_rows = []
        _gf_z_rows = []
        _gf_R_diag = []
        ...
        for _gf_ri, _gf_m in enumerate(geom):
            ...
            _gf_sig2  = min(_gf_sig2, 1.0**2)   # v127: cap 1.2²→1.0² (tighten GF constraint to break iono–ambiguity degeneracy)
        ...
        # Combine: main obs (n_obs rows) + ZWD prior (1 row) + GF rows (n_gf rows)
        _n_extra = 1 + _gf_used_count          # ZWD prior + GF
        ...
        # GF rows appended after ZWD prior
        ...
        # ── ADAPTIVE IONO Q: update per-sat dI_meas for next epoch's Q ───────
        for _sid_ai, _I_now_ai in _I_meas_ep.items():
            _I_prev_ai = _iono_prev_I_meas.get(_sid_ai)
            if _I_prev_ai is not None:
                _iono_dI_adapt[_sid_ai] = abs(_I_now_ai - _I_prev_ai)
            _iono_prev_I_meas[_sid_ai] = _I_now_ai
        # ── end ADAPTIVE IONO Q tracking ──────────────────────────────────────
        ...
        # ── v124 DIAG: [FRAME_CHECK] per-sat sliding-window L1m-P1c every 300 ep ─
        # Validates that code and phase observables are in the same reference frame
        # after OSB application.  Expected values (IF-frame clocks + OSB applied):
        #   mean  : within ±5–10 m  (residual = geometric range + clock + iono, ~20–30 km)
        #   std   : <15 m typical under equatorial conditions; flag if >30 m
        #   drift : <5 m/window acceptable; flag if |drift| > 10 m/window
        # Large mean or growing drift → code and phase NOT in the same frame.
        if nproc % 300 == 0 and _diag_lp1_sat:
            print(f"[FRAME_CHECK] SOD={sod:.0f} — sliding-window L1m−P1c per sat "
                  f"(last 300 ep, frame consistency; GPS_DISABLE_FALLBACK={GPS_DISABLE_FALLBACK}):")
            _fc_gps_ok = 0; _fc_gps_bad = 0; _fc_gal_ok = 0; _fc_gal_bad = 0
            for _sid_fc in sorted(_diag_lp1_sat.keys()):
                _hist_fc = _diag_lp1_sat[_sid_fc]
                _win_fc  = _hist_fc[-300:]   # last 300 epochs only (sliding window)
                if len(_win_fc) < 10:
                    continue
                _arr_fc  = np.array(_win_fc)
                _mn_fc   = float(np.mean(_arr_fc))
                _sd_fc   = float(np.std(_arr_fc))
                _drift_fc = float(_arr_fc[-1] - _arr_fc[0])
                _sys_fc  = 'GPS' if _sid_fc[0] == 'G' else 'GAL'
                # Flag criteria: std > 20 m OR |drift| > 15 m in window
                _bad_fc  = (_sd_fc > 20.0 or abs(_drift_fc) > 15.0)
                _flag_fc = ' *** FRAME_MISMATCH' if _bad_fc else ''
                # Get signal type used for this satellite
                _sigmap_fc = _sat_signal_map.get(_sid_fc, ('?', '?', '?', '?'))
                _sig_fc    = f"{_sigmap_fc[0]}/{_sigmap_fc[2]}"
                print(f"  [FRAME_CHECK] sat={_sid_fc} sys={_sys_fc} sig={_sig_fc} "
                      f"n={len(_win_fc)} mean={_mn_fc:+.2f}m std={_sd_fc:.2f}m "
                      f"drift={_drift_fc:+.2f}m{_flag_fc}")
                if _sys_fc == 'GPS':
                    _fc_gps_bad += int(_bad_fc); _fc_gps_ok += int(not _bad_fc)
                else:
                    _fc_gal_bad += int(_bad_fc); _fc_gal_ok += int(not _bad_fc)
            print(f"  [FRAME_CHECK_SUMMARY] GPS ok={_fc_gps_ok} bad={_fc_gps_bad}  "
                  f"GAL ok={_fc_gal_ok} bad={_fc_gal_bad}")
        # ── end FRAME_CHECK ──────────────────────────────────────────────────
        ...
        if ep_idx % 300 == 0:
            ...
            # v124 PART3: [IONO_BALANCE_CHECK] compact summary every 300 epochs
            _ibc_R_vals = [t[3] for t in _iono_resp if math.isfinite(t[3])]
            _ibc_mean_R = float(np.mean(_ibc_R_vals)) if _ibc_R_vals else float('nan')
            print(f"[IONO_BALANCE_CHECK] SOD={sod:.0f}  "
                  f"code_pct={_c_pct:.2f}%  "
                  f"ratio_phase_to_GF={_ratio_gf_ph:.1f}  "
                  f"corr(I,N1)={_mean_corr_IN1:+.3f}  "
                  f"mean_R={_ibc_mean_R:.3f}")

            # v126: [BALANCE_CHECK] mandatory diagnostic — code/phase split + iono dynamics
            _bc_gf_sig = (math.sqrt(sum(_gf_R_diag) / len(_gf_R_diag))
                          if _gf_R_diag else float('nan'))
            print(f"[BALANCE_CHECK] SOD={sod:.0f}  "
                  f"code_pct={_c_pct:.2f}%  "
                  f"phase_pct={_p_pct:.2f}%  "
                  f"mean_GF_sigma={_bc_gf_sig:.3f}m  "
                  f"corr(I,N1)={_mean_corr_IN1:+.3f}  "
                  f"mean_dI_state={_mean_dI_st:.4f}m  "
                  f"mean_dI_code={_mean_dI_co:.4f}m")

            # v128 PART 4 — HARD safety monitor every 300 epochs (MANDATORY)
            # TARGET: code_pct → 4–6%  |  corr(I,N1) → <0.4  |  GF_sigma → <1.0m
            _ic_corr_up_clk = float("nan")
            if len(_corr_clk_buf) >= 30:
                _ic_ua = np.array(_corr_up_buf[-300:] if len(_corr_up_buf) > 300 else _corr_up_buf)
                _ic_ca = np.array(_corr_clk_buf[-300:] if len(_corr_clk_buf) > 300 else _corr_clk_buf)
                _ic_da = _ic_ua - _ic_ua.mean(); _ic_db = _ic_ca - _ic_ca.mean()
                _ic_den = float(np.std(_ic_ua) * np.std(_ic_ca))
                if _ic_den > 1e-12:
                    _ic_corr_up_clk = float(np.dot(_ic_da, _ic_db) / (len(_ic_ua) * _ic_den))
            print(f"[IONO_CONTROL] SOD={sod:.0f}  "
                  f"code_pct={_c_pct:.2f}%  "
                  f"phase_pct={_p_pct:.2f}%  "
                  f"corr(I,N1)={_mean_corr_IN1:+.3f}  "
                  f"corr(Up,CLK)={_ic_corr_up_clk:+.3f}  "
                  f"mean_GF_sigma={_bc_gf_sig:.3f}m  "
                  f"mean_code_sigma={_mean_code_sig:.3f}m  "
                  f"mean_phase_sigma={_mean_phase_sig:.4f}m")
    ...
    print(f"\n{'='*68}")
    print(f"[IONO_DIAG] label={_iono_diag_label}  ION_PROC_NOISE≈3e-5*scale m²/s")
    print(f"{'='*68}")
    ...
    try:
        ...
        if _lim_rejection:
            _dom_lim = "rejection"
            _rec_act = "investigate CODE_REJECT patterns; consider loosening 3σ gate or checking OSB quality"
    ...
    # ── v125 OUTPUT: [FINAL_CHECK] ───────────────────────────────────────────
    # Confirm code_pct, corr(I,N1), corr(Up,CLK) and mean Up error numerically.
    try:
        _fc_code_pct = (100.0 * _ep_code_info_acc /
                        (_ep_code_info_acc + _ep_phase_info_acc + 1e-30))
        _fc_corr_IN1 = float('nan')
        _fc_corr_IN1_vals = []
        for _fcs in sidx.values():
            _fcn1 = _fcs + 1
            if _fcn1 < len(P):
                _fp2 = max(P[_fcs, _fcs], 1e-20)
                _fn2 = max(P[_fcn1, _fcn1], 1e-20)
                _fc_corr_IN1_vals.append(P[_fcs, _fcn1] / math.sqrt(_fp2 * _fn2))
        if _fc_corr_IN1_vals:
            _fc_corr_IN1 = float(np.mean(_fc_corr_IN1_vals))
        # corr(Up,CLK) from rolling buffer
        _fc_corr_up_clk = float('nan')
        if len(_corr_up_buf) >= 30:
            _fc_ua = np.array(_corr_up_buf)
            _fc_ca = np.array(_corr_clk_buf)
            _fc_corr_up_clk = float(_corr(_fc_ua, _fc_ca))
        # mean Up error (all epochs)
        _fc_mean_up = float(np.mean(_up_err_hist)) if _up_err_hist else float('nan')
        _fc_up_rms  = float(np.sqrt(np.mean(np.array(_up_err_hist)**2))) if _up_err_hist else float('nan')
        # improvement flags
        _ok_pct  = '✓' if _fc_code_pct  >= 2.0  else '✗'
        _ok_corr = '✓' if abs(_fc_corr_IN1) < 0.5 else '✗'
        _ok_clk  = '✓' if abs(_fc_corr_up_clk) < 0.7 else '✗'
        print(f"\n{'='*68}")
        print(f"[FINAL_CHECK]  (v128: SP=0.015  SC=0.20  ION_PROC_NOISE=3e-5  GF_CODE_SCALE=0.4  GF_cap=1.0m  phase_clip=0.1m)")
        print(f"  code_pct       = {_fc_code_pct:.2f}%    {_ok_pct}  (target ≥2%)")
        print(f"  corr(I,N1)     = {_fc_corr_IN1:+.3f}   {_ok_corr}  (target |corr|<0.5)")
        print(f"  corr(Up,CLK)   = {_fc_corr_up_clk:+.3f}   {_ok_clk}  (target |corr|<0.7)")
        print(f"  mean_Up_err    = {_fc_mean_up:+.1f} mm")
        print(f"  Up_RMS         = {_fc_up_rms:.1f} mm")
        print(f"{'='*68}\n")
    except Exception as _fce:
        print(f"[FINAL_CHECK] (skipped: {_fce})")
    # ── end FINAL_CHECK ──────────────────────────────────────────────────────
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    print(f"\n{'='*72}")
    print("[v124] GPS+Galileo PPP — C1W+L1C intermediate path + FRAME_CHECK diagnostics (Septentrio-aware)")
    print(f"{'='*72}")

    print("\n[OSB_AUDIT]")
    print("  mode=DYNAMIC_PER_SAT_PER_SIGNAL")
    print("  where_applied=_proc/_proc_gal BEFORE measurement model (P1c/P2c/L1m/L2m)")
    print("  signals_used=GPS:C1W/L1C+C2W/L2W (intermediate, Septentrio) or C1W/L1W+C2W/L2W (strict) or C1C/L1C (fallback)  GAL:C1C/L1C+C5Q/L5Q")
    ...
    # ── Results summary ────────────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print("[v124] RESULTS SUMMARY  (pure float, v124: C1W+L1C intermediate path — C1W code in clock-product frame)")
    print(f"{'='*72}")
    ...
    try:
        ...
        fig_v.suptitle(
            'PPP v123 — ZWD Q×1.5 restored + Prior σ=8cm + Height Gate=3h/20cm  (pure float)',
            fontsize=11, fontweight='bold')
        ...
        fig_a.suptitle('PPP v123 — Full Analysis  (pure float)',
        ...
        print(f"[v124] Analysis plot saved: {_ap}")
        plt.close(fig_a)

    except Exception as _ve:
        print(f"[v106] Plot failed: {_ve}")

    print(f"\n[v124] Wall time: {_time.time() - t0:.1f}s")
    print("="*72)
