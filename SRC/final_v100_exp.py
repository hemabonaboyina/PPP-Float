...
# ==============================================================================
#  v101 RUNTIME / LOGGING OPTIMIZATION — PATCH 1: Global debug switches
#  ZERO KF / stochastic / estimator changes.  Pure IO / print throttling.
# ==============================================================================
FORENSIC_MODE       = True    # False → skip all heavy forensic CSV creation & writes

DEBUG_GF_READY      = False   # True  → print [GF_READY] periodic lines
DEBUG_REENTRY       = False   # True  → print [REENTRY_GUARD] per-epoch per-sat lines
DEBUG_WL_EVENTS     = True    # already event-driven; keep True
DEBUG_GAL_APC       = False   # True  → print [GAL_APC_PARSE]/[GAL_APC_ACTIVE] per sat
DEBUG_RMS_SPIKE     = True    # already low-frequency; keep True
DEBUG_ARC_RESET     = True    # already event-driven; keep True

DEBUG_SUMMARY_ONLY  = True    # compact hourly counters instead of per-epoch spam

LOG_PERIOD_EPOCHS   = 300     # minimum epochs between generic periodic prints
CSV_FLUSH_PERIOD    = 1000    # rows accumulated before a buffered CSV flush

# Runtime counters — reset per pass; printed as hourly [SUMMARY] block
_stats: dict = {}
# ==============================================================================
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              ...
              # ── STOCHASTIC EXPERIMENT KNOBS (v100-EXP) ──────────────────
              # Multipliers applied ONLY to Galileo code / phase sigma.
              # GPS weighting, process noise, KF equations — all untouched.
              gal_code_scale=1.0,   # Runs B(×2) C(×4) E(adaptive)
              gal_phase_scale=1.0,  # Run D(×1.5)  E(adaptive)
              adaptive_gal=False,   # Run E: override with rolling-RMS scales
              ):
    """
    ...
    # ── v100-EXP: rolling GAL residual buffers for adaptive weighting (Run E) ─
    # Filled from the per-epoch GAL_P2 / L2 post-fit residuals computed in the
    # RMS-SPLIT DIAGNOSTICS block.  Used NEXT epoch to set _adaptive_code_sf /
    # _adaptive_phase_sf — intentionally one-epoch lag (causal, no look-ahead).
    _gal_code_rms_buf  = deque(maxlen=60)   # rolling 60-epoch code RMS  [metres]
    _gal_phase_rms_buf = deque(maxlen=60)   # rolling 60-epoch phase RMS [metres]
    _adaptive_code_sf  = gal_code_scale     # effective scale this epoch
    _adaptive_phase_sf = gal_phase_scale    # effective scale this epoch
    ...
    # v101 PATCH 3: write buffer for rms_split CSV (once per epoch → ~11k rows/4 runs)
    _rms_csv_buf: list = []

    # v101 PATCH 5: reset per-pass stats counters
    _stats.clear()
    ...
    # ── v100: STATE-BASED GF READINESS GATE ──────────────────────────────────
    # Replaces the time-only arc_age < 3 bypass with a physically validated
    # readiness criterion.  The GF slip detector activates ONLY when:
    #   1. arc_age > 2           (minimum 3 epochs of filter history)
    #   2. sigma_I < TAU_GF     (filter has converged on iono state)
    #   3. |I_state - I_code| < EPS_BIAS  (state consistent with code-GF)
    # While NOT ready: GF/MW detector is fully suppressed.
    # While READY: existing detector runs UNCHANGED.
    TAU_GF   = 0.5   # m — iono sigma threshold for readiness
    EPS_BIAS = 1.0   # m — max tolerated state-vs-code-GF discrepancy
    _gf_ready_first   = {}   # sid -> first SOD at which GF became ready
    _gf_ready_epoch_count = {}  # sid -> epoch counter since readiness (for periodic print)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # v101 PATCH 3: write buffer for c2w audit (per-sat per-epoch → ~90k rows/pass)
    _c2w_buf: list = []
    # ─────────────────────────────────────────────────────────────────────────
    ...
    for epoch in eplist:
        ...
        for sid,so in sorted(sobs.items()):
            ...
            if _disc_is_reentry:
                ...
                _stats['arc_resets'] = _stats.get('arc_resets', 0) + 1
            ...
            # ── v100: STATE-BASED GF READINESS GATE ──────────────────────────
            # Replaces the time-only arc_age < 3 bypass.
            # GF slip testing is permitted ONLY when the ionosphere state is
            # measurement-constrained AND consistent with code-GF observables.
            # While NOT ready: detector is fully suppressed (no dGF update,
            # no slip trigger).  MW accumulation and ambiguity state preserved.
            _p3_arc_age = _sat_age.get(sid, 0)
            _p3_ki      = sidx.get(sid)          # None if state not yet allocated
            # Compute readiness components (safe: all guarded against None/NaN)
            if _p3_ki is not None:
                _p3_sigma_I = math.sqrt(max(P[_p3_ki, _p3_ki], 0.0))
                _p3_I_state = x[_p3_ki]
            else:
                _p3_sigma_I = 999.0              # state not yet allocated → not ready
                _p3_I_state = 0.0
            _p3_gam = m.get('gamma', 2.0)
            if abs(_p3_gam - 1.0) > 1e-6 and 'P2c' in m and 'P1c' in m:
                _p3_I_code = (m['P2c'] - m['P1c']) / (_p3_gam - 1.0)
            else:
                _p3_I_code = _p3_I_state         # fallback: treat as consistent
            _p3_bias   = abs(_p3_I_state - _p3_I_code)
            # GF ready iff all three physical conditions met
            _p3_gf_ready = (
                _p3_arc_age > 2
                and _p3_sigma_I < TAU_GF
                and _p3_bias    < EPS_BIAS
            )
            # Determine reason for NOT ready (for diagnostic)
            if not _p3_gf_ready:
                if _p3_arc_age <= 2:
                    _p3_reason = "Age"
                elif _p3_sigma_I >= TAU_GF:
                    _p3_reason = "SIGMA"
                else:
                    _p3_reason = "Bias"
            else:
                _p3_reason = "READY"
            # ── GF_READY diagnostic: print at first transition and every 300 ep
            _p3_was_ready = sid in _gf_ready_first
            if _p3_gf_ready and not _p3_was_ready:
                _gf_ready_first[sid] = sod
                _gf_ready_epoch_count[sid] = 0
                _stats['gf_ready_yes'] = _stats.get('gf_ready_yes', 0) + 1
                # First-time transition → always print (state change)
                print(f"[GF_READY] SOD={sod:.0f} sat={sid} arc_age={_p3_arc_age}"
                      f" sigma_I={_p3_sigma_I:.4f}m I_state={_p3_I_state:.4f}m"
                      f" I_code={_p3_I_code:.4f}m bias={_p3_bias:.4f}m"
                      f" ready=YES reason=READY")
            elif not _p3_gf_ready and _p3_was_ready:
                # State regressed (e.g. after reset) — clear readiness record
                _gf_ready_first.pop(sid, None)
                _gf_ready_epoch_count.pop(sid, None)
                _stats['gf_ready_no_bias'] = _stats.get('gf_ready_no_bias', 0) + 1
                # Transition back to NOT ready → always print
                print(f"[GF_READY] SOD={sod:.0f} sat={sid} arc_age={_p3_arc_age}"
                      f" sigma_I={_p3_sigma_I:.4f}m I_state={_p3_I_state:.4f}m"
                      f" I_code={_p3_I_code:.4f}m bias={_p3_bias:.4f}m"
                      f" ready=NO reason={_p3_reason}")
            else:
                # Periodic print — only when DEBUG_GF_READY is enabled
                _p3_ep = _gf_ready_epoch_count.get(sid, 0) + 1
                _gf_ready_epoch_count[sid] = _p3_ep
                if DEBUG_GF_READY and _p3_ep % LOG_PERIOD_EPOCHS == 0:
                    print(f"[GF_READY] SOD={sod:.0f} sat={sid} arc_age={_p3_arc_age}"
                          f" sigma_I={_p3_sigma_I:.4f}m I_state={_p3_I_state:.4f}m"
                          f" I_code={_p3_I_code:.4f}m bias={_p3_bias:.4f}m"
                          f" ready={'YES' if _p3_gf_ready else 'NO'} reason={_p3_reason}")
            # NOT ready: optionally log REENTRY_GUARD (DEBUG_REENTRY=False silences spam)
            _p3_slip_bypass = not _p3_gf_ready
            if _p3_slip_bypass and sid in prev_mw and prev_mw[sid] is not None:
                _stats['reentry_guard'] = _stats.get('reentry_guard', 0) + 1
                if DEBUG_REENTRY:
            ...
            # ── end v100 state-based readiness gate ───────────────────────────
        ...
        for ri,m in enumerate(geom):
            ...
            # v100-EXP: Galileo code sigma scaled by experiment knob (GPS unchanged)
            _p1_code_sf = (_adaptive_code_sf if not _is_gps_obs else 1.0)
            _r = (_sig(m['el'],SC) * _p1_code_sf)**2
            ...
            if not _omit_p2l2:
                ...
                # Galileo P2 weight controlled by experiment knob (gal_code_scale).
                _sig_P2_base = _sig(m['el'],SC)
                if _is_gps_obs:
                    _sig_P2_after = _sig_P2_base * _C2W_DEWEIGHT_CODE_SCALE
                    _r = _sig_P2_after ** 2
                else:
                    # v100-EXP: Galileo P2 sigma scaled by adaptive/static knob
                    _sig_P2_after = _sig_P2_base * _adaptive_code_sf
                    _r = _sig_P2_after ** 2
            ...
            # v100-EXP: Galileo L1 phase sigma scaled by experiment knob
            if not _is_gps_obs:
                _phase_sig *= _adaptive_phase_sf
            _r = _phase_sig**2
            ...
            if not _omit_p2l2:
                ...
                # Galileo L2 weight controlled by experiment knob (gal_phase_scale).
                _sig_L2_base = _sig(m['el'],SP)*(5. if m.get('age',99)<=3 else 1.)
                if _is_gps_obs:
                    _sig_L2_after = _sig_L2_base * _C2W_DEWEIGHT_PHASE_SCALE
                    _r = _sig_L2_after ** 2
                else:
                    # v100-EXP: Galileo L2 sigma scaled by adaptive/static knob
                    _sig_L2_after = _sig_L2_base * _adaptive_phase_sf
                    _r = _sig_L2_after ** 2
                if abs(_z)>PHASE_RES_GATE: _r=max(_r, 1.0**2)
                ...
                if _is_gps_obs and _sig_P2_base is not None:
                    ...
                    if len(_c2w_buf) >= CSV_FLUSH_PERIOD:
                        _c2w_audit_w.writerows(_c2w_buf)
                        _c2w_buf.clear()
        ...
        if nproc % 300 == 0 and sidx:
            ...
            # v101 PATCH 5: compact hourly [SUMMARY] block (replaces per-epoch per-sat prints)
            if DEBUG_SUMMARY_ONLY and nproc % 3600 == 0:
                print(f"[SUMMARY] SOD={sod:.0f}"
                      f"  GF_READY_YES={_stats.get('gf_ready_yes',0)}"
                      f"  GF_READY_NO_BIAS={_stats.get('gf_ready_no_bias',0)}"
                      f"  REENTRY_BYPASSED={_stats.get('reentry_guard',0)}"
                      f"  ARC_RESETS={_stats.get('arc_resets',0)}"
            ...
            # v101: only print per-sat signal detail when DEBUG_REENTRY is on
            # (the per-sat lines are high-volume at IISC with 15+ visible satellites)
            if DEBUG_REENTRY:
        ...
        # ── Write CSV row (buffered — flush every CSV_FLUSH_PERIOD rows) ────
        ...
        if len(_rms_csv_buf) >= CSV_FLUSH_PERIOD:
            _rms_csv_w.writerows(_rms_csv_buf)
            _rms_csv_buf.clear()

        # ── v100-EXP: update adaptive scales for NEXT epoch (Run E only) ────
        # Scale = min(4.0, rolling_RMS / sigma_ref), where sigma_ref is the
        # zenith code/phase sigma (SC=0.30 m, SP=0.010 m).
        # Triggers ONLY when adaptive_gal=True; otherwise knob is static.
        if adaptive_gal:
            if _gal_p2 is not None:
                _gal_code_rms_buf.append(_gal_p2 / 1000.)   # mm → m
            if _l2_rms is not None:
                _gal_phase_rms_buf.append(_l2_rms / 1000.)  # mm → m
            _roll_code  = (sum(_gal_code_rms_buf)  / len(_gal_code_rms_buf)
                           if _gal_code_rms_buf  else 0.0)
            _roll_phase = (sum(_gal_phase_rms_buf) / len(_gal_phase_rms_buf)
                           if _gal_phase_rms_buf else 0.0)
            _adaptive_code_sf  = min(4.0, _roll_code  / SC)  if _roll_code  > 1.0    else gal_code_scale
            _adaptive_phase_sf = min(4.0, _roll_phase / SP)  if _roll_phase > 0.15   else gal_phase_scale
    ...
    # v101 PATCH 3 / PATCH 6: flush all pending write buffers before file close
    if _c2w_buf:
        _c2w_audit_w.writerows(_c2w_buf)
        _c2w_buf.clear()
    if _rms_csv_buf:
    ...
    # v101 PATCH 5: end-of-pass summary
    if DEBUG_SUMMARY_ONLY:
        print(f"[PASS_SUMMARY] label={label}"
              f"  GF_READY_YES={_stats.get('gf_ready_yes',0)}"
              f"  GF_READY_NO_BIAS={_stats.get('gf_ready_no_bias',0)}"
              f"  REENTRY_BYPASSED={_stats.get('reentry_guard',0)}"
              f"  ARC_RESETS={_stats.get('arc_resets',0)}"
              f"  WL_FIXED_TOTAL={len(wl_fixed)}")
    _bias_csv_fh.flush()
    _bias_csv_fh.close()
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    # ── v100-EXP STOCHASTIC EXPERIMENT CAMPAIGN ──────────────────────────────
    # FROZEN ARCHITECTURE — only measurement weighting differs between runs.
    # GPS weighting, process noise, KF structure: ALL unchanged.
    run_configs = [
        dict(label='A:Baseline',
             const='GE', disable_gps_nl=False, disable_gal_states=False,
             gal_code_scale=1.0, gal_phase_scale=1.0, adaptive_gal=False),
        dict(label='B:GAL-code-x2',
             const='GE', disable_gps_nl=False, disable_gal_states=False,
             gal_code_scale=2.0, gal_phase_scale=1.0, adaptive_gal=False),
        dict(label='C:GAL-code-x4',
             const='GE', disable_gps_nl=False, disable_gal_states=False,
             gal_code_scale=4.0, gal_phase_scale=1.0, adaptive_gal=False),
        dict(label='D:GAL-phase-x1.5',
             const='GE', disable_gps_nl=False, disable_gal_states=False,
             gal_code_scale=1.0, gal_phase_scale=1.5, adaptive_gal=False),
        dict(label='E:Adaptive-GAL',
             const='GE', disable_gps_nl=False, disable_gal_states=False,
             gal_code_scale=1.0, gal_phase_scale=1.0, adaptive_gal=True),
    ]
    ...
    for cfg in run_configs:
        ...
        print(f"[RUN] {label}  constellation='{const}'  "
              f"gal_code_scale={cfg['gal_code_scale']}  "
              f"gal_phase_scale={cfg['gal_phase_scale']}  "
              f"adaptive_gal={cfg['adaptive_gal']}")
        _rts_store._data=[]
        fwd,ex,ec,ez,wl_f,fwd_amb,fwd_snap=_ppp_pass(
            ...
            gal_code_scale=cfg['gal_code_scale'],
            gal_phase_scale=cfg['gal_phase_scale'],
            adaptive_gal=cfg['adaptive_gal'],
            **_common)
    ...
    print(f"\n{'='*72}")
    print("  v100-EXP STOCHASTIC WEIGHTING EXPERIMENT SUMMARY")
    ...
    primary_fwd=all_fwd.get('A:Baseline',{})
    primary_rts=all_rts.get('A:Baseline',{})
...
def _plot_comparison(all_fwd,all_rts,REF):
    ...
    colors={'A:Baseline':'#3cb44b','B:GAL-code-x2':'#e6194b',
            'C:GAL-code-x4':'#f58231','D:GAL-phase-x1.5':'#4363d8',
            'E:Adaptive-GAL':'#911eb4'}
    fig,axes=plt.subplots(2,2,figsize=(14,10))
    fig.suptitle('PPP-AR v100-EXP — Stochastic Weighting Campaign: A/B/C/D/E',
    ...
    ax=axes[0,1]
    m=_compute_metrics(all_fwd.get('A:Baseline',{}),REF)
    if m is not None:
        sh=m['sods']/3600.
    ...
    ax=axes[1,1]
    mf=_compute_metrics(all_fwd.get('A:Baseline',{}),REF)
    mr=_compute_metrics(all_rts.get('A:Baseline',{}),REF)
