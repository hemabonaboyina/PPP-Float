...
# ══════════════════════════════════════════════════════════════════════════════
#  IONEX / GIM support  (v124 external ionosphere constraint)
# ══════════════════════════════════════════════════════════════════════════════

def parse_ionex(path):
    """Parse IONEX file.  Returns (lat_arr, lon_arr, maps).
    lat_arr : 1-D ndarray of latitudes  (deg), may be descending
    lon_arr : 1-D ndarray of longitudes (deg)
    maps    : list of (sod_float, vtec_2d) where vtec_2d[i_lat, i_lon] in TECU
    Only TEC maps are read.  All AUX DATA (DCBs, biases) are skipped.
    """
    exponent  = -1
    lat1 = lat2 = dlat = None
    lon1 = lon2 = dlon = None
    ...
    maps      = []
    cur_sod   = None
    cur_grid  = []          # list of rows, each row = list of floats (TECU)
    cur_row   = []          # accumulating values for the current lat band

    with open(path, 'r') as fh:
        for raw in fh:
            label = raw[60:].strip() if len(raw) > 60 else ''

            # ── AUX DATA (DCBs) — skip entirely ───────────────────────────
            if 'START OF AUX DATA' in label:
                in_aux = True;  continue
            if 'END OF AUX DATA'   in label:
                in_aux = False; continue
            if in_aux:
                continue

            # ── Header records ─────────────────────────────────────────────
            if 'EXPONENT' in label:
                exponent = int(raw[:6].strip())
            elif 'LAT1 / LAT2 / DLAT' in label:
                p = raw[:60].split()
                lat1, lat2, dlat = float(p[0]), float(p[1]), float(p[2])
            elif 'LON1 / LON2 / DLON' in label:
                p = raw[:60].split()
                lon1, lon2, dlon = float(p[0]), float(p[1]), float(p[2])
            ...
            # ── TEC map blocks ─────────────────────────────────────────────
                ...
                if cur_row:                  # flush last lat band
                    cur_grid.append(cur_row)
                if cur_sod is not None and cur_grid:
                    maps.append((cur_sod, np.array(cur_grid, dtype=np.float32)))
                in_tec = False

            elif in_tec:
                if 'EPOCH OF CURRENT MAP' in label:
                    p = raw[:60].split()
                    hr, mi, se = int(p[3]), int(p[4]), int(p[5])
                    cur_sod = float(hr * 3600 + mi * 60 + se)

                elif 'LAT/LON1/LON2/DLON/H' in label:
                    if cur_row:              # save previous lat band
                        cur_grid.append(cur_row)
                    cur_row = []             # start new lat band

                else:
                    # Data line: space-separated integers (scaled by 10^exponent)
                    scale = 10.0 ** exponent
                    for tok in raw.split():
                        try:
                            cur_row.append(float(tok) * scale)
                        except ValueError:
                            pass

    # Build coordinate arrays
    if lat1 is None or lon1 is None:
        raise ValueError(f"[IONEX] LAT/LON grid not found in {path}")
    n_lat = int(round((lat2 - lat1) / dlat)) + 1
    n_lon = int(round((lon2 - lon1) / dlon)) + 1
    lat_arr = np.array([lat1 + i * dlat for i in range(n_lat)], dtype=np.float64)
    lon_arr = np.array([lon1 + j * dlon for j in range(n_lon)], dtype=np.float64)

    print(f"[IONEX] {len(maps)} TEC maps  "
          f"lat=[{lat_arr[0]:.1f}..{lat_arr[-1]:.1f}] step={dlat}  "
          f"lon=[{lon_arr[0]:.1f}..{lon_arr[-1]:.1f}] step={dlon}")
    return lat_arr, lon_arr, maps


def _ionex_ipp_latlon(rx_ecef, el_rad, az_rad, H_km=450.0):
    """Ionospheric pierce point latitude and longitude (degrees).
    Single-layer model at height H_km above mean Earth surface.
    rx_ecef : ECEF + nominal offset (nom + x[:3]), metres
    el_rad  : satellite elevation (radians)
    az_rad  : satellite azimuth (radians, from N)
    Returns  (ipp_lat_deg, ipp_lon_deg)
    """
    Re = 6371.0                              # km  (IONEX BASE RADIUS)
    # Earth-central angle between receiver and IPP
    psi = math.pi / 2.0 - el_rad - math.asin(Re / (Re + H_km) * math.cos(el_rad))
    # Receiver geodetic lat/lon (radians)
    lat_r, lon_r, _ = _lla(rx_ecef)
    # IPP latitude
    lat_ipp = math.asin(
        math.sin(lat_r) * math.cos(psi) +
        math.cos(lat_r) * math.sin(psi) * math.cos(az_rad))
    # IPP longitude  (guard against cos(lat_ipp) ≈ 0 near poles)
    cos_lat_ipp = math.cos(lat_ipp)
    if abs(cos_lat_ipp) < 1e-9:
        lon_ipp = lon_r
    else:
        lon_ipp = lon_r + math.asin(
            math.sin(psi) * math.sin(az_rad) / cos_lat_ipp)
    return math.degrees(lat_ipp), math.degrees(lon_ipp)


def _ionex_vtec_slant(ionex_data, sod, ipp_lat_deg, ipp_lon_deg, el_rad, H_km=450.0):
    """Convert IONEX VTEC to L1 slant delay in metres.
    ionex_data : (lat_arr, lon_arr, maps) from parse_ionex
    sod        : second of day (float)
    ipp_lat_deg, ipp_lon_deg : IPP coordinates (degrees)
    el_rad     : satellite elevation (radians)
    Returns slant delay in metres, or None on failure.
    """
    lat_arr, lon_arr, maps = ionex_data
    if not maps:
        return None

    # ── temporal interpolation ─────────────────────────────────────────────
    sods_m = [m[0] for m in maps]
    if sod <= sods_m[0]:
        vtec_grid = maps[0][1].astype(np.float64)
    elif sod >= sods_m[-1]:
        vtec_grid = maps[-1][1].astype(np.float64)
    else:
        idx2 = int(np.searchsorted(sods_m, sod))
        idx2 = max(1, min(idx2, len(maps) - 1))
        idx1 = idx2 - 1
        t1, g1 = maps[idx1]
        t2, g2 = maps[idx2]
        frac = (sod - t1) / (t2 - t1) if (t2 - t1) > 0.0 else 0.0
        vtec_grid = g1.astype(np.float64) * (1.0 - frac) + g2.astype(np.float64) * frac

    # ── spatial bilinear interpolation ─────────────────────────────────────
    # Normalise lon to grid range
    lon = ipp_lon_deg
    while lon < lon_arr[0]:  lon += 360.0
    while lon > lon_arr[-1]: lon -= 360.0

    # lat_arr may be descending (87.5 → -87.5); work on sorted copies
    lat_min = float(np.min(lat_arr)); lat_max = float(np.max(lat_arr))
    lat = float(np.clip(ipp_lat_deg, lat_min, lat_max))
    lon = float(np.clip(lon, float(lon_arr[0]), float(lon_arr[-1])))

    # Find bracketing indices
    if lat_arr[0] > lat_arr[-1]:                      # descending
        # searchsorted on reversed array
        rev = lat_arr[::-1]                            # ascending
        i_r = int(np.searchsorted(rev, lat))
        i_r = int(np.clip(i_r, 1, len(rev) - 1))
        # map back to original (descending) indices
        i_lat_lo = len(lat_arr) - 1 - i_r             # smaller lat in grid
        i_lat_hi = i_lat_lo + 1                        # larger lat in descending → lower index
        i_lat_hi, i_lat_lo = len(lat_arr)-1-(i_r-1), len(lat_arr)-1-i_r
        lat0 = float(lat_arr[i_lat_lo]);  lat1f = float(lat_arr[i_lat_hi])
    else:                                              # ascending
        i_lat_lo = int(np.searchsorted(lat_arr, lat)) - 1
        i_lat_lo = int(np.clip(i_lat_lo, 0, len(lat_arr) - 2))
        i_lat_hi = i_lat_lo + 1
        lat0 = float(lat_arr[i_lat_lo]);  lat1f = float(lat_arr[i_lat_hi])

    i_lon_lo = int(np.searchsorted(lon_arr, lon)) - 1
    i_lon_lo = int(np.clip(i_lon_lo, 0, len(lon_arr) - 2))
    i_lon_hi = i_lon_lo + 1
    lon0 = float(lon_arr[i_lon_lo]);  lon1f = float(lon_arr[i_lon_hi])

    dlat_g = lat1f - lat0
    dlon_g = lon1f - lon0

    if abs(dlat_g) < 1e-9 or abs(dlon_g) < 1e-9:
        vtec_val = float(vtec_grid[i_lat_lo, i_lon_lo])
    else:
        t = (lat - lat0) / dlat_g
        u = (lon - lon0) / dlon_g
        t = float(np.clip(t, 0.0, 1.0))
        u = float(np.clip(u, 0.0, 1.0))
        v00 = float(vtec_grid[i_lat_lo, i_lon_lo])
        v01 = float(vtec_grid[i_lat_lo, i_lon_hi])
        v10 = float(vtec_grid[i_lat_hi, i_lon_lo])
        v11 = float(vtec_grid[i_lat_hi, i_lon_hi])
        vtec_val = (v00 * (1.0-t) * (1.0-u)
                  + v01 * (1.0-t) * u
                  + v10 * t       * (1.0-u)
                  + v11 * t       * u)

    # ── VTEC → vertical L1 delay (m) ──────────────────────────────────────
    # 40.3e16 / f1^2 = 40.3e16 / (1575.42e6)^2 ≈ 0.16218 m/TECU
    I_vert = 0.16218 * vtec_val

    # ── mapping function ───────────────────────────────────────────────────
    Re = 6371.0
    sin_z = (Re / (Re + H_km)) * math.cos(el_rad)
    sin_z = min(sin_z, 0.99999)
    M = 1.0 / math.sqrt(1.0 - sin_z * sin_z)
...
# ── v121 MEAS_DEBUG instrumentation ───────────────────────────────────────
# Logs raw/corrected L1 phase and P1 code per epoch for target satellites.
# Throttled to first epoch + every 60 epochs per satellite to avoid log flood.
_MEAS_DBG_SATS: set = {'G01', 'G05', 'G09'}
_meas_dbg_epoch: dict = {}   # sid → number of epochs seen since reset

# ==============================================================================
#  v123_PCAUDIT — Phase-Code Inconsistency Audit (Parts 1-9)
#  D = L1m − P1c  =  (L1·λ1 − phase_OSB)  −  (P1 − code_OSB)
#  Tracks per-satellite phase-code health from first epoch onward.
#  Controls: startup code-noise relaxation, N1_init fix, final report.
# ==============================================================================
_pc_diff_hist:          dict  = {}    # sid → list[float] of D values (first 2h, max 240)
_pc_init_D:             dict  = {}    # sid → D at very first valid epoch
_pc_init_flag:          dict  = {}    # sid → 'OK' | 'BAD'
_pc_2h_stats:           dict  = {}    # sid → {'mean','std','drift','type'}
_pc_n1_corrected:       set   = set() # sats whose N1_init was fixed via Part 6
_startup_relax_end_sod: float = 0.0   # sod_start + 3600 (set at pass start)
_startup_relax_printed: bool  = False # [STARTUP_MODE] printed once per pass
# OSB IF-combination check: printed once per satellite per pass
_osb_if_check_printed:  set   = set()
# Validation: mean|D| in first 20 minutes (40 epochs at 30s)
_pc_val_buf:            dict  = {}    # sid → list[float] of |D| for first 20 min
...
#  DISABLE_WL_FIXING = True  → no WL assignment, no WL usage, no WL prints,
#                              wl_fixed is force-cleared every epoch.
#  PURE_FLOAT        = True  → affirms that the run is unconditionally float.
# ==============================================================================
DISABLE_WL_FIXING: bool = False  # WL fixing ENABLED for verification
PURE_FLOAT:        bool = False  # WL ambiguity fixing active (NL still off)
...
_diag_influence_sat  = {}  # sid -> latest position-direction influence norm (m)
...
def _proc(sid,so,tow,rxyz,ah,sp3t,sp,sc,clkd,osb,lat0,doy,zhd,elm,satx,att,recx,
    ...
    # ── v121 [MEAS_DEBUG] per-epoch instrumentation ───────────────────────
    # Logs raw/corrected L1 phase and P1 code for target satellites.
    # Throttle: first epoch seen + every 60 subsequent epochs (~30 min at 30s).
    if sid in _MEAS_DBG_SATS:
        _meas_dbg_epoch[sid] = _meas_dbg_epoch.get(sid, 0) + 1
        _dbg_cnt = _meas_dbg_epoch[sid]
        if _dbg_cnt == 1 or _dbg_cnt % 60 == 0:
            # Raw phase in meters (pre-OSB)
            _L1m_raw = L1 * lam1
            # OSB-corrected values (L1m / P1c already computed above)
            _L1m_corr = L1m
            _P1c_corr  = P1c
            _delta     = _L1m_corr - _P1c_corr
            # Lambda check: L1m_raw / L1_cycles should ≈ lam1 ≈ 0.1903 m
            _lam_check = (_L1m_raw / L1) if L1 != 0. else float('nan')
            # Phase OSB and code OSB actually applied (None → 0 if OSB missing)
            _p_osb_applied = _phase_osb_1 if (_osb_complete and _phase_osb_1 is not None) else 0.0
            _c_osb_applied = _code_osb_1  if (_osb_complete and _code_osb_1  is not None) else 0.0
            _sod_dbg = tow % 86400.0
            print(
                f"[MEAS_DEBUG] sat={sid} SOD={_sod_dbg:.0f} epoch={_dbg_cnt}"
                f"\n  raw_L1_cycles={L1:.3f}  raw_P1_meters={P1:.3f}"
                f"\n  lambda_L1={lam1:.9f} m  freq_L1={CLIGHT/lam1:.3f} Hz"
                f"\n  L1_m_raw(=L1*lam1)={_L1m_raw:.3f}  lam_check={_lam_check:.9f} m"
                f"\n  phase_osb_L1={_p_osb_applied:+.6f} m  (applied={_osb_complete})"
                f"\n  code_osb_L1 ={_c_osb_applied:+.6f} m  (applied={_osb_complete})"
                f"\n  L1_corr={_L1m_corr:.3f}  P1_corr={_P1c_corr:.3f}"
                f"\n  delta_L1_P1={_delta:+.4f} m"
                f"\n  osb_complete={_osb_complete}  no_AR={no_AR}"
            )
    # ── end v121 MEAS_DEBUG ────────────────────────────────────────────────
...
def _ppp_pass(epochs,sp3t,sp,sc,clkd,osb,ah,nom,iclk,izwd,lat0,doy,zhd,tref,
              satx,att,recx,elm=math.radians(15.),SC=0.35,SP=0.035,  # v121 PART2: SP 0.015→0.035 | DIAG: SC 0.25→0.35 (σ_code tightened)
    ...
    global _meas_dbg_epoch        # v121: reset per-pass MEAS_DEBUG epoch counters
    _meas_dbg_epoch = {}
    # v123_PCAUDIT — reset all per-pass phase-code audit state
    global _pc_diff_hist, _pc_init_D, _pc_init_flag, _pc_2h_stats
    global _pc_n1_corrected, _startup_relax_end_sod, _startup_relax_printed
    global _osb_if_check_printed, _pc_val_buf
    _pc_diff_hist          = {}
    _pc_init_D             = {}
    _pc_init_flag          = {}
    _pc_2h_stats           = {}
    _pc_n1_corrected       = set()
    _startup_relax_end_sod = 0.0   # retained for reference; relaxation is disabled
    _startup_relax_printed = False
    _osb_if_check_printed  = set()
    _pc_val_buf            = {}
    # ── PART 5 DIAG: per-pass accumulators for CODE_USAGE/IONO_DIAG/PC_DIFF/IONEX ──
    global _diag5_code_cnt, _diag5_obs_cnt
    global _diag5_I_buf, _diag5_N1_buf
    global _diag5_pc_diff_10min, _diag5_sod_start_ref
    global _diag5_ionex_diff_buf
    _diag5_code_cnt         = 0     # code rows accumulated since last 300-ep print
    _diag5_obs_cnt          = 0     # total obs rows accumulated since last 300-ep print
    _diag5_I_buf            = []    # I_state values for corr(I,N1)
    _diag5_N1_buf           = []    # N1_state values for corr(I,N1)
    _diag5_pc_diff_10min    = []    # |L1m-P1c| values in first 10 min
    _diag5_sod_start_ref    = None  # sod of first epoch (gate first-10-min window)
    _diag5_ionex_diff_buf   = []    # |I_model - I_est| per satellite per epoch
    ...
    global _diag_lp1_sat, _diag_influence_sat
    _diag_prev_sig_pair  = {}
    ...
    _diag_influence_sat  = {}
    # ── end DIAG reset ────────────────────────────────────────────────────
    # v121 diagnostics — print once per pass
    print(f"[WEIGHT_FIX] gps_extra_scale_removed = True")
    print(f"[PHASE_SIGMA] old=0.015 new={SP:.3f}")
    init_osb_csv()
    wl_fixed=dict(wl_init) if wl_init else {}
    _amb_init=dict(amb_init) if amb_init else {}
    _amb_seeded=set()
    nl_fixed={}
    # ── WL stability tracking (Steps 5 & 6) ──────────────────────────────────
    _wl_fix_count     = {}   # per-sat: total times WL integer was assigned
    _wl_rel_count     = {}   # per-sat: total times WL was released (slip/clear)
    _wl_first_fix_sod = None # SOD of very first WL fix this pass
    wl_last_log       = {}   # per-sat: last integer logged via WL_EVENT (suppresses repeats)
    ...
    # ── v_RDCB PART 2/5: Receiver Differential Code Bias (GPS only) ──────────
    # RDCB_G = b_rec(C1W) − b_rec(C2W)  in metres
    # P1_GPS = ρ + c·dt + I + T − BETA·RDCB_G
    # P2_GPS = ρ + c·dt + γ·I + T − ALFA·RDCB_G
    # Initial value 0.0 m; σ_0 = 10.0 m (uninformative prior)
    IDX_RDCB_G = len(x)
    x = np.append(x, [0.0])
    _Pn_rdcb = np.zeros((len(x), len(x)))
    _Pn_rdcb[:len(x)-1, :len(x)-1] = P
    _Pn_rdcb[IDX_RDCB_G, IDX_RDCB_G] = 10.0**2   # PART 5: σ_0 = 10 m
    P = _Pn_rdcb

    # sidx maps sat_id → base index of its 3-state block [I, N1, N2]
    # sidx_bsat maps sat_id → index of its per-satellite code bias state b_sat
    sidx={}; sidx_bsat={}; namb=0; phi={}; wum={}; prev_mw={}; prev_gf={}
    mw_hist=defaultdict(list)
    results={}; psod=None; nproc=0
    # [CODE_REJECT] 5.5σ code prefit gate counters (v123: widened from 3.0σ)
    _cr_total   = 0   # total code measurements seen
    _cr_rejected= 0   # code measurements soft-rejected (Rd inflated)
    _cr_ratio_sum = 0.0  # v122 PART3: sum of |res|/sigma for accepted code obs
    _cr_ratio_cnt = 0    # v122 PART3: count of accepted code obs
    ...
    # ── [A1] Satellite Health Pre-Filter — data structures ─────────────────
    # Purpose: detect pathological code behaviour BEFORE it corrupts R matrix,
    # ionosphere states, and clock.  Three independent metrics; any one triggers
    # CODE_SUSPECT.  Flagged satellites have their code sigma inflated ×5
    # (variance ×25); phase is never touched.
    #
    # Metric A — rolling std of (L1m − P1c) over last 300 epochs.
    #   Buffer is separate from _lp1_hist (which is maxlen=100 and cleared on
    #   slip).  A1 buffer persists across short gaps but is cleared on slip.
    _a1_lp1_buf  = defaultdict(lambda: deque(maxlen=300))  # sid → deque of (L1m-P1c)
    #
    # Metric B — rolling code rejection rate over last 300 epochs.
    #   Counters track P1 and P2 rejections independently; total / window size.
    #   Entry format: deque of 1 (rejected) or 0 (not rejected) per P1+P2 pair
    #   → 2 entries per epoch, window length 300 epochs = 600 slots.
    _a1_rej_buf  = defaultdict(lambda: deque(maxlen=600))   # sid → deque of 0/1 per obs
    #
    # Metric C — sign reversal: two sub-windows of 60 epochs each.
    #   Buffer stores last 120 code prefit residuals (P1 only for simplicity).
    _a1_res_buf  = defaultdict(lambda: deque(maxlen=120))   # sid → deque of P1 residuals
    #
    # Flag state: how many consecutive epochs each satellite has been CODE_SUSPECT.
    _a1_flag_age = defaultdict(int)   # sid → consecutive epochs flagged (0 = not flagged)
    _a1_flagged  = set()              # currently active CODE_SUSPECT satellites
    #
    # Thresholds (fixed, GPS / Galileo may differ for metric A)
    _A1_STD_GPS   = 6.0    # m  — GPS  rolling-std threshold for (L1m-P1c)
    _A1_STD_GAL   = 8.0    # m  — Galileo rolling-std threshold (looser: GAL code is noisier at IISC)
    _A1_REJ_RATE  = 0.40   # fraction — >40% P1+P2 rejected in window → suspect
    _A1_SIGN_MAG  = 3.5    # m  — magnitude threshold for mean sub-window residual
    _A1_PERSIST   = 60     # epochs — minimum epochs flag stays set once raised
    _A1_SIGMA_MUL = 3.0    # code sigma multiplier for flagged satellites (×3 → variance ×9)
    #
    # ── [A1-FAST] Short-term fast-trigger data structures ───────────────────
    # Fast Trigger 1 — consecutive epochs where CODE_REJECT fired for P1 or P2.
    #   Updated in A1-REJBUF (after CODE_REJECT).  Evaluated in A1-INNER using
    #   previous-epoch value.  Decremented by 1 (gradual decay) when no rejection.
    _a1_consec_rej   = defaultdict(int)              # sid → consecutive reject-epoch count
    #
    # Fast Trigger 2 — short rolling buffer of abs(P1 prefit) last 10 epochs.
    #   Trigger fires when ≥3 of last 5 epochs exceed _A1_FAST_RES_THR.
    _a1_fast_res_buf = defaultdict(lambda: deque(maxlen=10))  # sid → abs P1 prefit (m)
    #
    # Activation delay: fast_flag requires 2 consecutive pending epochs before
    # becoming active (prevents single-spike false positives).
    _a1_fast_pend    = defaultdict(int)   # sid → pending epochs count (0 = not pending)
    #
    # Active fast-flag state with minimum-persistence tracking.
    _a1_fast_flagged = set()              # sids with active fast_flag
    _a1_fast_age     = defaultdict(int)   # sid → epochs fast_flag has been continuously active
    #
    # Fast-trigger thresholds (do not change)
    _A1_FAST_CONSEC  = 3     # consecutive reject epochs → trigger
    _A1_FAST_RES_THR = 5.0   # m — abs prefit threshold for residual trigger
    _A1_FAST_PERSIST = 30    # epochs — minimum fast_flag duration once raised
    _A1_FAST_PEND    = 2     # epochs — required pending duration before activation
    # ── [A1-FAST] end data structures ───────────────────────────────────────
    # ── [A1] end data structures ────────────────────────────────────────────

    # ISB rolling buffer for stability diagnostic (last 300 epochs ≈ 2.5 h)
    _isb_buf = deque(maxlen=300)
    ...
    # ── ADAPTIVE IONO Q (per-sat dI_meas tracking) ───────────────────────────
    _iono_prev_I_meas = {}          # sid → I_meas from previous epoch (m)
    _iono_dI_adapt    = defaultdict(float)  # sid → |dI| used for Q scaling
    _iono_prev_state_adapt = {}     # v123 FIX4: post-update I state for state-based dI

    # ── IONO LAG DIAGNOSTIC (read-only, no filter changes) ──────────────────
    # _iono_state_hist[sid] : list of x[ki] after each KF update
    # _iono_meas_hist[sid]  : list of I_meas = (P2c-P1c)/(gamma-1) each epoch
    # _iono_prev_state[sid] : x[ki] from previous epoch for dI_state computation
    _iono_state_hist  = defaultdict(list)   # sid → [I_state per epoch]
    _iono_meas_hist   = defaultdict(list)   # sid → [I_meas  per epoch]
    _iono_sod_hist    = defaultdict(list)   # sid → [sod per epoch]
    _iono_diag_label  = None               # filled from label at first epoch
    _v122_I_meas_prev = {}    # v122 PART4: sid → I_meas from previous epoch (for dI_code)
    ...
    _up_err_hist: list = []          # v117 FIX5: rolling Up-error history for detrended hump-bin RMS
    _UP_DETREND_WIN = 600            # v117 FIX5: half-width in epochs (matches ELEV_AUDIT detrend_win)
    # ── end accumulators ──────────────────────────────────────────────────────
    ...
    ep_idx = 0  # v121: epoch counter for once-per-run prints

    for epoch in eplist:
        sod=epoch['t']; sobs=epoch['sats']
        if DISABLE_WL_FIXING:
            wl_fixed.clear()   # HARD disable: guarantee no stale WL state survives
        ...
        # ── v_RDCB PART 6: RDCB process noise ────────────────────────────────
        # (0.001 m)² per epoch — allows slow receiver hardware drift
        Q[IDX_RDCB_G, IDX_RDCB_G] = (0.001)**2 * dt
        ...
        ION_PROC_NOISE = 5e-4 * ion_q_scale   # v123: removed ×2.0 (was added in v121 to "restore code
                                               # observability" but with code_pct=0.37% it only allowed
                                               # iono to wander more freely, making degeneracy worse)
        if ep_idx == 0:
            print(f"[IONO_Q_UPDATE] ION_PROC_NOISE={ION_PROC_NOISE:.2e} (scale={ion_q_scale:.2f}, no x2 multiplier)")
        q_iono = ION_PROC_NOISE * dt       # base; per-sat elevation weight applied in Q loop
        ...
        _iono_adapt_scale = {}            # sid → adaptive Q scale this epoch
        for sid_k,ki in sidx.items():
            _dI_k   = _iono_dI_adapt.get(sid_k, 0.0)
            _scale_k = min(1.0 + 2.0 * _dI_k, 5.0)   # v123 FIX3: cap 20→5, sens 5→2
            _iono_adapt_scale[sid_k] = _scale_k
            Q[ki  ,ki  ]=q_iono * _iono_q_factor * _scale_k  # I slot: adaptive Q
            ...
            # b_sat process noise: (0.001 m)^2 * dt — slow satellite hardware drift
            kb_k = sidx_bsat[sid_k]
            Q[kb_k, kb_k] = (0.001)**2 * dt
        ...
        # ── DIAG PART 1: STARTUP RELAXATION — DISABLED ───────────────────────────
        # Code-weight inflation during startup (x9 on Rd) has been removed.
        # sigma_code = SC = 0.35 m is applied uniformly from epoch 1.
        _in_startup_relax = False   # always False; inflation block also removed below
        if not _startup_relax_printed:
            _startup_relax_printed = True
            print("[STARTUP_MODE] disabled")
        # ── end PART 1 ────────────────────────────────────────────────────────────
        ...
        for sid,so in sorted(sobs.items()):
            ...
            if sid not in wl_fixed:
                ...
                if n_hist>=15:
                    ...
                    if n_hist>=min_n:
                        ...
                        # ── Steps 2-4: WL_EVENT — log only on integer change ──
                        sigma_wl   = sd        # MW std used as WL float sigma
                        N_wl_float = mn_corr   # bias-corrected MW mean
                        N_int      = int(round(N_wl_float))
                        if (abs(N_wl_float - N_int) < 0.2
                                and (sid not in wl_last_log
                                     or wl_last_log[sid] != N_int)):
                            print(f"[WL_EVENT] SOD={sod:.0f} sat={sid} "
                                  f"Nwl={N_wl_float:.3f} sigma={sigma_wl:.3f} "
                                  f"int={N_int}")
                            wl_last_log[sid] = N_int
                        if sys_id not in b_rec_frozen:
                            pass
                            ...
                            if not DISABLE_WL_FIXING:
                                ...
                                # ── Step 3: WL_FIXED log ─────────────────────
                                N_wl_int = NWL_to_use
                                print(f"[WL_FIXED] SOD={sod:.0f} sat={sid} "
                                      f"Nwl_int={N_wl_int}")
                                # ── Step 4: WL residual check ─────────────────
                                wl_residual = mn_corr - N_wl_int
                                print(f"[WL_CHECK] SOD={sod:.0f} sat={sid} "
                                      f"residual={wl_residual:.3f}")
                                # ── Step 5: stability fix counter ─────────────
                                if _wl_first_fix_sod is None:
                                    _wl_first_fix_sod = sod
                                _wl_fix_count[sid] = _wl_fix_count.get(sid, 0) + 1
                            if not DISABLE_WL_FIXING:
            ...
            # b_sat: per-satellite code bias state (1 extra slot, appended after N2)
            if sid not in sidx:
                d=len(x)
                # Append 4 new states: I, N1, N2, b_sat
                x=np.append(x,[0.,0.,0.,0.])
                Pn=np.zeros((d+4,d+4)); Pn[:d,:d]=P
                Pn[d,  d  ]=300.**2   # I  initial var
                Pn[d+1,d+1]=300.**2   # N1 initial var
                Pn[d+2,d+2]=300.**2   # N2 initial var
                Pn[d+3,d+3]=20.**2    # b_sat initial var (20 m)^2
                P=Pn; sidx[sid]=d; sidx_bsat[sid]=d+3; namb+=1; phi[sid]=False
            ki=sidx[sid]  # ki → I slot; ki+1 → N1 slot; ki+2 → N2 slot
            kb=sidx_bsat[sid]  # kb → b_sat slot

            if slip:
                # Reset all 3 states on cycle slip
                x[ki]=0.; x[ki+1]=0.; x[ki+2]=0.
                # v123 BUG FIX: clear full rows+cols — previously only diagonal was
                # reset, leaving surviving off-diagonal covariances that caused
                # spurious position/clock/ZWD updates on the first post-slip epoch
                # (root cause of max_dI=27m events seen in GPS iono diagnostics).
                for _slip_idx in (ki, ki+1, ki+2):
                    P[_slip_idx, :] = 0.
                    P[:, _slip_idx] = 0.
                # b_sat reset on slip: new arc, hardware state may change
                x[kb] = 0.
                P[kb, :] = 0.; P[:, kb] = 0.
                P[kb, kb] = 20.**2
                # Note: P[ki,ki] (iono) will be properly set by the 'if not phi' block
                # below using Fix5b (elevation-scaled). N1/N2 diagonals set to 300² here
                # as placeholders until the 'if not phi' block re-initialises them.
                P[ki,ki]=9.**2; P[ki+1,ki+1]=300.**2; P[ki+2,ki+2]=300.**2  # I=9m placeholder
                phi[sid]=False
                ...
                _lp1_hist[sid].clear()
                # [A1] clear pre-filter buffers on slip — new arc starts fresh
                _a1_lp1_buf[sid].clear()
                _a1_rej_buf[sid].clear()
                _a1_res_buf[sid].clear()
                # [A1-FAST] clear fast-trigger state on slip
                _a1_consec_rej[sid]   = 0
                _a1_fast_res_buf[sid].clear()
                _a1_fast_pend[sid]    = 0
                _a1_fast_flagged.discard(sid)
                _a1_fast_age[sid]     = 0
            ...
            # [A1] feed the 300-epoch std buffer (Metric A)
            _a1_lp1_buf[sid].append(m['L1m'] - m['P1c'])

            # ── v123_PCAUDIT PART 1: D = L1m − P1c (OSB-corrected phase−code) ──
            _D_val = m['L1m'] - m['P1c']   # true phase−code difference after OSB
            # Per-epoch log (throttled: first epoch + every 30 epochs per sat)
            _pc_ep_cnt = len(_pc_diff_hist.get(sid, []))
            if _pc_ep_cnt == 0 or _pc_ep_cnt % 30 == 0:
                print(f"[PC_DIFF] sat={sid} SOD={sod:.0f} D={_D_val:+.4f}m  ep={_pc_ep_cnt+1}")

            # Accumulate 2-h history (max 240 entries ≈ 2h at 30s)
            if sid not in _pc_diff_hist:
                _pc_diff_hist[sid] = []
            if len(_pc_diff_hist[sid]) < 240:
                _pc_diff_hist[sid].append(_D_val)

            # Validation buffer: first 40 epochs (~20 min)
            if sid not in _pc_val_buf:
                _pc_val_buf[sid] = []
            if len(_pc_val_buf[sid]) < 40:
                _pc_val_buf[sid].append(abs(_D_val))

            # ── v123_PCAUDIT PART 2: INITIAL CONSISTENCY CHECK (first epoch only) ──
            if sid not in _pc_init_D:
                _pc_init_D[sid]  = _D_val
                _ok_flag = 'OK' if abs(_D_val) <= 5.0 else 'BAD'
                _pc_init_flag[sid] = _ok_flag
                print(f"[PC_INIT] sat={sid} D0={_D_val:+.4f}m  flag={_ok_flag}")

            # ── v123_PCAUDIT PART 4: OSB IF-COMBINATION CONSISTENCY ──────────────
            # Check that (F1²·osb_code1 − F2²·osb_code2)/(F1²−F2²) ≈ 0
            # (non-zero means OSBs carry a residual IF bias → contaminates clocks)
            if sid not in _osb_if_check_printed:
                _osb_if_check_printed.add(sid)
                _c1k = m.get('code1_type', 'C1W' if sid[0]=='G' else 'C1C')
                _l1k = m.get('phase1_type', 'L1C')
                _c2k = m.get('code2_type', 'C2W')
                _prn_osb_pc = osb.get(sid, {}) if osb else {}
                _oc1 = _prn_osb_pc.get(_c1k, 0.0) or 0.0
                _oc2 = _prn_osb_pc.get(_c2k, 0.0) or 0.0
                _op1 = _prn_osb_pc.get(_l1k, 0.0) or 0.0
                if sid[0] == 'G':
                    _f1sq, _f2sq = FREQ1**2, FREQ2**2
                elif sid[0] == 'E':
                    _f1sq, _f2sq = FREQ_E1**2, FREQ_E5A**2
                else:
                    _f1sq, _f2sq = FREQ1**2, FREQ2**2
                _if_code   = (_f1sq*_oc1 - _f2sq*_oc2) / (_f1sq - _f2sq)
                _mismatch  = abs(_if_code) > 0.05
                _phase_diff_01 = _op1  # phase OSB at L1 (direct, not combined)
                print(f"[OSB_VERIFY] sat={sid} sig={_c1k}/{_c2k}/{_l1k} "
                      f"IF_code={_if_code:+.4f}m  phase_osb_L1={_op1:+.6f}m "
                      f"{'MISMATCH' if _mismatch else 'OK'}")

            # ── v123_PCAUDIT PART 5: SIGNAL MAP LOG (every 300 epochs) ──────────
            if nproc % 300 == 0:
                _c1_sm = m.get('code1_type', '?')
                _p1_sm = m.get('phase1_type', '?')
                _c2_sm = m.get('code2_type', '?')
                _prn_sm = osb.get(sid, {}) if osb else {}
                _osb_c1_sm = _prn_sm.get(_c1_sm, float('nan'))
                _osb_p1_sm = _prn_sm.get(_p1_sm, float('nan'))
                print(f"[SIGNAL_MAP] sat={sid} using P1={_c1_sm}(osb={_osb_c1_sm:+.4f}m), "
                      f"L1={_p1_sm}(osb={_osb_p1_sm:+.6f}m), P2={_c2_sm}  D={_D_val:+.4f}m")
            # ── end v123_PCAUDIT ─────────────────────────────────────────────────

            # ── DIAG: accumulate L1m-P1c into all-epoch diagnostic dict ──
            if sid not in _diag_lp1_sat:
                _diag_lp1_sat[sid] = []
            _diag_lp1_sat[sid].append(m['L1m'] - m['P1c'])
            # ── end DIAG ─────────────────────────────────────────────────

            # ── v54 RAW STATE INIT [I, N1, N2] ──────────────────────────────
            if not phi.get(sid,False):
                rp0=_rp(m,x[IDX_CLK],x[IDX_ZWD])        # geometric + corrections (no ionosphere)
                gam=m['gamma']

                # I initialised from geometry-free pseudorange
                # v123 FIX5a: clamp I_init to physical range.
                # Raw code GF at IISC can be 7-10m at startup (G08 observed at SOD=0).
                # Max physical L1 iono at equatorial station: ~15m (80 TECU peak).
                I_init_raw = (m['P2c']-m['P1c'])/(gam-1.)
                I_init = max(-5.0, min(15.0, I_init_raw))
                x[ki] = I_init
                # v123 FIX5b: elevation-scaled initial iono P; replaces fixed 50²=2500m².
                # σ_I_init ≈ SC*√2/(sin²(el)*(γ-1)).  At el=36° GPS: σ_I≈1.6m→P≈2.5m².
                # 50²=2500m² was ~1000× too large → massive cross-state updates on
                # first observations (root cause of persistent max_dI=27m in GPS iono).
                _sel_init = max(math.sin(math.radians(m['el'])), 0.15)
                _I_sigma_init = SC * math.sqrt(2.0) / (_sel_init**2 * (gam - 1.0))
                _I_sigma_init = max(3.0, min(12.0, _I_sigma_init))  # 3–12 m bound
                P[ki, ki] = _I_sigma_init**2

                # N1 init: L1mc = rp0 − I + λ1·N1 + ISB  →  N1 = (L1mc − rp0 + I − ISB)/λ1
                # N2 init: L2mc = rp0 − γI + λ2·N2 + ISB →  N2 = (L2mc − rp0 + γI − ISB)/λ2
                # Without subtracting ISB, N1_init absorbs ISB/λ1 (~8 cycles for Galileo E1),
                # making z[L1] ≈ −ISB (~−1.5 m) on first use → fires PHASE_RES_GATE every
                # new Galileo arc.  GPS: _isb_s=0, no change.
                _isb_s = x[IDX_ISB_GAL] if sid[0] == 'E' else 0.0
                x[ki+1]=(L1mc - rp0 + I_init - _isb_s)/lam1
                P[ki+1,ki+1]=300.**2

                x[ki+2]=(L2mc - rp0 + gam*I_init - _isb_s)/lam2
                P[ki+2,ki+2]=300.**2
                ...
                # ── v123_PCAUDIT PART 6: N1_init correction for INCONSISTENT_START ──
                # If |D0| > 5 m, seed N1 directly from D so the filter starts
                # with a better ambiguity estimate instead of relying on I_init
                # (which may be clamped).  N1_init = D0/λ1 places the predicted
                # L1 phase exactly at the observed value on the first epoch.
                # We only apply this once per satellite arc.
                if sid not in _pc_n1_corrected:
                    _D0_init = _pc_init_D.get(sid, 0.0)
                    if abs(_D0_init) > 5.0:
                        # Compute N1 directly from observed D instead of geometry
                        # D0 = L1m − P1c ≈ λ1·N1 − 2·I   so  N1 ≈ (D0 + 2·I_init)/λ1
                        # But cleaner: solve from the phase equation directly:
                        #   L1mc = rp0 − I_init + λ1·N1_new  =>  N1_new = (L1mc − rp0 + I_init − _isb_s)/λ1
                        # The existing formula IS correct; the problem is only when I_init is
                        # clamped.  Use un-clamped I_re = (P2c−P1c)/(γ−1) for the N1 seed:
                        _I_unclamped = (m['P2c'] - m['P1c']) / (gam - 1.0)
                        _N1_from_D   = (m.get('L1mc', m['L1m']) - rp0 + _I_unclamped - _isb_s) / lam1
                        x[ki]   = _I_unclamped    # un-clamped I_init
                        x[ki+1] = _N1_from_D      # N1 absorbs full phase-code offset
                        x[ki+2] = (m.get('L2mc', m['L2m']) - rp0 + gam*_I_unclamped - _isb_s) / lam2
                        _pc_n1_corrected.add(sid)
                        print(f"[N_INIT] sat={sid} D0={_D0_init:+.4f}m  "
                              f"I_clamped={I_init:.4f}m  I_unclamped={_I_unclamped:.4f}m  "
                              f"N1_init={_N1_from_D:.3f}cyc")
                # ── end PART 6 ───────────────────────────────────────────────────
            ...
            # v111 FIX: undo flat Q addition, apply correct elevation-weighted noise
            # Low el -> high noise (physically correct); high el -> low noise
            _adapt_k    = _iono_adapt_scale.get(sid, 1.0)
            _q_flat_k   = q_iono * _iono_q_factor * _adapt_k
            _q_iono_elev = ION_PROC_NOISE * dt * _iono_q_factor * _adapt_k / (_sel ** 2)
            P[ki, ki] = P[ki, ki] - _q_flat_k + _q_iono_elev  # replace flat with elev-weighted
            P[ki, ki] = min(max(P[ki, ki], 0.0), 50.0)  # cap: 50 m² (~7 m sigma)
            m['ki']=ki; m['NWL']=(wl_fixed.get(sid,None) if not DISABLE_WL_FIXING else None); m['age']=_sat_age[sid]
            geom.append(m)
        ...
        # ── DIAG: [SIG] per-sat signal usage (every 300 epochs) ───────────
        if nproc % 300 == 0:
            _n_fb_ep = 0
            for _m_sig in geom:
                _sid_sig = _m_sig['sid']
                _c1_sig  = _m_sig.get('code1_type', 'C1W' if _sid_sig[0]=='G' else 'C1C')
                _p1_sig  = _m_sig.get('phase1_type', 'L1W' if _sid_sig[0]=='G' else 'L1C')
                _fb_sig  = _m_sig.get('_fallback_used', False)
                _ar_sig  = _m_sig.get('use_for_ar', False)
                _el_sig  = math.degrees(_m_sig['el'])
                # Tier label for GPS: T1=C1W/L1W, T2=C1W/L1C, T3=C1C/L1C
                _c1_sig_d = _m_sig.get('code1_type', 'C1W')
                if _sid_sig[0] == 'G':
                    _tier = 'T1' if (_c1_sig_d=='C1W' and _p1_sig=='L1W') else \
                            'T2' if (_c1_sig_d=='C1W' and _p1_sig=='L1C') else 'T3'
                else:
                    _tier = 'GAL'
                if _fb_sig:
                    _n_fb_ep += 1
                _sw_total = _diag_sig_switch_cnt.get(_sid_sig, 0)
                _c2_sig  = _m_sig.get('code2_type', 'C2W' if _sid_sig[0]=='G' else 'C5Q')
                _p2_sig  = _m_sig.get('phase2_type', 'L2W' if _sid_sig[0]=='G' else 'L5Q')
                print(f"[SIG] SOD={sod:.0f} sat={_sid_sig} "
                      f"code={_c1_sig_d}/{_c2_sig} phase={_p1_sig}/{_p2_sig} "
                      f"tier={_tier} fallback={_fb_sig} ar_eligible={_ar_sig} "
                      f"el={_el_sig:.1f}° total_switches={_sw_total}")
            print(f"[SIG_SUMMARY] SOD={sod:.0f} fallback_count={_n_fb_ep}/{len(geom)}")
        # ── end DIAG ──────────────────────────────────────────────────────

        if nproc==0:
            # First epoch: bootstrap receiver clock from GPS-only pseudoranges.
            # Using GPS-only avoids absorbing ISB_GAL (unknown at epoch 0) into
            # the clock estimate, which would bias ALL ambiguity initializations.
            # Fall back to all satellites only when no GPS is present (Galileo-only).
            _gps_geom = [m for m in geom if m['sid'][0] == 'G']
            _clk_src  = _gps_geom if _gps_geom else geom
            clk_est=float(np.clip(np.mean([m['P1c']-_rp(m,0.,x[IDX_ZWD]) for m in _clk_src]),-3e6,3e6))
            x[IDX_CLK]=clk_est
            # Re-init I, N1, N2 with updated clock estimate.
            # For Galileo, subtract ISB_GAL (0 at epoch 0) from phase terms so
            # the ambiguity init is consistent with the measurement model:
            #   N1 = (L1mc − rp0 + I − ISB) / λ1
            for m in geom:
                ki=m['ki']; rp0=_rp(m,x[IDX_CLK],x[IDX_ZWD]); gam=m['gamma']
                _isb_init = x[IDX_ISB_GAL] if m['sid'][0] == 'E' else 0.0
                I_re=(m['P2c']-m['P1c'])/(gam-1.)
                x[ki]=I_re
                x[ki+1]=(m['L1mc']-rp0+I_re-_isb_init)/m['lam1']
                x[ki+2]=(m['L2mc']-rp0+gam*I_re-_isb_init)/m['lam2']
        ...
        # v121 PART4 accumulators for IONO_INNOV_SPLIT (reset each epoch)
        _ep_code_info_acc = 0.0
        _ep_phase_info_acc = 0.0

        for ri,m in enumerate(geom):
            ki=m['ki']        # I slot
            kn1=ki+1          # N1 slot
            kn2=ki+2          # N2 slot
            kb=sidx_bsat[m['sid']]   # b_sat slot
            u=m['unit']; mw=m['mw']; gam=m['gamma']
            lam1=m['lam1']; lam2=m['lam2']
            is_gal = (m['sid'][0] == 'E')
            rp=_rp(m,xs[IDX_CLK],xs[IDX_ZWD])   # geometry term (single clock)
            I_s=xs[ki]; N1_s=xs[kn1]; N2_s=xs[kn2]
            isb_s = xs[IDX_ISB_GAL] if is_gal else 0.0
            # v111: troposphere gradient correction (all sats)
            _el_m = m['el']; _az_m = m.get('az', 0.0)
            _sel_g = max(math.sin(_el_m), 0.05)
            _cel_g = math.cos(_el_m)
            cot_el  = _cel_g / _sel_g
            cos_az  = math.cos(_az_m); sin_az = math.sin(_az_m)
            _h_gn   = cot_el * cos_az
            _h_ge   = cot_el * sin_az
            ...
            # ── P1 row: P1c = rp + I + ISB(Gal) + grad + b_sat [− BETA·RDCB_G (GPS)] ──
            r=base
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,IDX_CLK]=1.; H[r,IDX_ZWD]=mw
            H[r,ki]=1.
            H[r,IDX_ISB_GAL] = 1. if is_gal else 0.
            H[r,IDX_Gn]=_h_gn; H[r,IDX_Ge]=_h_ge
            H[r,kb]=1.   # b_sat: per-satellite code bias
            # v_RDCB PART 3/4: GPS only — P1 = … − BETA·RDCB_G
            # BETA = F2SQ/(F1SQ−F2SQ); do NOT add for Galileo (different hardware chain)
            if not is_gal:
                H[r, IDX_RDCB_G] = -BETA
                rdcb_corr_P1 = -BETA * xs[IDX_RDCB_G]
            else:
                rdcb_corr_P1 = 0.0
            z[r]=m['P1c']-(rp+I_s+isb_s+grad_s+rdcb_corr_P1+xs[kb])
            # TEST A — arc ramp-in
            ...
            if arc_ramp_n > 0 and _arc_age < arc_ramp_n:
                ...
                _code_scale = 1.0
            # v111: Galileo scale 1.2 (was 1.3); sin²(el) weighting for code
            _cst_scale = 1.2 if is_gal else 1.0
            # Per-signal code noise: GPS C2W (semi-codeless) has ~1.0 m noise;
            # all other signals (C1W, C1C, Galileo) use SC=0.30 m.
            # C1 sigma (P1 row) — same for GPS and Galileo (SC-based)
            _sigma_c1 = SC  # C1W / C1C — clean signal, use SC
            _code_sig_p1 = max(_sig_exp(m['el'], _sigma_c1, 2.0) * _code_scale * _cst_scale, 0.3)
            Rd[r]=_code_sig_p1**2

            # ── P2 row: P2c = rp + γ·I + ISB(Gal) + grad + b_sat [− ALFA·RDCB_G (GPS)] ─
            r=base+1
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,IDX_CLK]=1.; H[r,IDX_ZWD]=mw
            H[r,ki]=gam
            H[r,IDX_ISB_GAL] = 1. if is_gal else 0.
            H[r,IDX_Gn]=_h_gn; H[r,IDX_Ge]=_h_ge
            H[r,kb]=1.   # b_sat: same bias enters P1 and P2 equally
            # v_RDCB PART 3/4: GPS only — P2 = … − ALFA·RDCB_G
            if not is_gal:
                H[r, IDX_RDCB_G] = -ALFA
                rdcb_corr_P2 = -ALFA * xs[IDX_RDCB_G]
            else:
                rdcb_corr_P2 = 0.0
            z[r]=m['P2c']-(rp+gam*I_s+isb_s+grad_s+rdcb_corr_P2+xs[kb])
            # C2 sigma: GPS C2W → 0.5 m (v123 FIX1: 1.0→0.5; IISC Septentrio cleaner)
            # Galileo C5Q uses SC=0.25 m (unchanged).
            _sigma_c2 = SC   # v123: unify GPS P2 noise to SC (was hardcoded 0.5 for GPS, SC for GAL)
                             # GPS L2W multipath ~= L1W at geodetic receivers; 0.5m over-penalized GPS GF
            _code_sig_p2 = max(_sig_exp(m['el'], _sigma_c2, 2.0) * _code_scale * _cst_scale, 0.3)
            Rd[r]=_code_sig_p2**2

            # ── PART 1: startup Rd inflation REMOVED — _in_startup_relax always False ──

            # ── L1 row: L1mc = rp − I + λ1·N1 + ISB(Gal) + grad ─────────
            r=base+2
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,IDX_CLK]=1.; H[r,IDX_ZWD]=mw
            H[r,ki]=-1.
            H[r,kn1]=lam1
            H[r,IDX_ISB_GAL] = 1. if is_gal else 0.
            H[r,IDX_Gn]=_h_gn; H[r,IDX_Ge]=_h_ge
            pred_L1=rp - I_s + lam1*N1_s + isb_s + grad_s
            # v120: OSB now applied in _proc/_proc_gal — L1mc is already bias-free.
            z[r] = m['L1mc'] - pred_L1
            _res_L1 = z[r]   # save pre-gate residual for pair-consistency check
            _SP_cst = SP * _cst_scale
            # v116 FIX1: stable phase noise — (SP/sin(el))² clamped to [0.005², 0.05²]
            # Removes: sin²+0.05 denom, elev-cap, 5× age factor — all caused Rd_phase instability
            _sin_el = max(math.sin(m['el']), 0.1)
            _ph_rd_raw = (_SP_cst / _sin_el) ** 2
            _ph_rd = float(np.clip(_ph_rd_raw, 0.005**2, 0.05**2))

            # [REACQ_FIX] Scale phase noise for Galileo sats in re-acquisition cooldown.
            # phase_scale transitions 5.0 → 1.0 over REACQ_COOLDOWN seconds.
            # This prevents biased freshly-acquired Galileo ambiguities from
            # pulling the combined clock/position solution during the hump window.
            # Code measurements (Rd[base], Rd[base+1]) are NOT touched.
            _reacq_phase_scale = 1.0
            _reacq_in_freeze   = False
            if is_gal and sid in _reacq_state:
                _elapsed_ra = sod - _reacq_state[sid]
                _alpha_ra   = min(1.0, _elapsed_ra / REACQ_COOLDOWN)
                _reacq_phase_scale = 5.0 - 4.0 * _alpha_ra   # 5 → 1
                if _elapsed_ra <= REACQ_AMB_FREEZE:
                    _reacq_in_freeze = True
            # [REACQ_FIX end]

            Rd[r] = _ph_rd * (_reacq_phase_scale ** 2)   # L1 phase variance scaled

            # ── L2 row: L2mc = rp − γ·I + λ2·N2 + ISB(Gal) + grad ───────
            r=base+3
            H[r,0]=-u[0]; H[r,1]=-u[1]; H[r,2]=-u[2]; H[r,IDX_CLK]=1.; H[r,IDX_ZWD]=mw
            H[r,ki]=-gam
            H[r,kn2]=lam2
            H[r,IDX_ISB_GAL] = 1. if is_gal else 0.
            H[r,IDX_Gn]=_h_gn; H[r,IDX_Ge]=_h_ge
            pred_L2=rp - gam*I_s + lam2*N2_s + isb_s + grad_s
            # v120: OSB applied in _proc/_proc_gal — L2mc is already bias-free.
            z[r] = m['L2mc'] - pred_L2
            _res_L2 = z[r]   # save pre-gate residual for pair-consistency check
            Rd[r] = _ph_rd * (_reacq_phase_scale ** 2)   # L2 phase variance scaled

            # [REACQ_FIX] During AMB_FREEZE window: zero the ambiguity columns in
            # the L1 and L2 H rows.  The phase observation still updates position,
            # clock, ZWD, ISB, and ionosphere — but does NOT drive N1/N2 states.
            # This prevents the biased float ambiguity (unmodeled OSB, ~+0.25 cy)
            # from being reinforced by the KF update on the first 300 s of a new arc.
            if _reacq_in_freeze:
                H[base+2, kn1] = 0.0   # L1: remove N1 column
                H[base+3, kn2] = 0.0   # L2: remove N2 column
            # [REACQ_FIX end]

            # v115 FIX3: downweight high-elevation code (el>60°) — code drives iono→vertical
            if m['el'] > math.radians(60.):
                Rd[base]   *= 1.5   # P1
                Rd[base+1] *= 1.5   # P2

            # v121 PART1: GPS-only code ×1.5 extra multiplier REMOVED — restores
            # ionosphere observability from code pseudoranges.
            # [WEIGHT_FIX] gps_extra_scale_removed = True
            pass  # v111 multiplier removed

            # v115 FIX4: Galileo code boost only — phase stable
            if is_gal:
                Rd[base]   *= 0.8   # P1 code
                Rd[base+1] *= 0.8   # P2 code
                # v116 FIX1: NO phase multiplier

            # v116 FIX1: removed elev-bin phase multipliers (was causing Rd_phase instability)

            # ── [A1-INNER] Per-sat: feed res_buf, evaluate metrics, inflate Rd ──
            # This runs AFTER all Rd multipliers are set and BEFORE Fisher info
            # accumulation, so the inflated Rd is what enters CODE_REJECT and
            # filter_standard.  The rej_buf is fed AFTER CODE_REJECT (see below)
            # using the previous epoch's rejection results.
            _a1_sid_i  = m['sid']
            _a1_gal_i  = is_gal

            # Feed Metric C residual buffer with this epoch's P1 prefit
            _a1_res_buf[_a1_sid_i].append(float(z[base]))

            # ── [A1-FAST] Feed fast residual buffer (z[base] only available here) ─
            _a1_fast_res_buf[_a1_sid_i].append(abs(float(z[base])))
            # NOTE: FT1/FT2 evaluation and fast_pend/fast_flagged updates are now
            # performed in [A1-FAST-EVAL] AFTER A1-REJBUF so that consec_rej
            # reflects the CURRENT epoch's CODE_REJECT outcome (not the previous
            # epoch's), eliminating the 1-epoch trigger delay.

            # Evaluate Metric A — rolling std of (L1m − P1c)
            _a1_std_thr_i = _A1_STD_GAL if _a1_gal_i else _A1_STD_GPS
            _a1_lp1_arr_i = _a1_lp1_buf[_a1_sid_i]
            _a1_met_A_i = False
            if len(_a1_lp1_arr_i) >= 30:
                _a1_std_i = float(np.std(_a1_lp1_arr_i))
                _a1_met_A_i = (_a1_std_i > _a1_std_thr_i)

            # Evaluate Metric B — rolling code rejection rate (from previous epochs)
            _a1_rej_arr_i = _a1_rej_buf[_a1_sid_i]
            _a1_met_B_i = False
            if len(_a1_rej_arr_i) >= 20:
                _a1_rej_rate_i = float(sum(_a1_rej_arr_i)) / len(_a1_rej_arr_i)
                _a1_met_B_i = (_a1_rej_rate_i > _A1_REJ_RATE)

            # Evaluate Metric C — sign reversal across two 60-epoch sub-windows
            _a1_res_arr_i = list(_a1_res_buf[_a1_sid_i])
            _a1_met_C_i = False
            if len(_a1_res_arr_i) >= 120:
                _a1_win_old_i = _a1_res_arr_i[:60]
                _a1_win_new_i = _a1_res_arr_i[60:]
                _a1_mean_old_i = float(np.mean(_a1_win_old_i))
                _a1_mean_new_i = float(np.mean(_a1_win_new_i))
                _a1_met_C_i = (
                    abs(_a1_mean_old_i) > _A1_SIGN_MAG
                    and abs(_a1_mean_new_i) > _A1_SIGN_MAG
                    and (_a1_mean_old_i * _a1_mean_new_i) < 0.0
                )

            # Combined flag: long-term (A/B/C) OR fast trigger
            _a1_trigger_i = _a1_met_A_i or _a1_met_B_i or _a1_met_C_i
            _a1_fast_flag_i = _a1_sid_i in _a1_fast_flagged

            # Long-term flag state management (unchanged)
            if _a1_trigger_i:
                _a1_flag_age[_a1_sid_i] += 1
                _a1_flagged.add(_a1_sid_i)
            else:
                if _a1_sid_i in _a1_flagged:
                    if _a1_flag_age[_a1_sid_i] >= _A1_PERSIST:
                        _a1_flagged.discard(_a1_sid_i)
                        _a1_flag_age[_a1_sid_i] = 0
                    else:
                        _a1_flag_age[_a1_sid_i] += 1
                else:
                    _a1_flag_age[_a1_sid_i] = 0

            # CODE_SUSPECT = long-term flag OR fast flag
            _a1_code_suspect_i = (_a1_sid_i in _a1_flagged) or _a1_fast_flag_i

            # Apply Rd inflation for CODE_SUSPECT satellites
            # Phase rows (base+2, base+3) are NEVER touched.
            if _a1_code_suspect_i:
                _a1_sigma_sq_i = _A1_SIGMA_MUL ** 2   # 9.0 at sigma_mul=3.0
                _a1_old_p1 = math.sqrt(float(Rd[base]))
                _a1_old_p2 = math.sqrt(float(Rd[base+1]))
                Rd[base]   *= _a1_sigma_sq_i   # P1
                Rd[base+1] *= _a1_sigma_sq_i   # P2
                # [A1_APPLY] debug — prints every epoch for flagged sats
                _a1_reason_i = ("FAST" if _a1_fast_flag_i and not (_a1_sid_i in _a1_flagged)
                                else "LONG" if _a1_sid_i in _a1_flagged else "FAST+LONG")
                print(f"[A1_APPLY] sat={_a1_sid_i} SOD={sod:.0f} "
                      f"sigma_old_P1={_a1_old_p1:.3f}m "
                      f"sigma_new_P1={math.sqrt(float(Rd[base])):.3f}m "
                      f"met_A={_a1_met_A_i}(std={float(np.std(_a1_lp1_arr_i)) if len(_a1_lp1_arr_i)>=30 else 0:.2f}m) "
                      f"met_B={_a1_met_B_i} "
                      f"met_C={_a1_met_C_i} "
                      f"fast={_a1_fast_flag_i} reason={_a1_reason_i}")

            # [CODE_SUSPECT] summary log every 300 epochs for any suspect sat
            if _a1_code_suspect_i and nproc % 300 == 0:
                _a1_std_disp  = float(np.std(_a1_lp1_arr_i)) if len(_a1_lp1_arr_i) >= 30 else float('nan')
                _a1_rate_disp = (float(sum(_a1_rej_arr_i))/len(_a1_rej_arr_i)
                                 if len(_a1_rej_arr_i) >= 20 else float('nan'))
                _a1_reason_log = ("FAST" if _a1_fast_flag_i and not (_a1_sid_i in _a1_flagged)
                                  else "LONG" if _a1_sid_i in _a1_flagged else "FAST+LONG")
                print(f"[CODE_SUSPECT] sat={_a1_sid_i} SOD={sod:.0f} "
                      f"std={_a1_std_disp:.2f}m "
                      f"rej_rate={_a1_rate_disp:.3f} "
                      f"sign_flip={_a1_met_C_i} "
                      f"flag_age={_a1_flag_age[_a1_sid_i]} "
                      f"reason={_a1_reason_log}")
            # ── [A1-INNER end] ────────────────────────────────────────────────

            # v121 PART4: accumulate Fisher info contribution for ionosphere state (H_iono = ±1 or ±γ)
            if Rd[base]   > 0: _ep_code_info_acc  += 1.0 / Rd[base]    # P1: H_iono=+1
            if Rd[base+1] > 0: _ep_code_info_acc  += (gam**2) / Rd[base+1]  # P2: H_iono=+γ
            if Rd[base+2] > 0: _ep_phase_info_acc += 1.0 / Rd[base+2]   # L1: H_iono=-1 → H²=1
            if Rd[base+3] > 0: _ep_phase_info_acc += (gam**2) / Rd[base+3]  # L2: H_iono=-γ → H²=γ²

            # v115 FIX5: accumulate geometry diagnostics
            _el_deg_dbg = math.degrees(m['el'])
            if is_gal:
                _dbg_el_E.append(_el_deg_dbg); _dbg_rdph_E.append(Rd[base+2])
            else:
                _dbg_el_G.append(_el_deg_dbg); _dbg_rdph_G.append(Rd[base+2])

            # v111: Galileo phase rejection DISABLED — allow all phase into filter
            # (was: hard reject if |res_L1|>0.10 or |res_L2|>0.10)
            _bad_L1 = False
            _bad_L2 = False
            if _bad_L1 or _bad_L2:
                # Zero the entire H rows — contribution to H^T R^-1 H and
                # H^T R^-1 z becomes exactly zero regardless of Rd.
                H[base+2, :] = 0.0
                H[base+3, :] = 0.0
                z[base+2]    = 0.0
                z[base+3]    = 0.0
                if is_gal and abs(_res_L1) > 0.2:
                    print(f"[GAL PHASE REJECTED] {m['sid']} L1={_res_L1:.3f} L2={_res_L2:.3f}")

            # v93 Run-F: subdaily vertical absorber
            if enable_subdaily_absorber:
                _h_vl = -float(np.dot(u, _e_up_sda))
                _sda_col = IDX_Gn - 1   # subdaily state sits immediately before Gn
            ...
            if _rng_sat_rd > 8.0:
                # code rows only — phase rows are hard-rejected above if corrupt;
                # do NOT soften/downweight phase here (bad phase must be removed, not tuned)
                Rd[base]   *= 4.0
                Rd[base+1] *= 4.0
            ...
            # v117 FIX5: use detrended position Up error instead of phase prefit innovation.
            # 1-epoch lag: _up_err_hist contains Up errors up to the previous epoch.
            # Causal rolling window (last _UP_DETREND_WIN epochs) for local mean.
            if _up_err_hist:
                _win_sl = _up_err_hist[max(0, len(_up_err_hist) - _UP_DETREND_WIN):]
                _up_local_mean_m = float(np.mean(_win_sl)) * 1e-3  # mm → m
                _up_raw_m = _up_err_hist[-1] * 1e-3               # last epoch Up, mm → m
            else:
                _up_local_mean_m = 0.0
                _up_raw_m = 0.0
            _up_detrended_m = _up_raw_m - _up_local_mean_m
            _up_contrib  = _up_detrended_m ** 2  # m² detrended Up-position error
        ...
        # ── [CODE_REJECT] 5.5σ code prefit gate ──────────────────────────────
        # Applied AFTER all Rd multipliers (GPS/GAL boost, scintillation, etc.)
        # and BEFORE filter_standard.  Phase rows are NEVER touched.
        # Rejection = inflate Rd to 1e6 m² (soft: measurement still enters H).
        # v123: gate widened 3.0σ → 5.5σ to allow moderate residuals (3–6 m)
        #       to enter filter so ionosphere states can converge.
        #       Hard cap at 20 m prevents true outliers from entering.
        _CODE_GATE_K   = 5.5   # [CODE_GATE] widened from 3.0 to 5.5
        _CODE_HARD_CAP = 20.0  # [CODE_GATE] absolute hard cap (m)
        if nproc == 1:
            print(f"[CODE_GATE] K={_CODE_GATE_K}  hard_cap={_CODE_HARD_CAP}m")
        for _cr_ri, _cr_m in enumerate(geom):
            _cr_base = 4 * _cr_ri
            _cr_sid  = _cr_m['sid']
            for _cr_off, _cr_label in ((0, 'P1'), (1, 'P2')):
                _cr_row = _cr_base + _cr_off
                _cr_total += 1
                _cr_res   = float(z[_cr_row])
                _cr_sigma = math.sqrt(float(Rd[_cr_row]))
                _cr_abs   = abs(_cr_res)
                # Hard cap: always reject extreme outliers regardless of sigma
                _cr_hard_reject = _cr_abs > _CODE_HARD_CAP
                # Soft gate: widened to 5.5σ
                _cr_soft_reject = (_cr_sigma > 0.0 and _cr_abs > _CODE_GATE_K * _cr_sigma)
                if _cr_hard_reject or _cr_soft_reject:
                    Rd[_cr_row] = 1e6          # inflate — phase untouched
                    _cr_rejected += 1
                    print(f"[CODE_REJECT] SOD={sod:.0f} sat={_cr_sid} "
                          f"sig={_cr_label} residual={_cr_res:+.3f}m "
                          f"sigma={_cr_sigma:.3f}m "
                          f"ratio={_cr_abs/_cr_sigma:.1f}σ")
                elif _cr_sigma > 0.0:          # accumulate accepted code ratio
                    _cr_ratio_sum += _cr_abs / _cr_sigma
                    _cr_ratio_cnt += 1
        # ── [CODE_REJECT_STATS] summary every 300 epochs ─────────────────────
        if nproc > 0 and nproc % 300 == 0:
            _cr_ratio = (_cr_rejected / _cr_total) if _cr_total > 0 else 0.0
            print(f"[CODE_REJECT_STATS] SOD={sod:.0f} "
                  f"total_code={_cr_total} "
                  f"rejected={_cr_rejected} "
                  f"ratio={_cr_ratio:.4f}")
            # [CODE_USAGE] — tracks code influence on filter
            _avg_cr_ratio = (_cr_ratio_sum / _cr_ratio_cnt) if _cr_ratio_cnt > 0 else float('nan')
            _code_pct = ((_cr_total - _cr_rejected) / _cr_total * 100.) if _cr_total > 0 else 0.
            print(f"[CODE_USAGE] SOD={sod:.0f}  "
                  f"total_code_obs={_cr_total}  "
                  f"rejected_code_obs={_cr_rejected}  "
                  f"rejection_ratio={_cr_ratio:.4f}  "
                  f"avg_code_residual_over_sigma={_avg_cr_ratio:.3f}  "
                  f"code_pct={_code_pct:.2f}%")
        # ── [CODE_REJECT end] ─────────────────────────────────────────────────

        # ── [A1-REJBUF] Record this epoch's CODE_REJECT outcomes for Metric B ──
        # Flag evaluation and Rd inflation already ran inside the per-sat loop
        # (A1-INNER) using previous-epoch rejection history.  Record current
        # epoch's outcomes so they are available from next epoch onward.
        for _a1_ri_rb, _a1_m_rb in enumerate(geom):
            _a1_sid_rb  = _a1_m_rb['sid']
            _a1_base_rb = 4 * _a1_ri_rb
            _a1_ep_rejected_rb = False
            for _a1_off_rb in (0, 1):   # P1 then P2
                _a1_row_rb = _a1_base_rb + _a1_off_rb
                _a1_is_rej = (float(Rd[_a1_row_rb]) >= 1e5)
                _a1_rej_buf[_a1_sid_rb].append(1 if _a1_is_rej else 0)
                if _a1_is_rej:
                    _a1_ep_rejected_rb = True
            # [A1-FAST] update consecutive reject counter (gradual decay when clean)
            if _a1_ep_rejected_rb:
                _a1_consec_rej[_a1_sid_rb] += 1
            else:
                # Gradual decay: decrement by 1 (not instant reset) for stability
                _a1_consec_rej[_a1_sid_rb] = max(0, _a1_consec_rej[_a1_sid_rb] - 1)
        # ── [A1-REJBUF end] ───────────────────────────────────────────────────

        # ── [A1-FAST-EVAL] Fast trigger evaluation AFTER consec_rej update ──
        # Moved here from A1-INNER so that FT1 reads the CURRENT epoch's
        # rejection outcome (set by A1-REJBUF above) rather than the previous
        # epoch's outcome, eliminating the 1-epoch trigger delay.
        for _a1_ri_fe, _a1_m_fe in enumerate(geom):
            _a1_sid_fe  = _a1_m_fe['sid']

            # FT1: consecutive CODE_REJECT epochs (current epoch result now visible)
            _a1_ft1_fe = (_a1_consec_rej[_a1_sid_fe] >= _A1_FAST_CONSEC)

            # FT2: ≥3 of last 5 epochs exceed residual threshold
            _a1_fast_res_win_fe = list(_a1_fast_res_buf[_a1_sid_fe])
            _a1_ft2_fe = False
            _a1_high_count_fe = 0
            if len(_a1_fast_res_win_fe) >= 5:
                _a1_high_count_fe = sum(
                    1 for _r in _a1_fast_res_win_fe[-5:] if _r > _A1_FAST_RES_THR)
                _a1_ft2_fe = (_a1_high_count_fe >= 3)

            # Activation delay: require _A1_FAST_PEND consecutive pending epochs
            _a1_raw_fast_fe = _a1_ft1_fe or _a1_ft2_fe
            if _a1_raw_fast_fe:
                _a1_fast_pend[_a1_sid_fe] += 1
            else:
                # Gradual decay of pending counter (not instant reset)
                _a1_fast_pend[_a1_sid_fe] = max(0, _a1_fast_pend[_a1_sid_fe] - 1)

            _a1_fast_active_fe = (_a1_fast_pend[_a1_sid_fe] >= _A1_FAST_PEND)

            # Manage fast_flagged state with minimum persistence
            if _a1_fast_active_fe:
                _a1_fast_flagged.add(_a1_sid_fe)
                _a1_fast_age[_a1_sid_fe] += 1
            else:
                if _a1_sid_fe in _a1_fast_flagged:
                    if _a1_fast_age[_a1_sid_fe] >= _A1_FAST_PERSIST:
                        _a1_fast_flagged.discard(_a1_sid_fe)
                        _a1_fast_age[_a1_sid_fe] = 0
                    else:
                        _a1_fast_age[_a1_sid_fe] += 1  # hold for minimum duration
                else:
                    _a1_fast_age[_a1_sid_fe] = 0

            # Debug: confirms ordering — consec_rej is CURRENT epoch's value
            if _a1_ft1_fe or _a1_ft2_fe:
                print(f"[A1_FAST_EVAL] sat={_a1_sid_fe} SOD={sod:.0f} "
                      f"consec_rej={_a1_consec_rej[_a1_sid_fe]} "
                      f"high_res_count={_a1_high_count_fe} "
                      f"pend={_a1_fast_pend[_a1_sid_fe]} "
                      f"flagged={'Y' if _a1_sid_fe in _a1_fast_flagged else 'N'}")
        # ── [A1-FAST-EVAL end] ────────────────────────────────────────────────
        ...
        # v106 PART 3: balanced constellation normalization.
        # Prevents one constellation from dominating when numbers differ.
        # Only applied when both GPS and Galileo are present in geom.
        _nG_cn = sum(1 for _m_cn in geom if _m_cn['sid'][0] == 'G')
        _nE_cn = sum(1 for _m_cn in geom if _m_cn['sid'][0] == 'E')
        if _nG_cn > 0 and _nE_cn > 0:
            _tot_cn = float(_nG_cn + _nE_cn)
            _scale_G = _tot_cn / (2.0 * _nG_cn)
            _scale_E = _tot_cn / (2.0 * _nE_cn)
            for _ri_cn, _m_cn in enumerate(geom):
                _b_cn = 4 * _ri_cn
                _s_cn = _scale_G if _m_cn['sid'][0] == 'G' else _scale_E
                for _rr_cn in range(_b_cn, _b_cn + 4):
                    Rd[_rr_cn] *= _s_cn
        ...
        _gf_z_rows = []
        _gf_R_diag = []
        _gf_rejected_count = 0
        if USE_GF:
            for _gf_ri, _gf_m in enumerate(geom):
                _gf_ki  = _gf_m['ki']
                _gf_gam = _gf_m['gamma']
                _gf_dg  = _gf_gam - 1.0
                if _gf_dg < 0.1:            # degenerate: skip if γ≈1
                    _gf_rejected_count += 1
                    continue
                _gf_obs   = _gf_m['P2c'] - _gf_m['P1c']     # measured GF (code)
                _gf_pred  = _gf_dg * xs[_gf_ki]              # predicted from PRE-UPDATE state xs
                _gf_innov = _gf_obs - _gf_pred
                _gf_base  = 4 * _gf_ri
                _gf_sig2  = GF_CODE_SCALE * (Rd[_gf_base] + Rd[_gf_base + 1])
                _gf_sig2  = min(_gf_sig2, 1.5**2)   # v122 PART2: cap GF sigma at 1.5 m
                if abs(_gf_innov) > GF_GATE * math.sqrt(_gf_sig2):
                    _gf_rejected_count += 1
                    continue                # outlier gate
                _h_gf = np.zeros(nst)
                _h_gf[_gf_ki] = _gf_dg    # H[iono_state] = (gamma-1); all other entries = 0
                _gf_H_rows.append(_h_gf)
                _gf_z_rows.append(_gf_innov)
                _gf_R_diag.append(_gf_sig2)
        _gf_used_count = len(_gf_H_rows)
        # ── end GF row building ────────────────────────────────────────────────

        # ── IONEX pseudo-measurements (v124) — external iono constraint ───────
        # One pseudo-obs per visible satellite: z = I_slant_model, H[ki]=1.
        # PART 3: sigma_iono tightened from 3.0 m → 0.7 m (stronger constraint).
        # R_ionex = sigma_iono^2 = 0.49 m^2 applied per satellite iono state only.
        # Applied BEFORE filter_standard, after all Rd decisions.
        # DO NOT touch code/phase rows, OSB, b_sat, or DCB values.
        _IONEX_SIGMA = 0.7       # metres  (PART 3: 3.0 → 0.7 m)
        _ionex_H_rows = []
        _ionex_z_rows = []
        _ionex_R_diag = []
        _ionex_model_vals = {}   # sid → I_slant_model (for debug)
        if ionex_vtec is not None:
            _rx_now = nom + x[:3]   # current ECEF estimate
            for _ix_m in geom:
                _ix_ki  = _ix_m['ki']
                _ix_el  = float(_ix_m['el'])
                _ix_az  = float(_ix_m.get('az', 0.0))
                _ix_sid = _ix_m['sid']
                if _ix_el < math.radians(5.0):   # skip very low elevations
                    continue
                try:
                    _ipp_lat, _ipp_lon = _ionex_ipp_latlon(
                        _rx_now, _ix_el, _ix_az)
                    _I_slant = _ionex_vtec_slant(
                        ionex_vtec, sod, _ipp_lat, _ipp_lon, _ix_el)
                except Exception:
                    _I_slant = None
                if _I_slant is None or not math.isfinite(_I_slant):
                    continue
                if _I_slant < 0.0 or _I_slant > 200.0:  # sanity gate
                    continue
                _h_ix = np.zeros(nst)
                _h_ix[_ix_ki] = 1.0
                _innov_ix = _I_slant - float(x[_ix_ki])
                _ionex_H_rows.append(_h_ix)
                _ionex_z_rows.append(_innov_ix)
                _ionex_R_diag.append(_IONEX_SIGMA ** 2)
                _ionex_model_vals[_ix_sid] = _I_slant
        _n_ionex = len(_ionex_H_rows)
        # ── end IONEX row building ─────────────────────────────────────────────

        # Combine: main obs (n_obs rows) + ZWD prior (1 row) + GF rows (0 when USE_GF=False) + IONEX rows
        _n_extra = 1 + _gf_used_count + _n_ionex   # ZWD prior + GF + IONEX
        ...
        # IONEX rows appended after ZWD prior + GF rows
        for _ix_i, (_ix_h, _ix_z, _ix_r) in enumerate(
                zip(_ionex_H_rows, _ionex_z_rows, _ionex_R_diag)):
            _row_idx = n_total + 1 + _gf_used_count + _ix_i
            H_p[_row_idx, :]  = _ix_h
            z_p[_row_idx]     = _ix_z
            Rd_p[_row_idx]    = _ix_r
        ...
        # ── IONO LAG DIAG PART 2: measurement-based iono ────────────────────
        # I_meas = (P2c - P1c) / (gamma - 1)  — geometry-free pseudorange
        # Computed from the same m dict that feeds the KF so units are identical.
        _I_meas_ep = {}
        for _m_ld in geom:
            _gam_ld = _m_ld['gamma']
            if _gam_ld != 1.0:
                _I_meas_ep[_m_ld['sid']] = (_m_ld['P2c'] - _m_ld['P1c']) / (_gam_ld - 1.0)
        # ── end IONO LAG DIAG PART 2 ─────────────────────────────────────────
        ...
        for _sid_ai, _I_now_ai in _I_meas_ep.items():
            _iono_prev_I_meas[_sid_ai] = _I_now_ai  # diagnostic only
        # ── end ───────────────────────────────────────────────────────────────
        ...
        # v123 FIX4: state-based adaptive Q (post-update). Replaces code-based dI
        # which was noise-dominated (C2W ~0.9m/ep → scale=20×→ hump).
        _EMA_DI = 0.3
        for _m_aq in geom:
            _sid_aq = _m_aq['sid']
            _I_post  = float(x[_m_aq['ki']])
            _I_prv   = _iono_prev_state_adapt.get(_sid_aq, _I_post)
            _dI_st   = abs(_I_post - _I_prv)
            _iono_dI_adapt[_sid_aq] = (1.0-_EMA_DI)*_iono_dI_adapt[_sid_aq] + _EMA_DI*_dI_st
            _iono_prev_state_adapt[_sid_aq] = _I_post

        # Re-apply iono variance cap after combined update (GF now part of main update)
        for _gf_m2 in geom:
            P[_gf_m2['ki'], _gf_m2['ki']] = min(
                P[_gf_m2['ki'], _gf_m2['ki']], 50.0)

        # GF_DEBUG: report GF usage every 300 epochs (Part 6)
        if nproc > 0 and nproc % 300 == 0:
            _gf_mean_sig = (
                math.sqrt(sum(_gf_R_diag) / len(_gf_R_diag))
                if _gf_R_diag else 0.0)
            print(f"[GF_DEBUG] SOD={sod:.0f}  "
                  f"used_rows={_gf_used_count}  "
                  f"rejected_rows={_gf_rejected_count}  "
                  f"mean_sigma={_gf_mean_sig:.3f}m")

        # IONEX constraint debug — every 300 epochs
        if nproc > 0 and nproc % 300 == 0 and ionex_vtec is not None:
            for _dbg_m in geom:
                _dbg_sid = _dbg_m['sid']
                _dbg_ki  = _dbg_m['ki']
                _dbg_mod = _ionex_model_vals.get(_dbg_sid)
                if _dbg_mod is None:
                    continue
                _dbg_est  = float(x[_dbg_ki])
                _dbg_diff = _dbg_mod - _dbg_est
                print(f"[IONEX] sat={_dbg_sid} "
                      f"I_model={_dbg_mod:.3f}m "
                      f"I_est={_dbg_est:.3f}m "
                      f"diff={_dbg_diff:+.3f}m")

        # ── PART 5: per-epoch accumulation for CODE_USAGE / IONO_DIAG / PC_DIFF / IONEX ──
        # CODE_USAGE: count code rows (P1+P2 = 2 per sat) vs total rows in combined H_p
        _ep_code_rows = 2 * len(geom)
        _ep_obs_rows  = len(z_p)                 # all rows: code + phase + ZWD + IONEX
        _diag5_code_cnt += _ep_code_rows
        _diag5_obs_cnt  += _ep_obs_rows

        # IONO_DIAG: collect I and N1 states for corr(I, N1)
        for _d5m in geom:
            _d5_ki = _d5m['ki']
            _d5_I  = float(x[_d5_ki])
            _d5_N1 = float(x[_d5_ki + 1])
            if math.isfinite(_d5_I) and math.isfinite(_d5_N1):
                _diag5_I_buf.append(_d5_I)
                _diag5_N1_buf.append(_d5_N1)

        # PC_DIFF: first 10 min (600 s) — collect |L1m - P1c|
        if _diag5_sod_start_ref is None:
            _diag5_sod_start_ref = sod
        if (sod - _diag5_sod_start_ref) <= 600.0:
            for _d5m in geom:
                _d5_l1m = _d5m.get('L1m', float('nan'))
                _d5_p1c = _d5m.get('P1c', float('nan'))
                if math.isfinite(_d5_l1m) and math.isfinite(_d5_p1c):
                    _diag5_pc_diff_10min.append(abs(_d5_l1m - _d5_p1c))

        # IONEX avg_diff: |I_model - I_est| per satellite
        for _d5s, _d5mod in _ionex_model_vals.items():
            _d5ki = sidx.get(_d5s)
            if _d5ki is not None and math.isfinite(_d5mod):
                _d5est = float(x[_d5ki])
                if math.isfinite(_d5est):
                    _diag5_ionex_diff_buf.append(abs(_d5mod - _d5est))
        # ── end PART 5 accumulation ────────────────────────────────────────────────
        ...
        # v121 FIX: tightened epoch-to-epoch clock constraint 0.05->0.02 m
        # Consistent with reduced Q: allows ~2 mm/epoch drift, not 5 cm jumps.
        if _clk_prev is not None:
            _H_clk = np.zeros((1, len(x))); _H_clk[0, IDX_CLK] = 1.0
            _z_clk = np.array([_clk_prev - x[IDX_CLK]])
            _R_clk = np.array([[0.02**2]])  # v121: 0.05->0.02 m, matches tighter Q
        ...
        if nproc % 300 == 0:
            ...
            # v_RDCB PART 8: RDCB debug — expected range 2–6 m (GPS C1W−C2W bias)
            print(f"[RDCB] SOD={sod:.0f}  RDCB_G={x[IDX_RDCB_G]:+.4f}m  "
                  f"sigma={math.sqrt(P[IDX_RDCB_G,IDX_RDCB_G]):.4f}m")
        if nproc % 300 == 0 and _iono_adapt_scale:
            _scales_v = list(_iono_adapt_scale.values())
            _top5 = sorted(_iono_adapt_scale.items(), key=lambda kv: -kv[1])[:5]
            print(f"[IONO_Q_ADAPT] SOD={sod:.0f}  n={len(_scales_v)}  "
                  f"mean_scale={sum(_scales_v)/len(_scales_v):.2f}  "
                  f"max_scale={max(_scales_v):.2f}  "
                  f"top5={[(s, f'{sc:.1f}') for s, sc in _top5]}")

        # ── [B_SAT] per-satellite code bias diagnostics (every 300 epochs) ──
        if nproc % 300 == 0 and sidx_bsat:
            for _bs_sid, _bs_kb in sorted(sidx_bsat.items()):
                if _bs_sid in sidx:
                    _bs_bias  = float(x[_bs_kb])
                    _bs_sigma = math.sqrt(max(0.0, float(P[_bs_kb, _bs_kb])))
                    print(f"[B_SAT] sat={_bs_sid} bias={_bs_bias:+.4f}m sigma={_bs_sigma:.4f}m")
        ...
        IONO_RW_CAP   = 50.0   # m/epoch — v112: very loose, iono tracks freely
        IONO_INSTAB_T = 50.0   # m — only flag truly extreme jumps
        _iono_unstable = set()
        for _m_c in geom:
            _sid_c = _m_c['sid']
            _ki_c  = _m_c['ki']
            ...
            # RW cap: only block truly non-physical jumps
            if _dI > IONO_RW_CAP:
            ...
            # (discard KF update — iono propagates by process noise only)
                ...
                x[_ki_c] = _I_old
            # v112: hard iono variance cap removed — no over-constraint on iono state
            if _dI > IONO_INSTAB_T:
                _iono_unstable.add(_sid_c)
            # v112: iono damping — gently pull state toward zero each epoch
            # prevents slow secular drift; factor 0.995 ≡ τ≈200 epochs at 30s
            x[_ki_c] *= 0.995

        # [IONO VAR] / [GPS_SUMMARY] → removed (debug data in CSVs)

        # ── IONO LAG DIAG PARTS 1+2+3: record per-epoch state & meas ────────
        if _iono_diag_label is None:
            _iono_diag_label = label
        for _m_ld2 in geom:
            _sid_ld2 = _m_ld2['sid']
            _I_st_ld2 = float(x[_m_ld2['ki']])
            _I_ms_ld2 = _I_meas_ep.get(_sid_ld2, float('nan'))
            _iono_state_hist[_sid_ld2].append(_I_st_ld2)
            _iono_meas_hist[_sid_ld2].append(_I_ms_ld2)
            _iono_sod_hist[_sid_ld2].append(float(sod))
        # ── end IONO LAG DIAG recording ───────────────────────────────────────
        ...
        for m in geom:
            ...
            res_L1=m['L1mc']-(rp - x[ki] + m['lam1']*x[ki+1] + _isb_ph)
        ...
        for _ri_dr, _m_dr in enumerate(geom):
            ...
            _res_co = _m_dr['P1c'] - (_rp(_m_dr, x[IDX_CLK], x[IDX_ZWD]) + x[_m_dr['ki']] + _isb_diag)
            if _m_dr['sid'][0] == 'G':
        ...
        # ── DIAG: [INFLUENCE] per-sat position contribution (every 900 ep) ─
        # Proxy: norm of H_i[:3]^T * (z_i / Rd_i) — phase rows only.
        # Identifies which satellites are currently driving position updates.
        if nproc % 900 == 0 and len(geom) >= 4:
            for _ri_inf, _m_inf in enumerate(geom):
                _b_inf = 4 * _ri_inf
                # Phase rows: base+2 (L1), base+3 (L2)
                _h_L1 = H[_b_inf + 2, :3]  # position columns only
                _h_L2 = H[_b_inf + 3, :3]
                _z_L1 = z[_b_inf + 2]
                _z_L2 = z[_b_inf + 3]
                _r_L1 = max(Rd[_b_inf + 2], 1e-12)
                _r_L2 = max(Rd[_b_inf + 3], 1e-12)
                _infl = float(np.linalg.norm(
                    _h_L1 * (_z_L1 / _r_L1) + _h_L2 * (_z_L2 / _r_L2)))
                _diag_influence_sat[_m_inf['sid']] = _infl
            # Print sorted influence table
            _infl_sorted = sorted(_diag_influence_sat.items(), key=lambda kv: kv[1], reverse=True)
            print(f"[INFLUENCE] SOD={sod:.0f} — per-sat position influence (phase prefit, descending):")
            for _sid_inf, _infl_val in _infl_sorted[:15]:
                _sys_inf = 'GPS' if _sid_inf[0] == 'G' else 'GAL'
                print(f"  [INFLUENCE] sat={_sid_inf} sys={_sys_inf} influence_norm={_infl_val:.4f}")
        # ── end DIAG ──────────────────────────────────────────────────────
        ...
        for m in geom:
            ...
            # v119 FRAC_CHECK: log before/after OSB effect every 300 epochs on Galileo.
            # With proper OSB applied, raw_frac should drift toward 0 as filter converges.
            # If raw_frac stays at ~+0.2 cyc → OSB missing for this satellite.
            if nproc % 300 == 0 and abs(raw_frac) > 0.05:
                print(f"[FRAC_CHECK] sat={sid_m} "
                      f"raw_frac={raw_frac:+.4f} bias={bias_m:+.4f} "
                      f"corr_frac={corr_frac:+.4f} sigma={sigma_N1_m:.3f}m "
                      f"buf_n={len(buf)}")
        ...
        if nl_fixed and len(geom) >= 4:
            ...
            if nl_pairs:
                ...
                for _nl_iter in range(2):
                    ...
                    if _ret != 0:
                        break
                    # v112: iono variance cap after NL update removed — no over-constraint
                    # (no P[ki,ki] min applied here)
        ...
        # FIX3: code_res computed HERE (same geom as phase_res_now) to guarantee
        # len(code_res) == len(geom) == len(phase_res_now) at the warn check below.
        # Previously code_res was computed at end-of-loop after results{} dict,
        # causing the warn check to compare current geom against previous epoch's code_res.
        code_res=[m['P1c']-(  _rp(m,x[IDX_CLK],x[IDX_ZWD]) + x[m['ki']]  ) for m in geom]
        assert len(code_res) == len(geom), \
            f"[ASSERT] code_res({len(code_res)}) != geom({len(geom)}) — geometry mismatch"

        # ── DIAG: [OSB_CHECK] raw vs corrected per sat every 300 epochs ──────
        if nproc % 300 == 0:
            for _m_oc in geom:
                _sid_oc  = _m_oc['sid']
                _raw_P1  = _m_oc.get('P1', float('nan'))
                _cor_P1  = _m_oc.get('P1c', float('nan'))
                _raw_L1m = _m_oc.get('L1m', float('nan'))  # lam*L1, OSB corrected
                _raw_L1  = _m_oc.get('L1', float('nan'))   # raw cycles
                _lam1_oc = _m_oc.get('lam1', 0.1903)
                _raw_L1m_uncor = _raw_L1 * _lam1_oc        # without OSB
                _applied_code_bias = _raw_P1 - _cor_P1
                _applied_phase_bias = _raw_L1m_uncor - _raw_L1m
                _c1_oc = _m_oc.get('code1_type', '?')
                _p1_oc = _m_oc.get('phase1_type', '?')
                print(f"[OSB_CHECK] sat={_sid_oc} sig={_c1_oc}/{_p1_oc} "
                      f"raw_P1={_raw_P1:+.3f}m corr_P1={_cor_P1:+.3f}m "
                      f"applied_code_osb={_applied_code_bias:+.4f}m "
                      f"raw_L1m={_raw_L1m_uncor:+.3f}m corr_L1m={_raw_L1m:+.3f}m "
                      f"applied_phase_osb={_applied_phase_bias:+.4f}m")

        # ── DIAG: [CONSISTENCY] L1m-P1c mean/std per sat every 300 epochs ───
        if nproc % 300 == 0:
            print(f"[CONSISTENCY] SOD={sod:.0f} — per-sat L1m−P1c (code-phase consistency):")
            for _sid_cp in sorted(_diag_lp1_sat.keys()):
                _hist_cp = _diag_lp1_sat[_sid_cp]
                if len(_hist_cp) < 5:
                    continue
                _arr_cp  = np.array(_hist_cp)
                _mn_cp   = float(np.mean(_arr_cp))
                _sd_cp   = float(np.std(_arr_cp))
                _drift   = float(_arr_cp[-1] - _arr_cp[0]) if len(_arr_cp) > 1 else 0.0
                _sys_cp  = 'GPS' if _sid_cp[0] == 'G' else 'GAL'
                print(f"  [CONSISTENCY] sat={_sid_cp} sys={_sys_cp} n={len(_hist_cp)} "
                      f"mean={_mn_cp:+.4f}m std={_sd_cp:.4f}m drift={_drift:+.4f}m")

        # ── DIAG: [RES_GPS] / [RES_GAL] mean (not just RMS) residuals ────────
        if nproc % 300 == 0 and (_diag_res_gps_phase or _diag_res_gal_phase):
            _win_gps_ph = _diag_res_gps_phase[-300*9:]  # last ~300 ep × up to 9 sats
            _win_gal_ph = _diag_res_gal_phase[-300*6:]
            _win_gps_co = _diag_res_gps_code[-300*9:]
            _win_gal_co = _diag_res_gal_code[-300*6:]
            def _safe_stat(v):
                a = np.array(v)
                return float(np.mean(a))*1e3, float(np.std(a))*1e3
            _gps_ph_mn, _gps_ph_sd = _safe_stat(_win_gps_ph) if _win_gps_ph else (float('nan'), float('nan'))
            _gal_ph_mn, _gal_ph_sd = _safe_stat(_win_gal_ph) if _win_gal_ph else (float('nan'), float('nan'))
            _gps_co_mn, _gps_co_sd = _safe_stat(_win_gps_co) if _win_gps_co else (float('nan'), float('nan'))
            _gal_co_mn, _gal_co_sd = _safe_stat(_win_gal_co) if _win_gal_co else (float('nan'), float('nan'))
            print(f"[RES_GPS] SOD={sod:.0f} phase_mean={_gps_ph_mn:+.1f}mm "
                  f"phase_std={_gps_ph_sd:.1f}mm "
                  f"code_mean={_gps_co_mn:+.1f}mm code_std={_gps_co_sd:.1f}mm")
            print(f"[RES_GAL] SOD={sod:.0f} phase_mean={_gal_ph_mn:+.1f}mm "
                  f"phase_std={_gal_ph_sd:.1f}mm "
                  f"code_mean={_gal_co_mn:+.1f}mm code_std={_gal_co_sd:.1f}mm")
            if abs(_gps_ph_mn - _gal_ph_mn) > 50:   # >50mm systematic difference
                print(f"[RES_BIAS_ALERT] GPS-GAL phase mean difference = "
                      f"{_gps_ph_mn - _gal_ph_mn:+.1f}mm — systematic bias suspected")
        # ── end DIAG ─────────────────────────────────────────────────────────

        # ── PART 5: four mandatory diagnostics every 300 epochs ──────────────────
        if nproc > 0 and nproc % 300 == 0:
            # 1. CODE_USAGE
            _d5_code_pct = (100.0 * _diag5_code_cnt / _diag5_obs_cnt
                            if _diag5_obs_cnt > 0 else 0.0)
            print(f"[CODE_USAGE] SOD={sod:.0f}  code_pct={_d5_code_pct:.2f}%")
            _diag5_code_cnt = 0
            _diag5_obs_cnt  = 0

            # 2. IONO_DIAG — corr(I, N1) over accumulated window
            if len(_diag5_I_buf) >= 10:
                try:
                    _d5_arr_I  = np.array(_diag5_I_buf)
                    _d5_arr_N1 = np.array(_diag5_N1_buf)
                    _d5_std_I  = float(np.std(_d5_arr_I))
                    _d5_std_N1 = float(np.std(_d5_arr_N1))
                    if _d5_std_I > 1e-9 and _d5_std_N1 > 1e-9:
                        _d5_corr = float(np.corrcoef(_d5_arr_I, _d5_arr_N1)[0, 1])
                    else:
                        _d5_corr = float("nan")
                except Exception:
                    _d5_corr = float("nan")
                ...
                _d5_corr = float("nan")
            print(f"[IONO_DIAG] SOD={sod:.0f}  corr(I,N1)={_d5_corr:+.3f}  n={len(_diag5_I_buf)}")
            _diag5_I_buf.clear()
            _diag5_N1_buf.clear()

            # 3. PC_DIFF — mean |L1m - P1c| in first 10 min
            if _diag5_pc_diff_10min:
                _d5_mean_D = float(np.mean(_diag5_pc_diff_10min))
            else:
                _d5_mean_D = float("nan")
            print(f"[PC_DIFF] SOD={sod:.0f}  mean_abs_D(first-10min)={_d5_mean_D:.4f}m  n={len(_diag5_pc_diff_10min)}")
            # do NOT clear _diag5_pc_diff_10min — it is first-10-min-only and never grows further

            # 4. IONEX influence — avg |I_model - I_est|
            if _diag5_ionex_diff_buf:
                _d5_avg_diff = float(np.mean(_diag5_ionex_diff_buf))
            else:
                _d5_avg_diff = float("nan")
            print(f"[IONEX] SOD={sod:.0f}  avg_diff=|I_model-I_est|={_d5_avg_diff:.4f}m  n={len(_diag5_ionex_diff_buf)}")
            _diag5_ionex_diff_buf.clear()
        # ── end PART 5 diagnostics ────────────────────────────────────────────────
        ...
        _up_err_now = float(np.dot(dx, _e_up_ecef)) * 1e3
        _up_err_hist.append(_up_err_now)         # v117 FIX5: feed detrend history for hump-bin RMS
        # v116 FIX5: accumulate rolling window for Up/ZWD/CLK correlation
        _corr_up_buf.append(_up_err_now)
        _corr_zwd_buf.append(float(x[IDX_ZWD]) * 1e3)
        _corr_clk_buf.append(float(x[IDX_CLK]) * 1e3)
        if len(_corr_up_buf) > _CORR_WIN:
            _corr_up_buf.pop(0); _corr_zwd_buf.pop(0); _corr_clk_buf.pop(0)
        if not hasattr(_ppp_pass, '_up_err_window'):
            _ppp_pass._up_err_window = []
        _ppp_pass._up_err_window.append(_up_err_now)
        if len(_ppp_pass._up_err_window) > 300:
            _ppp_pass._up_err_window.pop(0)
        # ISB tracking — append every epoch when Galileo is present
        if 'E' in constellation:
            _isb_buf.append(x[IDX_ISB_GAL])
        ...
        if nproc % 300 == 0 and sidx:
            ...
            print(f"[SUMMARY] SOD={sod:.0f}  sats={len(geom)}"
                  ...
                  f"  3D={d3:.1f}mm")
            # ── Step 5: WL per-sat stability ─────────────────────────────────
            for _wl_sid in sorted(set(list(_wl_fix_count.keys()) +
                                      list(_wl_rel_count.keys()))):
                _nf = _wl_fix_count.get(_wl_sid, 0)
                _nr = _wl_rel_count.get(_wl_sid, 0)
                print(f"[WL_STABILITY] sat={_wl_sid} fixes={_nf} releases={_nr}")
            # ── Step 6: WL_STATS compact summary ─────────────────────────────
            print(f"[WL_STATS] SOD={sod:.0f} "
                  f"fixed={len(wl_fixed)} "
                  f"active={len([m for m in geom if m['sid'] in wl_fixed])}")
            # single clock + ISB diagnostic
            if 'E' in constellation:
                _clk_isb_sum2 = x[IDX_CLK] + x[IDX_ISB_GAL]
                _isb_std = float(np.std(_isb_buf)) if len(_isb_buf) > 1 else float('nan')
                print(f"[CLK] SOD={sod:.0f}  dt={x[IDX_CLK]:+.4f} m  "
                      f"ISB={x[IDX_ISB_GAL]:+.4f} m  "
                      f"dt+ISB={_clk_isb_sum2:+.4f} m  "
                      f"ISB_std(win)={_isb_std*1e3:.1f}mm  "
                ...
                if len(code_res) != len(geom):
                    print(f"[WARN] code_res({len(code_res)}) != geom({len(geom)})")
                ...
                _cres_G = [cr for cr, m in zip(code_res, geom) if m['sid'][0]=='G']
                _cres_E = [cr for cr, m in zip(code_res, geom) if m['sid'][0]=='E']
                _crms_G = math.sqrt(np.mean(np.array(_cres_G)**2))*1e3 if _cres_G else float('nan')
                _crms_E = math.sqrt(np.mean(np.array(_cres_E)**2))*1e3 if _cres_E else float('nan')
                print(f"[RES]  phase_RMS: GPS={_prms_G:.1f}mm  GAL={_prms_E:.1f}mm  "
                      f"code_RMS: GPS={_crms_G:.1f}mm  GAL={_crms_E:.1f}mm")
            ...
            _sig_nGPS_ar_acc     = 0
            _sig_nGPS_fb_acc     = 0
            # ── DIAG: trim running residual lists to avoid unbounded growth ──
            _diag_res_gps_phase  = _diag_res_gps_phase[-6000:]
        ...
        # code_res already computed above (before nproc++ / warn check) — do not recompute
        ...
        # v121 PART4: Fisher information contribution split (iono observability)
        if ep_idx % 300 == 0:
            _tot4 = _ep_code_info_acc + _ep_phase_info_acc
            _c_pct = 0.0; _p_pct = 0.0   # v122: initialize before conditional
            if _tot4 > 0:
                _c_pct = 100.0 * _ep_code_info_acc / _tot4
                _p_pct = 100.0 * _ep_phase_info_acc / _tot4
                print(f"[IONO_INNOV_SPLIT] SOD={sod:.0f}  code_pct={_c_pct:.2f}%  phase_pct={_p_pct:.2f}%")

            # ── v122 PART1: [IONO_DIAG_FULL] ────────────────────────────────
            _gf_mean_sig_now = (math.sqrt(sum(_gf_R_diag) / len(_gf_R_diag))
                                if _gf_R_diag else float('nan'))
            # mean code sigma (sqrt of mean Rd for P1/P2 rows, pre-rejection)
            _code_sigs = []; _phase_sigs = []
            for _dri, _dm in enumerate(geom):
                _db = 4 * _dri
                if Rd[_db]   < 1e5: _code_sigs.append(math.sqrt(Rd[_db]))
                if Rd[_db+1] < 1e5: _code_sigs.append(math.sqrt(Rd[_db+1]))
                if Rd[_db+2] < 1e5: _phase_sigs.append(math.sqrt(Rd[_db+2]))
                if Rd[_db+3] < 1e5: _phase_sigs.append(math.sqrt(Rd[_db+3]))
            _mean_code_sig  = float(np.mean(_code_sigs))  if _code_sigs  else float('nan')
            _mean_phase_sig = float(np.mean(_phase_sigs)) if _phase_sigs else float('nan')
            _ratio_gf_ph  = ((_gf_mean_sig_now / _mean_phase_sig)**2
                             if math.isfinite(_gf_mean_sig_now) and _mean_phase_sig > 0
                             else float('nan'))
            _ratio_co_ph  = ((_mean_code_sig / _mean_phase_sig)**2
                             if _mean_phase_sig > 0 else float('nan'))
            # mean absolute innovations
            _code_z_abs   = [abs(float(z_p[4*_ri]))    for _ri in range(len(geom)) if Rd_p[4*_ri] < 1e5]
            _phase_z_abs  = [abs(float(z_p[4*_ri+2]))  for _ri in range(len(geom)) if Rd_p[4*_ri+2] < 1e5]
            _gf_z_abs     = [abs(float(z_p[n_total+1+_gi])) for _gi in range(_gf_used_count)]
            _mean_abs_code  = float(np.mean(_code_z_abs))  if _code_z_abs  else float('nan')
            _mean_abs_phase = float(np.mean(_phase_z_abs)) if _phase_z_abs else float('nan')
            _mean_abs_gf    = float(np.mean(_gf_z_abs))    if _gf_z_abs    else float('nan')
            # iono state dynamics this epoch
            _dI_st_ep  = [abs(x[_m['ki']] - _I_before_update.get(_m['sid'], x[_m['ki']]))
                          for _m in geom]
            _dI_co_ep  = [abs(_I_meas_ep.get(_m['sid'], float('nan'))
                              - _v122_I_meas_prev.get(_m['sid'], float('nan')))
                          for _m in geom
                          if math.isfinite(_I_meas_ep.get(_m['sid'], float('nan')))
                             and math.isfinite(_v122_I_meas_prev.get(_m['sid'], float('nan')))]
            _mean_dI_st = float(np.mean(_dI_st_ep)) if _dI_st_ep else float('nan')
            _mean_dI_co = float(np.mean(_dI_co_ep)) if _dI_co_ep else float('nan')
            # geometry check — condition number of H_p (position rows)
            try:
                _H_pos = H_p[:, :3]
                _cond_H  = float(np.linalg.cond(_H_pos)) if _H_pos.shape[0] >= 3 else float('nan')
                _rank_H  = int(np.linalg.matrix_rank(H_p))
            except Exception:
                _cond_H = float('nan'); _rank_H = -1
            # coupling check — P-matrix correlations
            _corr_I_N1_vals = []; _corr_I_ZWD_vals = []; _corr_I_CLK_vals = []
            for _cm in geom:
                _cki = _cm['ki']; _ckn1 = _cki + 1
                _pi  = max(P[_cki, _cki], 1e-20)
                _pn1 = max(P[_ckn1, _ckn1], 1e-20)
                _pzw = max(P[IDX_ZWD, IDX_ZWD], 1e-20)
                _pck = max(P[IDX_CLK, IDX_CLK], 1e-20)
                _corr_I_N1_vals.append(P[_cki, _ckn1] / math.sqrt(_pi * _pn1))
                _corr_I_ZWD_vals.append(P[_cki, IDX_ZWD] / math.sqrt(_pi * _pzw))
                _corr_I_CLK_vals.append(P[_cki, IDX_CLK] / math.sqrt(_pi * _pck))
            _mean_corr_IN1  = float(np.mean(_corr_I_N1_vals))  if _corr_I_N1_vals  else float('nan')
            _mean_corr_IZWD = float(np.mean(_corr_I_ZWD_vals)) if _corr_I_ZWD_vals else float('nan')
            _mean_corr_ICLK = float(np.mean(_corr_I_CLK_vals)) if _corr_I_CLK_vals else float('nan')
            print(f"\n[IONO_DIAG_FULL] SOD={sod:.0f}")
            print(f"  1. code_pct={_c_pct:.2f}%  phase_pct={_p_pct:.2f}%")
            print(f"  2. mean_GF_sigma={_gf_mean_sig_now:.3f}m")
            print(f"  3. mean_code_sigma={_mean_code_sig:.3f}m")
            print(f"  4. mean_phase_sigma={_mean_phase_sig:.4f}m")
            print(f"  5. ratio_phase_to_GF  = {_ratio_gf_ph:.1f}  (GF/phase variance ratio)")
            print(f"  6. ratio_phase_to_code = {_ratio_co_ph:.1f}  (code/phase variance ratio)")
            print(f"  7. mean_abs_innovation: code={_mean_abs_code:.3f}m  "
                  f"phase={_mean_abs_phase:.4f}m  GF={_mean_abs_gf:.3f}m")
            print(f"  8. iono_state_dynamics: mean|dI_state|={_mean_dI_st:.4f}m  "
                  f"mean|dI_meas_code|={_mean_dI_co:.4f}m")
            print(f"  9. geometry: rank(H)={_rank_H}  cond(H_pos)={_cond_H:.1f}  "
                  f"nG={len(_dbg_el_G)}  nE={len(_dbg_el_E)}")
            print(f" 10. coupling: corr(I,N1)={_mean_corr_IN1:+.3f}  "
                  f"corr(I,ZWD)={_mean_corr_IZWD:+.3f}  corr(I,CLK)={_mean_corr_ICLK:+.3f}")

            # ── v122 PART5: [GEOM_CHECK] ─────────────────────────────────────
            _nG_gc = len(_dbg_el_G); _nE_gc = len(_dbg_el_E)
            _mel_G = float(np.mean(_dbg_el_G)) if _dbg_el_G else float('nan')
            _mel_E = float(np.mean(_dbg_el_E)) if _dbg_el_E else float('nan')
            print(f"[GEOM_CHECK] SOD={sod:.0f}  nGPS={_nG_gc}  nGAL={_nE_gc}  "
                  f"mean_el_GPS={_mel_G:.1f}°  mean_el_GAL={_mel_E:.1f}°  "
                  f"rank_H={_rank_H}  cond_H_pos={_cond_H:.1f}")

            # ── v122 PART4: [IONO_RESPONSE] for top 5 satellites ─────────────
            _iono_resp = []
            for _irm in geom:
                _sidi = _irm['sid']
                _gam_i = _irm['gamma']
                _dI_st_i = x[_irm['ki']] - _I_before_update.get(_sidi, x[_irm['ki']])
                _I_now_i  = _I_meas_ep.get(_sidi, float('nan'))
                _I_prev_i = _v122_I_meas_prev.get(_sidi, float('nan'))
                _dI_co_i  = (_I_now_i - _I_prev_i
                             if math.isfinite(_I_now_i) and math.isfinite(_I_prev_i)
                             else float('nan'))
                _ratio_i  = (abs(_dI_st_i) / abs(_dI_co_i)
                             if math.isfinite(_dI_co_i) and abs(_dI_co_i) > 1e-6
                             else float('nan'))
                _iono_resp.append((_sidi, _dI_st_i, _dI_co_i, _ratio_i))
            _iono_resp.sort(key=lambda t: abs(t[1]), reverse=True)
            print(f"[IONO_RESPONSE] SOD={sod:.0f}  top-5 by |dI_state|:")
            for _irr in _iono_resp[:5]:
                _sir, _dst, _dco, _rr = _irr
                _dco_str = f"{_dco:+.4f}m" if math.isfinite(_dco) else "N/A"
                _rr_str  = f"{_rr:.3f}" if math.isfinite(_rr) else "N/A"
                print(f"  sat={_sir}  dI_state={_dst:+.4f}m  "
                      f"dI_code={_dco_str}  R=|dI_st|/|dI_co|={_rr_str}")

        # v122 PART4: update previous-epoch I_meas store
        _v122_I_meas_prev = dict(_I_meas_ep)
    ...
    if DISABLE_WL_FIXING and len(wl_fixed) != 0:
        print("[ERROR] WL state leak detected — force clearing")
        wl_fixed.clear()

    # ── IONO LAG DIAGNOSTIC FINAL OUTPUT ────────────────────────────────────
    # PART 1: I_state dynamics  PART 2: tracking error  PART 3: lag correlation
    # Read-only analysis — no filter state modified.
    print(f"\n{'='*68}")
    print(f"[IONO_DIAG] label={_iono_diag_label}  ION_PROC_NOISE≈5e-4*scale m²/s")
    print(f"{'='*68}")
    _ld_all_sats = sorted(_iono_state_hist.keys())
    for _sid_ld in _ld_all_sats:
        _Ist_ld   = np.array(_iono_state_hist[_sid_ld], dtype=float)
        _Ims_ld   = np.array(_iono_meas_hist[_sid_ld],  dtype=float)
        _n_ld     = len(_Ist_ld)
        if _n_ld < 20:
            continue
        # PART 1 — I_state dynamics
        _dI_st    = np.diff(_Ist_ld)
        _mean_dI  = float(np.mean(_dI_st))
        _std_dI   = float(np.std(_dI_st))
        _max_dI   = float(np.max(np.abs(_dI_st)))
        print(f"[IONO_STATE_DYN]"
              f"  sat={_sid_ld}"
              f"  n={_n_ld}"
              f"  mean_dI={_mean_dI:+.4f}m"
              f"  std_dI={_std_dI:.4f}m"
              f"  max_dI={_max_dI:.4f}m")
        # PART 2 — tracking error vs measurement-based iono
        _valid_ld = np.isfinite(_Ims_ld) & np.isfinite(_Ist_ld)
        if _valid_ld.sum() > 20:
            _diff_ld   = _Ims_ld[_valid_ld] - _Ist_ld[_valid_ld]
            _t_ld      = np.arange(_valid_ld.sum(), dtype=float)
            _mn_diff   = float(np.mean(_diff_ld))
            _sd_diff   = float(np.std(_diff_ld))
            _drift_diff = float(np.polyfit(_t_ld, _diff_ld, 1)[0]) if len(_t_ld) > 2 else 0.0
            print(f"[IONO_TRACK_ERROR]"
                  f"  sat={_sid_ld}"
                  f"  mean={_mn_diff:+.4f}m"
                  f"  std={_sd_diff:.4f}m"
                  f"  drift={_drift_diff:+.6f}m/ep")
            # PART 3 — lag: corr(dI_state, dI_meas) contemporaneous
            _dI_ms_ld  = np.diff(_Ims_ld[_valid_ld])
            _dI_st_ld  = np.diff(_Ist_ld[_valid_ld])
            _min_n_ld  = min(len(_dI_st_ld), len(_dI_ms_ld))
            if _min_n_ld > 10 and np.std(_dI_ms_ld[:_min_n_ld]) > 1e-6 and np.std(_dI_st_ld[:_min_n_ld]) > 1e-6:
                _corr_ld = float(np.corrcoef(
                    _dI_st_ld[:_min_n_ld], _dI_ms_ld[:_min_n_ld])[0, 1])
                # lag-1 cross-correlation: corr(dI_state(t), dI_meas(t-1))
                # high lag-1 corr relative to lag-0 → state lags measurement by 1 epoch
                _corr_lag1 = float(np.corrcoef(
                    _dI_st_ld[1:_min_n_ld], _dI_ms_ld[:_min_n_ld-1])[0, 1]) if _min_n_ld > 11 else float('nan')
                print(f"[IONO_LAG]"
                      f"  sat={_sid_ld}"
                      f"  corr_lag0={_corr_ld:+.4f}"
                      f"  corr_lag1={_corr_lag1:+.4f}"
                      f"  {'LAG_SUSPECTED' if (np.isfinite(_corr_lag1) and _corr_lag1 > _corr_ld + 0.05) else 'tracking_ok'}")
                # v121 PART5: IONO_OBSERVABILITY — ratio of state dynamics to measurement dynamics
                # R = |dI_state| / |dI_meas|  target: 0.1–0.5 (too low=state frozen, too high=noisy)
                if len(_dI_st_ld) > 10 and np.std(_dI_ms_ld[:_min_n_ld]) > 1e-6:
                    _obs_R = float(np.mean(np.abs(_dI_st_ld[:_min_n_ld])) /
                                   (np.mean(np.abs(_dI_ms_ld[:_min_n_ld])) + 1e-9))
                    print(f"[IONO_OBSERVABILITY]  sat={_sid_ld}  mean_R={_obs_R:.3f}"
                          f"  {'OK' if 0.1 <= _obs_R <= 0.5 else ('FROZEN' if _obs_R < 0.1 else 'NOISY')}")
    print(f"{'='*68}\n")
    ...
    for _fh in (_nl_debug_fh, _float_diag_fh, _nl_events_fh, _summary_fh):
        _fh.flush(); _fh.close()
    # ── v122 OUTPUT: [FINAL_DIAGNOSIS] ──────────────────────────────────────
    # ── v123_PCAUDIT PARTS 3+8+9: classification, validation, final report ──
    try:
        # PART 3 — Classify per-satellite root cause over first 2h
        _pc_inconsistent_count = 0
        _pc_max_D0 = 0.0
        _pc_gps_drift  = []
        _pc_gal_drift  = []
        for _sid_pc, _hist_pc in _pc_diff_hist.items():
            if len(_hist_pc) < 10:
                continue
            _arr_pc = np.array(_hist_pc)
            _mn_pc  = float(np.mean(_arr_pc))
            _sd_pc  = float(np.std(_arr_pc))
            # Linear drift (m/hour): fit slope over available epochs (30 s spacing)
            _ep_pc  = np.arange(len(_arr_pc))
            _drift_m_ep = float(np.polyfit(_ep_pc, _arr_pc, 1)[0])   # m/epoch
            _drift_hr   = _drift_m_ep * 3600.0 / 30.0                # m/hour (30s epochs)
            # Root-cause classification
            if _sd_pc < 1.0 and abs(_mn_pc) > 5.0:
                _type_pc = 'A'   # constant bias (OSB frame / signal mismatch)
            elif _sd_pc > 1.0 and abs(_drift_hr) > 1.0:
                _type_pc = 'B'   # drift (iono not converging, ambiguity floating)
            elif _sd_pc > 3.0:
                _type_pc = 'C'   # noise (code multipath / scintillation)
            else:
                _type_pc = 'OK'
            _pc_2h_stats[_sid_pc] = {'mean': _mn_pc, 'std': _sd_pc,
                                      'drift': _drift_hr, 'type': _type_pc}
            print(f"[PC_CLASS] sat={_sid_pc} type={_type_pc} "
                  f"mean={_mn_pc:+.3f}m  std={_sd_pc:.3f}m  drift={_drift_hr:+.3f}m/hr")
            # Count BAD initializations
            if _pc_init_flag.get(_sid_pc, 'OK') == 'BAD':
                _pc_inconsistent_count += 1
            _D0_abs = abs(_pc_init_D.get(_sid_pc, 0.0))
            if _D0_abs > _pc_max_D0:
                _pc_max_D0 = _D0_abs
            # Accumulate drift by constellation
            if _sid_pc[0] == 'G':
                _pc_gps_drift.append(_drift_hr)
            elif _sid_pc[0] == 'E':
                _pc_gal_drift.append(_drift_hr)

        # PART 8 — Validation: check mean|D| < 1m within first 20 min
        print(f"\n[PC_VALIDATION] First-20min mean|D| per satellite:")
        _pc_val_ok_count  = 0
        _pc_val_bad_count = 0
        for _sid_v, _vbuf in sorted(_pc_val_buf.items()):
            if len(_vbuf) < 5:
                continue
            _mean_absD = float(np.mean(_vbuf))
            _ok_str = 'OK' if _mean_absD < 1.0 else 'HIGH'
            if _ok_str == 'OK':
                _pc_val_ok_count += 1
            else:
                _pc_val_bad_count += 1
            if _ok_str == 'HIGH' or nproc < 40:   # always print during first 20 min
                print(f"  sat={_sid_v} mean|D|={_mean_absD:.3f}m  {_ok_str}")
        print(f"  → {_pc_val_ok_count} sats OK (<1m),  {_pc_val_bad_count} sats HIGH (≥1m)")

        # PART 9 — Final PC report summary
        _pc_gps_mean_drift = float(np.mean(_pc_gps_drift)) if _pc_gps_drift else float('nan')
        _pc_gal_mean_drift = float(np.mean(_pc_gal_drift)) if _pc_gal_drift else float('nan')
        _D0_all = [abs(v) for v in _pc_init_D.values()]
        _mean_D0 = float(np.mean(_D0_all)) if _D0_all else float('nan')
        _max_D0  = float(np.max(_D0_all))  if _D0_all else float('nan')
        print(f"\n{'='*68}")
        print(f"[PC_FINAL_REPORT]")
        print(f"  max_initial_|D|          = {_max_D0:.3f}m  (target <5m)")
        print(f"  mean_initial_|D|         = {_mean_D0:.3f}m")
        print(f"  inconsistent_start_count = {_pc_inconsistent_count}  (|D0|>5m)")
        print(f"  n1_init_corrected        = {len(_pc_n1_corrected)}  sats: {sorted(_pc_n1_corrected)}")
        print(f"  GPS_mean_drift_2h        = {_pc_gps_mean_drift:+.3f}m/hr  (typ. <2m/hr OK)")
        print(f"  GAL_mean_drift_2h        = {_pc_gal_mean_drift:+.3f}m/hr")
        _n_type = {'A': 0, 'B': 0, 'C': 0, 'OK': 0}
        for _s in _pc_2h_stats.values():
            _n_type[_s.get('type', 'OK')] = _n_type.get(_s.get('type', 'OK'), 0) + 1
        print(f"  classification_A(bias)   = {_n_type['A']}  sats (const OSB bias)")
        print(f"  classification_B(drift)  = {_n_type['B']}  sats (iono/ambiguity drift)")
        print(f"  classification_C(noise)  = {_n_type['C']}  sats (code noise/multipath)")
        print(f"  classification_OK        = {_n_type['OK']}  sats")
        print(f"  startup_relax_applied    = {_startup_relax_printed}  (code σ×3 for first 1h)")
        print(f"{'='*68}\n")
    except Exception as _pce:
        print(f"[PC_FINAL_REPORT] (skipped: {_pce})")
    # ── end v123_PCAUDIT ───────────────────────────────────────────────────────
    # Summarise the dominant limitation from accumulated evidence
    # Evidence: code_pct, GF mean_sigma, rejection ratio, iono-ambiguity coupling
    # These are derived from the last-epoch snapshot values already computed above.
    try:
        _fd_cr_ratio = (_cr_rejected / _cr_total) if _cr_total > 0 else 0.0
        _fd_cr_avg   = (_cr_ratio_sum / _cr_ratio_cnt) if _cr_ratio_cnt > 0 else float('nan')
        _fd_gf_sig   = (math.sqrt(sum(_gf_R_diag) / len(_gf_R_diag))
                        if _gf_R_diag else float('nan'))
        # Accumulate last-seen code_pct (IONO_INNOV_SPLIT) from state
        _fd_c_pct = (100.0 * _ep_code_info_acc /
                     (_ep_code_info_acc + _ep_phase_info_acc + 1e-30))
        # P-matrix mean corr(I, N1) for coupling check
        _fd_corr_IN1_vals = []
        for _fds in sidx.values():
            _fdn1 = _fds + 1
            if _fdn1 < len(P):
                _fp = max(P[_fds, _fds], 1e-20)
                _fn = max(P[_fdn1, _fdn1], 1e-20)
                _fd_corr_IN1_vals.append(P[_fds, _fdn1] / math.sqrt(_fp * _fn))
        _fd_corr_IN1 = (float(np.mean(_fd_corr_IN1_vals))
                        if _fd_corr_IN1_vals else float('nan'))
        # Score each limitation hypothesis
        _lim_weighting  = _fd_c_pct < 2.0       # code barely contributes → weighting
        _lim_rejection  = _fd_cr_ratio > 0.010  # >1% rejected → rejection issue
        _lim_degeneracy = (math.isfinite(_fd_corr_IN1)
                           and abs(_fd_corr_IN1) > 0.50)  # high iono-ambiguity corr
        _lim_geometry   = False   # would need PDOP > threshold; skip for now
        if _lim_rejection:
            _dom_lim = "rejection"
            _rec_act = "v123: gate widened to 5.5σ (hard cap 20m); monitor code_pct — target >2%; if still high, check OSB quality"
        elif _lim_degeneracy:
            _dom_lim = "degeneracy (iono-ambiguity)"
            _rec_act = "ambiguity constraints are insufficient to separate I and N1; consider tighter NL constraint or longer convergence"
        elif _lim_weighting:
            _dom_lim = "weighting imbalance"
            _rec_act = "GF/code still too weak relative to phase; consider further reducing GF_CODE_SCALE or increasing code weight"
        else:
            _dom_lim = "mixed / converging"
            _rec_act = "run complete arc and re-evaluate IONO_DIAG_FULL at midpoint"
        print(f"\n{'='*68}")
        print(f"[FINAL_DIAGNOSIS]")
        print(f"  code_pct         = {_fd_c_pct:.2f}%  (target >2% for meaningful code influence)")
        print(f"  mean_GF_sigma    = {_fd_gf_sig:.3f}m  (target <1.5m; was ~5m in v121)")
        print(f"  rejection_ratio  = {_fd_cr_ratio:.4f}  (flag if >0.01)")
        print(f"  avg_code_res/sig = {_fd_cr_avg:.3f}  (flag if >0.8)")
        print(f"  corr(I,N1)       = {_fd_corr_IN1:+.3f}  (flag if |corr|>0.5)")
        print(f"  dominant_limitation = {_dom_lim}")
        print(f"  recommended_next_action = {_rec_act}")
        print(f"{'='*68}\n")
    except Exception as _fde:
        print(f"[FINAL_DIAGNOSIS] (skipped: {_fde})")

    print(f"[CSV]  logs/ → nl_debug2.csv  float_diag2.csv  nl_events2.csv  summary2.csv")
    return results,nom+x[:3],x[IDX_CLK],x[IDX_ZWD],wl_fixed,fwd_amb_out,_amb_snapshots,_elev_bins
...
def postpos(ts,te,ti,tu,popt,sopt,fopt,infiles,outfile,rov=None,base=None):
    ...
    inx_f=_f(['.inx','.INX'])   # IONEX GIM (v124 external iono constraint)
    ...
    # v124: IONEX external ionosphere constraint
    ionex_data = None
    if inx_f:
        try:
            ionex_data = parse_ionex(inx_f)
            print(f"[IONEX] Loaded from {os.path.basename(inx_f)}")
        except Exception as _ionex_err:
            print(f"[IONEX] WARNING: failed to load {inx_f}: {_ionex_err}")
            ionex_data = None
    else:
        print("[IONEX] No .INX file found — iono constraint disabled")
    ...
    print(f"\n{'='*72}")
    print("[v122] GPS+Galileo PPP — ZWD Q×1.5 restored + prior σ=8cm + height gate=3h/20cm (pure float)")
    print(f"{'='*72}")
    ...
    # ── Results summary ────────────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print("[v122] RESULTS SUMMARY  (pure float, v120: ZWD Q×0.3 + prior=3cm + h-gate=1.5h + prior σ=8cm + height gate=3h/R=(0.20)²)")
    print(f"{'='*72}")
    ...
    try:
        ...
        fig_v.suptitle(
            'PPP v122 — ZWD Q×1.5 restored + Prior σ=8cm + Height Gate=3h/20cm  (pure float)',
            fontsize=11, fontweight='bold')
        ...
        fig_a.suptitle('PPP v122 — Full Analysis  (pure float)',
        ...
        print(f"[v122] Analysis plot saved: {_ap}")
        plt.close(fig_a)

    except Exception as _ve:
        print(f"[v106] Plot failed: {_ve}")

    print(f"\n[v122] Wall time: {_time.time() - t0:.1f}s")
    print("="*72)
